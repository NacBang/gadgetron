...
select(.event_kind == "chat") | length >= 1`. This proves the
record_post write path actually persists end-to-end instead of
silently warn-and-dropping on INSERT failure, and that the admin
endpoint's `WHERE tenant_id = $ctx.tenant_id` honors the caller's
tenant boundary. Total: 64 → 93 PASS). Run
