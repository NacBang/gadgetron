...
Max-Age sanity. Harness 121 → 126. **#259 7v.6** — ISSUE 16 TASK
16.1 shipped the unified Bearer-or-cookie `auth_middleware`;
7v.6 runs a fresh login then reaches `/admin/users` (Management
scope) AND `/quota/status` (OpenAiCompat scope) using the cookie
alone (no `Authorization: Bearer` header), proving role→scope
synthesis (admin → `[OpenAiCompat, Management]`) and
`Uuid::nil()` sentinel compatibility with admin handlers.
Harness 126 → 129. **#267 7v.7** — ISSUE 21 / v0.5.13 shipped the pg `audit_log` consumer (`run_audit_log_writer`); 7v.7 verifies persistence (`SELECT COUNT(*) FROM audit_log ≥ 1` after a chat completion) + Bearer-caller `actor_api_key_id` non-NULL end-to-end. Harness 129 → 131. **#269 7v.8** — ISSUE 22 / v0.5.14 shipped the admin `/audit/log` query endpoint; 7v.8 pins response shape + OpenAiCompat key → 403 scope enforcement. Harness 131 → 133. **#271 7k.6b + Gate 13 regex-fix** — ISSUE 23 / v0.5.15 added `billing_events.actor_user_id` column; Gate 7k.6b asserts per-kind nullability directly against Postgres (chat + action rows MUST carry NULL pre-ISSUE-24, tool rows MUST carry non-NULL from `ctx.actor_user_id`) — 3 assertions; Gate 13 regex expanded to accept new `billing_events` tracing fields added by the ISSUE 23 column (+1 assertion). Harness 133 → 137. Total: 64 → 137 PASS). Run
...
| 7v.6 | unified Bearer-or-cookie auth middleware | PR #259 / ISSUE 16 TASK 16.1: fresh `/auth/login` → save cookie jar → call two endpoints WITHOUT any `Authorization: Bearer` header — (a) Management-scoped `GET /api/v1/web/workbench/admin/users` must return 200 with `.users` array (proves bootstrap admin's role=admin was mapped to `[OpenAiCompat, Management]` scopes in `validate_session_and_build_key`'s role→scope synthesis); (b) OpenAiCompat-scoped `GET /api/v1/web/workbench/quota/status` must return 200 (proves non-admin narrower endpoints also reached). Negative assertion: no Bearer + no cookie → 401 (middleware fallthrough, not a new error class). Catches: (a) middleware regression where Bearer-absent path short-circuits without checking cookies, (b) `validate_session_and_build_key` session-expiry check regression that would let stale cookies reach scoped endpoints, (c) role→scope mapping drift (admin silently losing Management scope, or member silently gaining it), (d) tenant-boundary regression where a cookie from tenant-A somehow returns tenant-B's rows. Sentinel pinned: `.users[]` works from cookie auth proves `api_key_id = Uuid::nil()` sentinel doesn't trip any downstream `NotNull` assumption in admin handlers. |
| 7v.7 | `audit_log` pg consumer writes rows | PR #267 / ISSUE 21 TASK 21.1: after an earlier chat completion (Gate 8 already landed `AuditEntry` on the mpsc), short sleep for async drain, then direct-Postgres `SELECT COUNT(*) FROM audit_log` from the pg container via `docker compose exec` (2 assertions — total ≥ 1 and `WHERE actor_api_key_id IS NOT NULL` ≥ 1). Proves end-to-end: chat handler → `TenantContext.actor_api_key_id` (Bearer → `Some(key_id)` ISSUE 20 plumbing) → `AuditEntry.actor_api_key_id` → `run_audit_log_writer` → `audit_log.actor_api_key_id` column. Catches regressions in: (a) consumer not spawning when `pg_pool.is_some()` (would leave `audit_log` empty despite tracing line firing), (b) nil-tenant-id skip logic widening (would attempt INSERT for auth-failure sentinel and violate `audit_log_tenant_id_fkey`), (c) `actor_*` column drift in the INSERT statement. |
| 7v.8 | admin `/audit/log` query endpoint | PR #269 / ISSUE 22 TASK 22.1: Management key `GET /api/v1/web/workbench/admin/audit/log?limit=5` → 200 + `.rows` is an array + `.rows | length >= 1`; OpenAiCompat key on same URL → 403 `forbidden` (scope enforcement). Catches: (a) handler skipping the tenant-pin (widens query beyond ctx.tenant_id), (b) `query_audit_log` prepared-statement drift between the 4 filter-permutation shapes, (c) `ListAuditLogResponse` rename of `rows` / `returned` top-level fields (wire contract regression), (d) scope-guard regression exposing the endpoint to OpenAiCompat scope. |
| 7k.6b | `billing_events.actor_user_id` per-kind nullability | PR #271 / ISSUE 23: 3 direct-Postgres assertions (via `docker compose exec postgres psql`) — chat rows MUST carry `actor_user_id IS NULL` (QuotaToken doesn't thread user_id until ISSUE 24), tool rows MUST carry non-NULL count ≥ 1 (populated from `ctx.actor_user_id` → `ValidatedKey.user_id` via ISSUE 17 plumbing), action rows MUST carry `actor_user_id IS NULL` (AuthenticatedContext.user_id is an api_key_id placeholder pre-ISSUE-24; security review flipped to NULL pre-publish to avoid contaminating the ledger with api_key_ids typed as user_ids). Catches: (a) QuotaEnforcer trait signature drift quietly threading `api_key_id` into the chat `actor_user_id` slot, (b) action handler accidentally passing `Some(actor.user_id)` where `actor.user_id` is an api_key_id placeholder, (c) tool handler regression dropping the `Some(ctx.actor_user_id)` argument. SQL-layer assertion because `BillingEventRow` doesn't (yet) flatten `actor_user_id` into the `/admin/billing/events` JSON response. |
