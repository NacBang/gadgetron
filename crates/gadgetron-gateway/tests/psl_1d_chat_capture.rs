//! Chat-capture integration test — first `capture_action` call site.
//!
...
// ---------------------------------------------------------------------------
// Successful non-streaming chat capture
// ---------------------------------------------------------------------------
...
async fn psl_1d_successful_non_streaming_chat_completions_captures_one_event() {
    ...
    // audit_event_id must be a freshly generated Uuid::new_v4(), NOT a
    // reused request_id. The handler generates it once per outcome and
    // threads it through both the AuditEntry and the capture event so
    // audit_log.id ↔ captured_activity_event.audit_event_id JOIN is
    // unambiguous.
    assert!(
    ...
    assert_ne!(
        ...
        "audit_event_id MUST differ from request_id"
    );
}
