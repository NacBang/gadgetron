...
async fn csp_currently_omits_trusted_types_for_nextjs_compat() {
    // Trusted Types was temporarily relaxed:
    ...
    //
    // This integration test pins the current (intended) state — re-introduce
    // trusted-types and restore the positive assertion when the web UI
    // migrates off inline scripts.
