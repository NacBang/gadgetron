//! Knowledge candidate fixture-diff test.
//!
//! The test exercises the full in-memory candidate slice:
//!
...
// ---------------------------------------------------------------------------
// Minimal fake projection — this test only exercises the candidate
...
async fn kc1_fixture_diff_shared_context_reflects_decide_candidate() {
    ...
    // Print the normalized blocks so the first CI run records them in
    // the test output (helpful for review / follow-up change diffs).
    println!("=== fixture-diff pre-decide (normalized) ===\n{normalized_pre}\n");
    println!("=== fixture-diff post-decide (normalized) ===\n{normalized_post}\n");
}
