...
///
/// First step toward real bundle manifests. Future work layers
/// per-bundle scope isolation, multi-bundle aggregation, and signed
/// manifests on top of this same struct.
...
pub struct BundleMetadata {
    ...
    pub version: String,
    /// Bundle-level scope floor. When set,
...
/// [`DescriptorCatalog::from_toml_file`].
///
/// ```toml
/// # Optional bundle metadata. Absent = anonymous.
/// [bundle]
/// id = "gadgetron-core"
/// version = "0.1.0"
///
...
/// (tens of entries). No `Arc` indirection needed.
#[derive(Debug, Clone)]
pub struct DescriptorCatalog {
    ...
    /// `None` for the hand-coded `seed_p2b()` catalog and for
    /// anonymous flat-TOML files that didn't declare a `[bundle]`
    /// table.
    pub(crate) bundle: Option<BundleMetadata>,
    /// Bundles that contributed to this catalog when built via
    /// [`Self::from_bundle_dir`]. Empty in all other constructors.
    /// A merged catalog has `bundle = None` AND `bundles = [...]`;
    /// a single-bundle catalog has `bundle = Some(meta)` AND
    /// `bundles = []`.
...
impl DescriptorCatalog {
    ...
    /// Hand-coded seed catalog for integration testing.
    ///
    /// Contains **one view** (`knowledge-activity-recent`) and **one action**
    /// (`knowledge-search`) wired to the candidate coordinator.
    pub fn seed_p2b() -> Self {
        ...
        let actions = vec![
            ...
            WorkbenchActionDescriptor {
                ...
                destructive: false,
                requires_approval: false,
                knowledge_hint: "wiki.write 가젯을 직접 호출합니다. 현재는 승인 흐름이 stub — 직접 기록됩니다.".into(),
                required_scope: None,
                disabled_reason: None,
            },
            WorkbenchActionDescriptor {
            ...
            WorkbenchActionDescriptor {
                // The canonical approval-gated action.
                // `destructive: true` funnels the invoke through step 6
                // `pending_approval`, which with an ApprovalStore wired
                // (production) persists a real ApprovalRequest. Approve
                // via the approval endpoint → `wiki.delete` dispatches
                // against the wiki.
                id: "wiki-delete".into(),
                title: "위키 삭제".into(),
                owner_bundle: "core".into(),
    ...
    ///
    /// This split is enforced at two layers — the descriptor-level
    /// scope filter strips inaccessible entries, and the
    /// instance-level policy gates the entire catalog.
    ...
    pub fn from_toml_file(path: &std::path::Path) -> gadgetron_core::error::Result<Self> {
        ...
        // Bundle-level scope floor. If the
    ...
    /// Each immediate subdirectory that contains
...
mod tests {
    ...
    fn from_toml_file_round_trips_a_minimal_catalog() {
        // File-based catalog source. Write a tiny
...
"#,
    ...
    fn first_party_gadgetron_core_bundle_file_loads_cleanly() {
        // First-party bundle on disk that mirrors
    ...
    fn from_bundle_dir_merges_multiple_bundles() {
        // Two bundle subdirectories, each with its
