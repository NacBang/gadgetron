...
pub struct InProcessWorkbenchActionService {
    ...
    /// `invoke` loads the current snapshot once so catalog reads and
    /// validator lookups see a consistent view — a concurrent reload
    /// cannot land a new catalog with stale validators or vice versa.
    ...
    /// Process-local billing insert failure counter,
...
impl InProcessWorkbenchActionService {
    ...
    /// Attach a Postgres pool for billing-ledger writes.
    ...
    /// Attach the process-local billing failure counter.
...
impl WorkbenchActionService for InProcessWorkbenchActionService {
    ...
    async fn invoke(
        ...
        // cannot half-swap underneath us, and
        ...
        if needs_approval {
            ...
            self.audit_sink
                .send(ActionAuditEvent::DirectActionCompleted {
                    ...
                    actor_user_id: actor.real_user_id.unwrap_or(actor.api_key_id),
                    tenant_id: actor_tenant_id,
...
/// `source_event_id` is the audit event UUID so the ledger joins
/// cleanly to `action_audit_events`. `cost_cents=0` today —
/// dispatcher doesn't surface cost yet; the invoice materializer
/// applies per-kind pricing at query time. No-op when `pool` is
/// `None` (test / no-DB deploys).
///
/// **`actor_user_id` sourcing.** Callers pass `actor.real_user_id` —
/// NOT `actor.user_id`. The legacy `user_id` field on
/// `AuthenticatedContext` is still an api_key_id placeholder, so
/// sourcing it here would re-introduce a type-confusion bug.
/// `real_user_id` carries the real user id via
/// `TenantContext.actor_user_id` plumbing, `None` for legacy api_keys
/// predating the user backfill.
...
mod tests {
    ...
    /// the gateway's standard threaded scope. Tests that want a
    /// scope-gated action use an empty slice explicitly.
    ...
    async fn invoke_seeded_action_succeeds_and_status_is_ok() {
        ...
        // audit_event_id is now populated on every ok path.
        assert!(
    ...
    async fn invoke_seeded_action_with_coordinator_captures_activity() {
        ...
        // audit_event_id is populated on every ok path.
        assert!(
            resp.result.audit_event_id.is_some(),
            "audit_event_id must be set on ok"
        );
