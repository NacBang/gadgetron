...
/// the actual refill time. A future iteration can thread the real
/// refill time through the token so the header reports the exact
/// countdown.
...
impl IntoResponse for ApiError {
    fn into_response(self) -> Response {
        ...
        // Quota-exceeded responses carry a
...
mod tests {
    ...
    async fn quota_exceeded_response_carries_retry_after_header_and_field() {
        // 429 responses must include both the
