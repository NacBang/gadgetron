//! Streaming chat SSE Drop-guard.
//!
//! Background: the non-streaming chat handler emits an AuditEntry +
...
impl Drop for StreamEndGuard {
    fn drop(&mut self) {
        ...
        // Compute cost_cents at amendment time from
        ...
        // Mirror the audit emission onto the live
...
mod tests {
    ...
    /// of the amendment; the capture path is exercised via integration
    /// tests (see `tests/psl_1d_chat_capture.rs`).
