...
///   `/api/v1/web/workbench/*`     → `Scope::OpenAiCompat`
...
///
/// Innermost layer of the auth stack. Budget: ~1µs (Vec<Scope> scan,
/// max 3 elements).
///
/// 403 scope failures MUST be audited per SOC2 CC6.7.
pub async fn scope_guard_middleware(
    ...
    let required_scope: Option<Scope> = if path.starts_with("/v1/") {
        ...
        // Admin subtree (catalog reload, future bundle install/uninstall)
        // requires Management — workbench users (OpenAiCompat) must NOT
        // be able to reload the catalog. Must appear BEFORE the broader
        // workbench match.
        Some(Scope::Management)
    } else if path.starts_with("/api/v1/web/workbench/") {
        // The workbench projection lives under /api/v1/* but uses
        // OpenAiCompat base scope (same as /v1/*). Finer-grained access
        // control lives in descriptor metadata. Must appear BEFORE the
