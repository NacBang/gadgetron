...
//! methods. [`InProcessPennySharedSurfaceService`] implements it by
//! delegating to the [`WorkbenchProjectionService`].
//!
//! # Assembler
//!
...
//! No I/O; no `async`. Designed for testing and for runtime wiring.
...
pub trait PennySharedSurfaceService: Send + Sync {
    ...
    ///
    /// `limit` is clamped to `[1, 20]` by the implementation. Returns
    /// `Ok(vec![])` until the candidate-coordinator is wired.
    ...
    ///
    /// `limit` is clamped to `[0, 10]` by the implementation. Returns
    /// `Ok(vec![])` until the approval store is wired.
    ...
    ///
    /// Returns `Err(GadgetronError::Penny { kind: ToolDenied, … })`
    /// until the candidate-coordinator is wired.
...
/// backing store with the same filter rules.
///
/// # Optional candidate wiring
///
/// `candidate_store` and `candidate_coordinator` are `Option` so existing
/// callers (and tests) continue to compile with `None`. When `Some`,
/// `pending_candidates` serves a live projection and `decide_candidate`
/// routes through the real coordinator; when `None`, `pending_candidates`
/// returns `Ok(vec![])` and `decide_candidate` returns a `ToolDenied`
/// stub so the gateway still boots cleanly against TOML that hasn't
/// wired the candidate store yet.
pub struct InProcessPennySharedSurfaceService {
    ...
    /// `None` preserves the legacy stub behavior.
    pub candidate_store: Option<Arc<dyn ActivityCaptureStore>>,
    /// Capture-plane coordinator — reserved for future use by
    /// `capture_action` through this service. Held as a field now so
    /// the wiring surface is in place even though only `candidate_store`
    /// is currently consulted on the read path.
    pub candidate_coordinator: Option<Arc<dyn KnowledgeCandidateCoordinator>>,
}

impl InProcessPennySharedSurfaceService {
    /// Construct with just the workbench projection (matches existing
    /// stub call sites). Leaves the capture-plane wiring unset.
    ...
    /// Wire the capture-plane dependencies onto an existing service.
    ///
...
impl PennySharedSurfaceService for InProcessPennySharedSurfaceService {
    ...
    async fn pending_candidates(
        ...
        // When the capture store is wired, serve a live projection.
        // Preserves the empty-vec behavior when `candidate_store` is None
        // so the gateway still boots cleanly against TOML that hasn't
        // wired the capture store yet.
    ...
    async fn decide_candidate(
        ...
        // When the capture store is wired, route decisions through the
        // real coordinator. Preserves the `ToolDenied` stub when
        // `candidate_store` is None so callers still get a typed
        // 403-equivalent if the capture plane hasn't been started yet.
        let Some(store) = self.candidate_store.as_ref() else {
        ...
        Ok(PennyCandidateDecisionReceipt {
            ...
            // Materialization is future work.
...
fn candidate_to_digest(candidate: KnowledgeCandidate) -> PennyCandidateDigest {
    ...
    PennyCandidateDigest {
        ...
        // Materialization status is added once writeback wires up.
        materialization_status: None,
    }
}
...
fn workbench_err_to_gadgetron(err: WorkbenchHttpError) -> GadgetronError {
    match err {
        ...
        // Approval variants — Penny shared-context
...
// `gadgetron_core::knowledge::candidate::snake_case_label`. Duplicate
// helpers in sibling crates were deleted because divergence would break
// the `<gadgetron_shared_context>` wire label.
...
mod tests {
    ...
    async fn pending_candidates_returns_empty_when_store_none() {
        // Preserves the legacy stub behavior — when `candidate_store`
        // is None, the read serves an empty Vec without contacting any
        // backend.
        let svc = make_service();
    ...
    async fn decide_candidate_denied_when_store_none() {
        // Preserves the stub behavior as a typed 403-equivalent so
        // production TOML that hasn't wired the candidate store still
        // gives Penny a clean error rather than a panic.
        let svc = make_service();
