//! Activity-capture helpers — fire-and-forget Penny shared-surface
...
//!   capture.
//! - `stream_end_guard.rs` — streaming stream-end amendment.
...
///
/// `audit_event_id` is the correlation key that joins this
/// `captured_activity_event` row to the matching `audit_log` row.
/// Callers MUST pass the SAME UUID they gave to `AuditWriter::send`.
/// Reusing `request_id` instead is a bug.
///
/// `actor_user_id` is a real parameter. Gateway callers MUST populate
/// it from the authenticated request (placeholder:
/// `TenantContext::api_key_id` until a real user table materialises).
...
pub async fn capture_chat_completion(
    ...
    let actor = AuthenticatedContext {
        ...
        // signature is orthogonal to billing scope — deferred to
        // future work when activity events need real-user attribution.
        real_user_id: None,
    };
...
pub fn error_class(e: &GadgetronError) -> &'static str {
    match e {
        ...
        GadgetronError::ConversationOwnershipMismatch => "conversation_ownership_mismatch",
