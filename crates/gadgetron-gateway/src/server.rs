...
pub struct AppState {
    ...
    /// Pre/post quota enforcement. Default: `InMemoryQuotaEnforcer`.
    ...
    /// Workbench projection service. Existing tests that build
    /// `AppState` without this field use `..Default::default()` or set
    /// `workbench: None` explicitly.
    pub workbench: Option<Arc<GatewayWorkbenchService>>,
    /// Penny shared surface service. Populated from the production
    /// `InProcessPennySharedSurfaceService`.
    pub penny_shared_surface: Option<Arc<dyn PennySharedSurfaceService>>,
    /// Agent configuration — shared_context sub-section drives bootstrap
    ...
    /// `DefaultPennyTurnContextAssembler` with `penny_shared_surface`.
    /// The handler uses this trait object to avoid handling the generic
    /// type parameter inline.
    pub penny_assembler:
        Option<Arc<dyn gadgetron_core::agent::shared_context::PennyTurnContextAssembler>>,
    /// Capture-plane store. The in-memory implementation lives in
    /// `gadgetron_knowledge::candidate::InMemoryActivityCaptureStore`
    /// and is used by tests + the fixture-diff gate.
    pub activity_capture_store: Option<Arc<dyn ActivityCaptureStore>>,
    /// Capture-plane coordinator. Mirrors `activity_capture_store` so
    /// they can be wired together at startup.
    ...
    /// `/api/v1/web/workbench/events/ws` WebSocket handler. Cheap
    /// `Clone`; each handler publish is fire-and-forget.
    pub activity_bus: gadgetron_core::activity_bus::ActivityBus,
    /// Read-only Gadget schema catalog — typically the same frozen
    /// `GadgetRegistry` Penny uses for dispatch, erased behind
    /// `GadgetCatalog` so the gateway does not need a dependency on
    /// `gadgetron-penny`. Present when `[knowledge]` was configured and
    /// Penny was registered at startup; `None` otherwise. Drives the
    /// `GET /v1/tools` MCP discovery endpoint so external MCP clients
    /// can enumerate available tools.
    ...
    /// independent of `gadgetron-penny`. Drives
    /// `POST /v1/tools/{name}/invoke` so external MCP clients can
    /// actually call the tools they discover via `/v1/tools`. Present
    /// when `[knowledge]` was configured and Penny was registered at
    /// startup; `None` otherwise, in which case
    /// `/v1/tools/{name}/invoke` returns 503.
    ...
    /// authenticated `TenantContext`. This correlates external-agent
    /// tool calls with Penny's `tool_audit_events` table: Penny calls
    /// land with `owner_id = None`, external MCP calls land with
    /// `owner_id = Some(api_key_id)`, so operators can query
    /// `WHERE owner_id IS NOT NULL` to pick out cross-session callers.
    /// `NoopGadgetAuditEventSink` when Postgres is not wired.
    pub tool_audit_sink: Arc<dyn gadgetron_core::audit::GadgetAuditEventSink>,
    /// Process-local billing insert failure counter. Incremented by
    /// all 3 billing emission sites (chat enforcer, tool handler
    /// spawn, action_service::emit_action_billing) when
    /// `insert_billing_event` returns Err. Exposed via
...
// ---------------------------------------------------------------------------
// Health handlers — real implementations (always 200)
// ---------------------------------------------------------------------------
...
///   counter. Scrape + alert on any non-zero delta.
///
...
pub async fn metrics_handler(State(state): State<AppState>) -> impl IntoResponse {
    ...
    let body = format!(
        "# HELP gadgetron_billing_insert_failures_total \
         ...
         Counter resets on restart; long-horizon reconciliation is future work.\n\
...
pub fn build_router(state: AppState) -> Router {
    ...
    let public_routes = Router::new()
        ...
        // Google OAuth sign-in. Routes 404 when `[auth.google]`
