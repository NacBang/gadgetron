-- Identity + self-service foundation.
--
-- Adds the `users` / `teams` / `team_members` / `user_sessions` tables.
-- Extends `api_keys` with `user_id` + `label` for key ownership and
-- `audit_log` with actor columns so every audit row can attribute to a
-- specific user and key.
--
-- All new rows default to the hardcoded "default" tenant
-- (`00000000-0000-0000-0000-000000000001`). This migration does NOT
-- insert that row — an earlier migration (or the bootstrap flow) owns it.
--
-- No retroactive `users` rows are inserted here. The bootstrap flow
-- creates the first admin from `[auth.bootstrap]` config when the `users`
-- table is empty. Existing `api_keys` rows temporarily carry user_id =
-- NULL (column is nullable here); a follow-up backfill script NULLs land
-- on, then flips the column to NOT NULL in a later migration.
...
-- ---------------------------------------------------------------------------
-- api_keys ← user_id + label (nullable for now; tightened by a follow-up backfill)
-- ---------------------------------------------------------------------------
