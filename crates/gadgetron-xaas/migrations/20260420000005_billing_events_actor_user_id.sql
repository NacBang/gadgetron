-- Add `actor_user_id` to billing_events so per-user spend reports can
-- select WHERE actor_user_id = $1 (with tenant_id also pinned) without
-- joining to audit_log. Nullable because:
--   * billing events emitted before this column was added don't carry it
--   * chat path passes NULL today (QuotaToken doesn't thread user_id yet)
--   * action path passes NULL (`AuthenticatedContext.user_id` is an
--     api_key_id placeholder in the workbench; only tool-path with an
--     updated AuthenticatedContext can populate it safely)
--   * legacy keys predating the user-id backfill still have
