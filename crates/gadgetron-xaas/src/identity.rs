//! Identity operations — user CRUD helpers.
//!
//! Typed `sqlx::query_as` wrappers around the `users` table that the
//! gateway's admin handlers (and the user CLI) both call. Kept
...
/// for rows that predate the user-table backfill. Idempotent — rows already carrying
