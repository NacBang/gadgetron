...
pub struct QuotaToken {
    ...
    /// Owning user id. Threaded from the handler via
    /// `QuotaEnforcer::check_pre` → sourced from
    /// `TenantContext.actor_user_id` (`ValidatedKey.user_id`).
    /// `None` for legacy api_keys predating the user backfill;
    /// otherwise `Some(real_users_id)`.
    /// `PgQuotaEnforcer::record_post` reads this and populates
    /// `billing_events.actor_user_id` so chat rows carry the owning
    /// user for per-user spend queries.
...
pub trait QuotaEnforcer: Send + Sync {
    /// `user_id` is the owning user id from the caller's
...
/// Postgres-backed quota enforcer.
///
...
pub struct PgQuotaEnforcer {
    pool: sqlx::PgPool,
    /// Process-local counter incremented when
...
impl QuotaEnforcer for PgQuotaEnforcer {
    async fn check_pre(
        ...
        // Future iterations may replace this with a per-request fresh
        // query to catch races between concurrent requests, but the
        // snapshot reflects reality closely enough given the
        // rate-limit gate upstream.
        if snapshot.remaining_daily_cents() <= 0 {
            return Err(GadgetronError::QuotaExceeded { tenant_id });
        }
        Ok(QuotaToken::new(tenant_id, 0, user_id))
    ...
    async fn record_post(&self, token: &QuotaToken, actual_cost_cents: i64) {
        ...
        // Integer-cent billing ledger. Every chat completion that
        // reaches `record_post` is a billable event; persist one
        // `billing_events` row so the invoice query path has the raw
        // data. We don't skip zero-cost rows (mock / free-tier
        // models) — the ledger is the append-only record of activity,
        // not just paid activity.
        // Failures log but don't propagate — the request already
        // succeeded, and the operator sees the failure in tracing.
        // `token.user_id` (sourced from `ctx.actor_user_id` at the
        // handler's check_pre call) populates `actor_user_id` so
        // chat rows carry owning-user attribution alongside tool +
        // action paths.
...
/// today — the gateway's `ApiError::into_response` already adds the
/// `Retry-After` header + `retry_after_seconds` body field to every
/// 429, so clients get a usable back-off hint without threading the
/// limiter's exact refill time through the error type. A future
/// iteration can widen `GadgetronError` with a dedicated variant
/// carrying `retry_after_seconds` from the limiter's
/// `RateLimitedError`.
...
mod tests {
    ...
    async fn check_pre_threads_user_id_into_token() {
        // Trait-level contract: `check_pre` MUST
