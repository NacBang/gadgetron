...
pub struct AuditEntry {
    ...
    pub tenant_id: Uuid,
    pub api_key_id: Uuid,
    /// Owning user. Populated from `ValidatedKey.user_id` when available
    /// (both Bearer and cookie-session paths). `None` for legacy keys
    /// predating the user backfill. Persistence into
    /// `audit_log.actor_user_id` follows when the pg audit consumer
    /// lands.
...
/// INSERTs one row per entry into `audit_log`. Exits cleanly when the
/// channel closes (all writers dropped).
///
...
///
/// Columns: a cookie-session caller (nil-sentinel `api_key_id`)
/// persists with `actor_api_key_id = NULL` and the Bearer caller gets
/// the real key id.
/// Projection row for `GET /api/v1/web/workbench/admin/audit/log`.
/// Mirrors the `audit_log` table schema — one row per billable /
/// auditable event.
