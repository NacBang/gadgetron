...
impl GadgetAuditEventSink for GadgetAuditEventWriter {
    fn send(&self, event: GadgetAuditEvent) {
        ...
        // Fan out to the capture coordinator so the
...
///
/// Wires the Penny tool-call audit trail to Postgres persistence.
/// Consumes the event stream that the Penny session emitter pushes.
