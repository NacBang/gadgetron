//! Billing event writer + query helpers.
...
pub struct BillingEventRow {
    ...
    /// Owning user. Denormalized projection of
    ...
    ///
    /// NULL for:
    ///   * legacy events pre-dating the actor_user_id migration
    ///   * callers whose `ValidatedKey.user_id` is None (legacy
    ///     api_keys predating the user backfill)
...
impl BillingEventInsert {
    ...
    /// `actor_user_id`) default to NULL; future work populates
    /// `actor_user_id` from `QuotaToken.user_id`.
    ...
    /// today — the invoice materializer (deferred) attributes cost at
    /// aggregation time from the underlying model call. `gadget_name`
    /// is stored in the `model` column, not a dedicated slot — legacy
    /// mapping matches the pre-refactor call at `handlers.rs`. Future
    /// work queues the real model identifier into its own field.
    ...
    /// Set `actor_user_id`. NULL is always acceptable
