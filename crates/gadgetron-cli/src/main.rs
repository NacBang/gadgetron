...
fn prepare_penny_router_registration(
    ...
    //
    // ISSUE 6 TASK 6.2: when a `KnowledgeCandidateCoordinator` is
...
async fn init_serve_runtime(
    ...
    // ISSUE 6 TASK 6.2: knowledge service + candidate coordinator must
    // be built BEFORE Penny registration so the audit sink can be
    // wired with `.with_coordinator(coord)` and fan out tool calls
    // to `/workbench/activity`. Previously this ran AFTER
    // build_provider_maps so the coord wasn't available yet.
...
"#,
    ...
    fn psl_1c_no_knowledge_section_leaves_observability_fields_none() {
        ...
        // build_workbench(None, None, None, None, None, None) → Some (degraded-mode projection —
        // always wired so the endpoint returns a degraded bootstrap rather than 404).
        let workbench = build_workbench(None, None, None, None, None, None);
        assert!(
            workbench.is_some(),
