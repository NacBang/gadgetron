...
async fn cmd_mcp_serve(config_path_override: Option<PathBuf>) -> Result<()> {
    ...
    let registry = load_penny_registry_from_config_for_mcp(&config_path, &agent_cfg)
        .await?
        .ok_or_else(|| {
            anyhow::anyhow!(
...
fn load_penny_registry_from_config(
    ...
    let provider = gadgetron_knowledge::KnowledgeToolProvider::new(knowledge_cfg, None)
...
async fn load_penny_registry_from_config_for_mcp(
    config_path: &std::path::Path,
    agent_cfg: &gadgetron_core::agent::AgentConfig,
) -> Result<Option<Arc<gadgetron_penny::McpToolRegistry>>> {
    let Some(knowledge_cfg) = load_knowledge_config_from_path(config_path)? else {
    ...
    let provider = gadgetron_knowledge::KnowledgeToolProvider::new(knowledge_cfg, pg_pool)
        .map_err(|e| anyhow::anyhow!("failed to open knowledge provider: {e:?}"))?;
