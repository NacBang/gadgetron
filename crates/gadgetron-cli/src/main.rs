...
struct AppStateParts {
    ...
    /// Process-local billing insert failure counter shared with the
    /// chat enforcer, tool handler, and action service.
...
enum Commands {
    ...
    /// Manage users. Requires PostgreSQL.
    User {
        #[command(subcommand)]
        command: UserCmd,
    },

    /// Manage teams. Requires PostgreSQL.
    Team {
        #[command(subcommand)]
        command: TeamCmd,
    },
    ...
    ///
    /// Operator-facing list/info subcommands are currently stubs and
    /// will ship behind the `bundle` + `gadget list` workflow.
    Gadget {
        #[command(subcommand)]
        command: GadgetCmd,
    },

    /// DEPRECATED: renamed to `gadgetron gadget serve`.
    ///
    ...
    /// Manage installed Bundles (list/install are currently stubs).
    Bundle {
        #[command(subcommand)]
        command: BundleCmd,
    },
    ...
    /// core port (LlmProvider, Extractor, BlobStore, etc.). Stub.
    Plug {
        #[command(subcommand)]
        command: PlugCmd,
    },

    /// Alias for `gadgetron bundle install <name>`.
    ///
    /// The everyday invocation path; `bundle install` is the canonical
    /// long form for docs and scripts. Currently stubbed.
    Install {
...
/// Subcommands for `gadgetron gadget` (canonical form).
#[derive(Subcommand)]
enum GadgetCmd {
    ...
    /// List every Gadget Penny can see. Currently stubbed; full
    /// implementation lands with the bundle-lifecycle work.
    List,
}
...
/// Subcommands for `gadgetron bundle` (currently stubs).
#[derive(Subcommand)]
enum BundleCmd {
...
/// Subcommands for `gadgetron plug` (currently stubs).
#[derive(Subcommand)]
enum PlugCmd {
...
/// Subcommands for `gadgetron user`.
#[derive(Subcommand)]
enum UserCmd {
...
/// Subcommands for `gadgetron team`.
#[derive(Subcommand)]
enum TeamCmd {
    ...
    match cli.command {
        ...
        Some(Commands::Mcp {
            ...
            tracing::warn!(
                ...
                removed_in = "v0.5",
                "`gadgetron mcp serve` is renamed to `gadgetron gadget serve`. The old form works through v0.3 and v0.4 and will be removed in v0.5. Update scripts, systemd units, and MCP config that invoke this subcommand."
            );
            cmd_gadget_serve(config).await
        }
...
// ---------------------------------------------------------------------------
// Deferred subcommand stubs
//
// These are wired into `clap` now so the operator-facing vocabulary is
// reserved; the real implementations land alongside the bundle manifest
// + registry work. Each stub prints a consistent message so scripts can
// distinguish "not yet implemented" from a hard error.
// ---------------------------------------------------------------------------

fn cmd_bundle_install_stub(name: &str) -> Result<()> {
    println!("bundle install {name}: not yet implemented.");
    Ok(())
}

fn cmd_bundle_list_stub() -> Result<()> {
    println!("bundle list: not yet implemented.");
    Ok(())
}

fn cmd_plug_list_stub() -> Result<()> {
    println!("plug list: not yet implemented.");
    Ok(())
}

fn cmd_gadget_list_stub() -> Result<()> {
    println!("gadget list: not yet implemented.");
    Ok(())
}
...
/// deprecation shim; the shim prints a `tracing::warn!` and forwards
/// unchanged.
async fn cmd_gadget_serve(config_path_override: Option<PathBuf>) -> Result<()> {
    ...
    // registry can enforce L3 defense-in-depth. The stdio Gadget
    // server is a grandchild process that does not go through the
    // gateway's `AppConfig::load` path, so we parse the full
    // AppConfig here and pick out `.agent`. A missing section is
    // tolerated because `AgentConfig` implements
...
fn prepare_penny_router_registration(
    ...
    // Replace the long-standing Noop sink with a
        ...
        pg_pool
    {
        ...
        let writer = match candidate_coordinator {
...
async fn init_database_runtime(use_no_db: bool) -> Result<DatabaseRuntime> {
    let (key_validator, pg_pool): (SharedKeyValidator, Option<sqlx::PgPool>) = if use_no_db {
        ...
        //   DATABASE_URL is wrapped in Secret<String> and is never emitted
...
fn init_audit_runtime(
    ...
    // Prefer the pg consumer when a pool is wired so audit
...
fn build_workbench(
    ...
    // The descriptor catalog is shared via
    ...
    // path so they don't accidentally depend on a live database.
    ...
    // Thread the PG pool so successful direct-action +
...
async fn init_serve_runtime(
    ...
    //   `InMemoryQuotaEnforcer` (check-only, no persistence).
    // - optional rate-limit wrapper: when
    //   `[quota_rate_limit] requests_per_minute > 0`, every request
    //   passes a per-tenant token-bucket check BEFORE the cost check.
    // First-admin bootstrap. Runs once per startup when a pg pool is
    // available; no-op when users table is already populated; hard
    // error if users is empty AND [auth.bootstrap] is missing. Never
    // runs on no-db startups (no users table to check).
    if let Some(pool) = pg_pool.as_ref() {
        ...
        // Backfill api_keys.user_id for rows that predate the user
        // table backfill. Idempotent; only touches NULL rows.
    ...
    // Build the ActivityBus HERE (one shared handle for the whole
    // process) so Penny's GadgetAuditEventWriter and
    ...
    // Knowledge service + candidate coordinator must be built BEFORE
    // Penny registration so the audit sink can be wired with
    ...
    // A second `GadgetAuditEventWriter` for the
    ...
    // live feed and the /workbench/activity capture plane in sync for
    // external callers as well. When `pg_pool` is `None` (no-db mode)
    // we fall back to Noop so unit/test flows keep working.
...
/// This matches the tolerance model for individual provider
...
fn register_penny_if_configured(
    ...
    //
    // The current build wires a `NoopGadgetAuditEventSink` as the audit
    // sink — the real `ToolAuditEventWriter` lives in `gadgetron-xaas`
    // and is connected to the DB writer loop there. The composition
    // root for that wiring lands with the broader `AppState` audit
...
// ---------------------------------------------------------------------------
// Audit consumer loop — current build logs to tracing only
// ---------------------------------------------------------------------------
...
async fn audit_consumer_loop(
    ...
    while let Some(entry) = rx.recv().await {
        // Current build: write to tracing.
        // Future: batch INSERT into audit_log PostgreSQL table.
        tracing::info!(
...
///
/// Supports: OpenAI, Anthropic, Gemini, Ollama, vLLM, SGLang.
...
"#,
    ...
    // ---------------------------------------------------------------------------
    // Penny startup smoke tests
    // ---------------------------------------------------------------------------

    /// Positive smoke test: when `[knowledge]` + `[knowledge.curation]
    ...
    /// Negative smoke test: when `[knowledge]` is absent, all
