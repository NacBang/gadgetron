...
//!
//! `cost_cents` is computed from real token counts so audit rows
//! carry a real number. Having a real number unblocks the
//! `/usage/summary` dashboard, per-tenant invoicing, and quota
//! enforcement against $$ (not just tokens).
//!
