...
//! `PolicyRejected` for an SSRF-blocked URL fetch) is additive. The
//! error-code strings returned by [`ExtractError::code`] are stable and
...
/// these in the `ImportReceipt` for operator review.
#[derive(Debug, Clone)]
...
///
/// `non_exhaustive` so adding (e.g.) `ScannedPdfDetected` stays additive.
/// `Serialize + Deserialize` with `snake_case` so the variant name is
/// stable wire-string for audit filtering.
#[non_exhaustive]
