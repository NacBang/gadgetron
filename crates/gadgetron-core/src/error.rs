...
///   boundary. `ToolApprovalTimeout` is reserved for a future approval-flow
///   release but the variant ships now for forward compatibility of the
///   enum surface.
#[non_exhaustive]
#[derive(Debug, Clone, PartialEq, Eq)]
pub enum PennyErrorKind {
    ...
    /// Tool rate-limit exceeded. Only used by Destructive-tier tools in
    /// future releases. The current build never emits this variant
    /// (T3 `enabled = false` is forced by V5). HTTP 429.
    ToolRateLimited {
        tool: String,
        remaining: u32,
        limit: u32,
    },
    /// Approval flow timed out waiting for user decision. Reserved for a
    /// future approval-flow release. HTTP 504.
...
/// Wiki knowledge layer error kinds.
///
/// Paired with `GadgetronError::Wiki { kind, message }`.
#[non_exhaustive]
#[derive(Debug, Clone, PartialEq, Eq)]
...
/// Knowledge plane nested error kinds.
///
/// Paired with `GadgetronError::Knowledge { kind, message }`. These kinds
/// surface at the `KnowledgeService` boundary — the wrapped `LlmWikiStore`
/// / keyword / semantic index implementations still use `WikiErrorKind`
/// internally, but outward-facing errors are normalized to
/// `KnowledgeErrorKind`.
#[non_exhaustive]
#[derive(Debug, Clone, PartialEq, Eq)]
...
pub enum GadgetronError {
    ...
    /// Chat or workbench request carried a `conversation_id` whose owner is a
    /// different principal than the one making the request. Distinct from
    /// `Forbidden` so the UI can tell "your key lacks permission" apart from
    /// "your browser cached someone else's conversation id". Maps to HTTP 403
    /// with a dedicated `error_code` so the chat client can clear the stale id
    /// and start a new conversation instead of asking the operator to fix
    /// scopes that are actually fine.
    #[error("Forbidden: conversation does not belong to current user")]
    ...
    /// Penny agent subsystem error. Subprocess spawn/run failures, timeouts.
    /// Never contains raw subprocess stderr — only post-redaction content.
    ...
    /// Wiki knowledge layer error. Path traversal, oversize pages,
    /// blocked credentials, git corruption.
    ...
    /// Knowledge plane error. Surfaces at the `KnowledgeService` boundary
    /// when a write/search/traverse call hits backend registration,
    /// availability, query validation, or derived fanout issues.
    ...
    /// Bundle subsystem error. Manifest parse failures, install hook errors,
    /// bundles-home resolution failures.
    ///
...
impl GadgetronError {
    pub fn error_code(&self) -> &'static str {
        match self {
            ...
            Self::ConversationOwnershipMismatch => "conversation_ownership_mismatch",
    ...
    pub fn error_message(&self) -> String {
        match self {
            ...
            Self::ConversationOwnershipMismatch => "This conversation belongs to a different account. Start a new chat to continue.".to_string(),
            ...
            Self::Penny { kind, .. } => match kind {
                ...
                PennyErrorKind::ToolApprovalTimeout { secs } =>
                    format!("A tool call required user approval but none arrived within {secs} seconds. (Approval flow is not yet functional in this build — this error indicates a misconfiguration or a forward-compat path.)"),
    ...
    pub fn error_type(&self) -> &'static str {
        match self {
            ...
            Self::ConversationOwnershipMismatch => "permission_error",
    ...
    pub fn http_status_code(&self) -> u16 {
        match self {
            ...
            Self::Forbidden => 403,
            Self::ConversationOwnershipMismatch => 403,
            Self::Billing(_) => 500,
...
mod tests {
    ...
    fn error_code_returns_stable_machine_string() {
        ...
        assert_eq!(
            GadgetronError::ConversationOwnershipMismatch.error_code(),
    ...
    fn error_type_follows_openai_taxonomy() {
        ...
        assert_eq!(
            GadgetronError::ConversationOwnershipMismatch.error_type(),
            "permission_error",
            "conversation ownership mismatch is still a 403/permission error so SDK clients can match on type without the new code"
        );
        assert_eq!(
            GadgetronError::QuotaExceeded {
                tenant_id: Uuid::nil()
    ...
    fn http_status_codes_match_spec() {
        ...
        assert_eq!(
            GadgetronError::ConversationOwnershipMismatch.http_status_code(),
            403
        );
        assert_eq!(
            GadgetronError::Database {
    ...
    fn all_fourteen_variants_exist() {
        let variants: Vec<GadgetronError> = vec![
            ...
            GadgetronError::ConversationOwnershipMismatch,
        ...
        // Test name says "fourteen" for legacy reasons — the actual contract
        // is "all variants must be enumerated here so the round-trip mappings
        // (error_code / error_type / http_status_code / message) are exercised
        // exhaustively at build time". Bump in lock-step with the enum.
        assert_eq!(variants.len(), 15);
    }

    /// User-visible error message must NEVER contain the `stderr_redacted`
    /// field content, regardless of what that field holds. Generic string only.
    #[test]
