...
pub struct TenantContext {
    ...
    /// Owning user. Populated from `ValidatedKey.user_id` by the
    /// tenant_context middleware. `None` for legacy keys predating the
    /// user backfill. Downstream audit populates
    /// `AuditEntry.actor_user_id` from this.
