//! `bundle.toml` manifest schema.
//!
//! Every Bundle ships a `BundleManifest` either compile-time (Rust-native
//! bundles, via `Bundle::manifest()`) or as a `bundle.toml` file
...
/// Top-level `bundle.toml` shape. Adding a new optional field is a
/// non-breaking change, adding a required field requires a
...
pub struct BundleManifest {
    ...
    /// Per-Gadget cascade dependency map. If any listed Plug is missing
    /// at `Bundle::install` time, the corresponding Gadget is not
    /// registered and a `WARN` is emitted.
    #[serde(default)]
