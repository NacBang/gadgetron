...
//!
//! `BundleRegistry` stores `BundleDescriptor` + registered Plug / Gadget
//! inventory + install status **only**. Live `dyn Bundle` values are
//! dropped after their `install()` returns. Do **not** use
//! `Vec<Arc<dyn Bundle>>` — that makes the `&mut self` mutability
//! contract incoherent (Arc::get_mut only works at refcount-1) and
//! creates the first-reinstall-forces-supersedes failure mode. Any state
//! that must outlive `install` lives in the registered services (the
//! `Arc<T>` stored in each `PlugRegistry<T>`), not in the Bundle object.
//!
//! ## Panic isolation
//!
...
/// `gadgetron bundle info <name>` CLI.
#[derive(Debug, Clone)]
        ...
        for bundle in bundles {
            ...
            match install_result {
                Ok(Ok(())) => {
                    ...
                    // `DisabledByConfig`. Future work adds
                    // `RegistrationFailed` sourcing from `requires_plugs`
                    // cascade.
                    //
                    // The "port" string names whichever axis actually
                    // received the registration — extractor bundles get
                    // `"extractor"`, LLM bundles stay on `"llm_provider"`.
                    // The CLI `gadgetron bundle info` uses this to group
                    // the PLUGS table by axis.
...
mod tests {
    ...
    // ---- Extractor axis — scratch atomicity + lookup ----
    ...
    fn extractor_plug_registration_flows_through_install_all() {
        ...
        // extractor axis — the second scratch BTreeMap must be merged
        // on success and dropped on panic/err.
        let cfg = AppConfig::default();
        let mut reg = BundleRegistry::new();
