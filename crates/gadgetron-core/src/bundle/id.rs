...
//!
//! `PlugId` and `GadgetName` wrap `Arc<str>` so that per-call clones
//! (common in registration paths) are ref-count bumps rather than heap
//! allocations. Kebab-case validation is enforced once at construction
//! time so that every downstream consumer can rely on the invariant.
//!
...
//! surface (see `BundleContext::is_plug_enabled_by_name`). Inside the
