...
//!
//! A `Bundle` is the unit operators install / enable / disable; at
//! `Bundle::install` it registers zero-or-more Plugs (Rust trait impls
//! consumed by core) and zero-or-more Gadgets (MCP tools consumed by
//! Penny) via `BundleContext`.
//!
//! ## Deferred
//!
//! - Populated `GadgetHandles` registration path (no canonical `Gadget`
//!   trait lives in `gadgetron-core` yet).
//! - Non-`LlmProvider` Plug axes beyond `Extractor` (`BlobStore`,
//!   `Scheduler`, `EmbeddingProvider`, `EntityKind`, HTTP routes).
...
impl BundleDescriptor {
    ...
    /// external-runtime loader when it deserializes `bundle.toml`.
...
/// # Trust boundary
///
/// **In-tree Bundles only.** Out-of-tree plugin Bundle impls are
/// unsupported until the plugin SDK stabilizes. User-defined Bundles
/// are a supply-chain review target — review every `Bundle`
...
/// `bundle.toml [runtime] kind = "subprocess"` and the external-runtime
/// enforcement floors (credentials, workdir, filesystem view, tenant
/// identity, audit principal, resource ceilings, egress policy).
pub trait Bundle: Send + Sync + 'static {
    ...
    ///
    /// - `cargo xtask check-bundles` — static lint that verifies
