...
//!
//! `#[must_use]` on the outcome so Bundle authors cannot silently
//! discard a skip decision; strict field-whitelist on the audit emit so
//! Plug `Arc<T>` values (which may embed API keys) are never
//! Debug-printed.
//!
//! ## Audit guarantee
//!
...
/// so operators can debug a cascade without having to re-run with
/// verbose tracing.
...
pub enum RegistrationOutcome {
    ...
    /// upstream disabled). Populated by the `requires_plugs` cascade
    /// resolver.
...
// ---------------------------------------------------------------------------
// Tests — contract + security regression
// ---------------------------------------------------------------------------
...
mod tests {
    ...
    // ---- FakeLlmProvider (inline; promotion to gadgetron-testing is future work) ----
    ...
    /// Companion helper for the extractor-axis scratch map. Every
    /// existing LlmProvider-focused test now allocates a second empty
    /// map alongside `empty_inner` so `BundleContext::new`'s dual-axis
    /// signature compiles.
