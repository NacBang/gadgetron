...
//!
//! Field-form access: Bundle authors see sibling fields (`ctx.plugs.*`,
//! `ctx.gadgets.*`, …) rather than method-form getters. The borrow
//! checker permits disjoint simultaneous `&mut` on sibling fields under
//! standard NLL rules; method-form getters would force sequential
//! borrows for no benefit.
//!
//! ## Ownership model
//!
...
///
/// Currently the `llm_providers` and `extractors` axes are wired.
/// BlobStore / Scheduler / EmbeddingProvider / EntityKind / HTTP route
/// axes land when their Rust traits ship — adding a new axis is an
/// additive change to this struct (no trait break; bundles that don't
/// use a new axis don't compile against it).
...
/// Per-category Gadget registration surface. Scaffold — the canonical
/// `Gadget` / `GadgetRegistry` trait does not live in `gadgetron-core`
/// yet, so this struct is intentionally minimal. Populated registration
/// paths land alongside `BundleRegistry::list_gadgets`.
///
...
///
/// Field-form access pattern: `ctx.plugs.llm_providers.register(...)`,
