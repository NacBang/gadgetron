...
//! capabilities are pluggable" architecture.
//!
//! # What lives here
//!
...
//!   graphify client). Core stays leaf.
//!
//! # How traits compose
//!
...
//! `KnowledgeEngine` trait was considered and rejected.
//!
//! # Error model
//!
...
//! are normalized to
//! [`KnowledgeErrorKind`](crate::error::KnowledgeErrorKind) at the
//! service boundary — no `WikiError` or `sqlx::Error` crosses this trait
//! surface.
//!
...
/// # Identity fields
///
/// Carries the two identity fields every consumer needs: `user_id`
/// (who's acting) and `tenant_id` (which tenant the action runs
/// against). Future ACL / audit-correlation / delegation work may layer
/// scopes on top without further trait-shape changes.
///
...
pub struct AuthenticatedContext {
    ...
    /// an api_key_id placeholder pre-multi-user) and was renamed
    /// to match its actual semantic. See `real_user_id` for the
    ...
    /// The owning user's real id. Populated at the gateway boundary
    /// from [`gadgetron_core::context::TenantContext::actor_user_id`],
    /// which was in turn populated by the auth middleware from
    /// `ValidatedKey.user_id`. `None` for:
    ///   * system / background operations (`AuthenticatedContext::system()`)
    ///   * legacy API keys pre-dating the `api_keys.user_id` backfill
    ///
    /// **Prefer this over `api_key_id` for billing / audit
    /// attribution** so rows do not get contaminated with api_key_ids
    /// typed as user_ids. Audit sinks that still need a non-NULL
    /// user_id should use `actor.real_user_id.unwrap_or(actor.api_key_id)`
    /// as the documented transition fallback.
...
/// trace that produced this write. Candidate materialization threads
/// capture-time hint tags / rationale / source-bundle identifiers into
/// the canonical store. Store implementations SHOULD persist the
/// provenance verbatim (e.g. `LlmWikiStore` merges it into page
/// frontmatter under `provenance:`) and MUST NOT drop unknown keys.
///
...
/// Additional stores MAY be registered for future multi-store scenarios
/// but current validation rejects that case.
///
...
mod tests {
    ...
    // ---- AuthenticatedContext identity invariants ----
