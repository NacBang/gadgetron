...
//!
//! The capture plane / semantic plane split is realized as two traits
//! here:
//!
...
///
/// `#[non_exhaustive]` so future surfaces (e.g. `ExternalRuntime`) can
/// be added without breaking match arms in callers.
#[non_exhaustive]
...
///
/// Candidate heuristics key off these variants (e.g. only `DirectAction`
/// and `GadgetToolCall` produce operational-journal candidates by
/// default). `#[non_exhaustive]` to allow the taxonomy to grow.
#[non_exhaustive]
#[derive(Debug, Clone, Copy, PartialEq, Eq, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
...
/// `#[non_exhaustive]` allows future states (e.g. `Superseded`) to be
/// added without breaking existing match arms in callers.
#[non_exhaustive]
#[derive(Debug, Clone, Copy, PartialEq, Eq, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
...
/// `#[non_exhaustive]` keeps the door open for future additions.
#[non_exhaustive]
#[derive(Debug, Clone, Copy, PartialEq, Eq, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
...
/// path-traversal, and control characters at the type boundary.
...
///
/// `path` is a validated [`KnowledgePath`]. `content` is still free-form
/// markdown. `provenance` is retained for future audit enrichment.
#[derive(Debug, Clone, Serialize, Deserialize)]
...
pub trait ActivityCaptureStore: Send + Sync + std::fmt::Debug {
    ...
    /// distinguish "absent" from "backend down" — the materialization
    /// fast-path uses this instead of scanning `list_candidates` with
    /// `usize::MAX`. No default impl: implementations include
    /// `InMemoryActivityCaptureStore` and the Postgres-backed store.
...
/// 3. Materialize accepted candidates via `KnowledgeService::write`.
#[async_trait]
pub trait KnowledgeCandidateCoordinator: Send + Sync + std::fmt::Debug {
    ...
    ///
    /// Returns the canonical path the candidate was written to. The
    /// in-memory implementation returns `candidate.proposed_path` (or a
    /// synthesized `"ops/journal/<uuid>"` fallback) without touching any
    /// `KnowledgeStore`; the Postgres-backed implementation runs a real
    /// `KnowledgeService::write` call.
    ///
