...
pub struct AppConfig {
    ...
    /// Per-Bundle configuration overrides.
    ...
    /// Per-tenant rate-limit settings. Default `requests_per_minute = 0`
    /// disables rate limiting — preserves the previous behavior for
    /// deployments that haven't opted in.
    #[serde(default)]
    pub quota_rate_limit: RateLimitConfig,
    /// Opt-in feature toggles. Currently only
    /// `tenant_plug_overrides_accepted_as_reserved` — additional toggles
    /// land here as features evolve.
    #[serde(default)]
    pub features: FeaturesConfig,
    /// First-admin bootstrap. When `[auth.bootstrap]`
...
/// Per-tenant token-bucket rate limit. When
...
pub struct WebConfig {
    ...
    /// in the embedded `index.html` at startup using this value.
    #[serde(default = "default_api_base_path")]
    pub api_base_path: String,
    /// Optional path to a TOML file that overrides `seed_p2b()` as the
    /// source for `/api/v1/web/workbench/admin/reload-catalog`.
    /// When set, the reload handler reads and parses this
    ...
    /// single catalog, and swaps it in atomically.
    ///
...
/// Default = unsigned installs allowed, no keys configured —
/// matches the legacy behavior so existing deployments don't need
/// a config bump.
#[derive(Debug, Clone, Default, Serialize, Deserialize)]
pub struct BundleSigningConfig {
    ...
    /// `false` preserves the legacy unsigned-install behavior for deployments that
...
impl WebConfig {
    ...
    /// (`<`, `>`), and JS/CSS string escape (`"`, `'`, backtick).
    /// Called from `AppConfig::load()` after `resolve_env_vars`.
...
// ---------------------------------------------------------------------------
// Bundle / Plug / Gadget config overrides
// ---------------------------------------------------------------------------
//
// Per-Bundle TOML stanzas look like:
//
//     [bundles.ai-infra]
//     enabled = true                    # Bundle enablement
//
//     [bundles.ai-infra.plugs.anthropic-llm]
//     enabled = false                   # per-Plug enable/disable
//
//     [bundles.ai-infra.plugs.anthropic-llm.tenant_overrides]
//     "tenant-a" = { enabled = false }  # reserved (parsed only)
//
//     [bundles.ai-infra.gadgets."gpu.list"]
//     tier = "read"
//     mode = "auto"
//
// The `tenant_overrides` stanza parses but is a no-op for now.
// Operators must acknowledge this by setting
// `[features] tenant_plug_overrides_accepted_as_reserved = true` or
// startup fails with CFG-045.
...
pub struct BundleOverride {
    /// Bundle-level enablement. `false` means no Plugs/Gadgets from
    /// this Bundle register. Defaults to `true` for opt-out behaviour.
    #[serde(default = "default_bundle_enabled")]
    pub enabled: bool,

    /// Per-Plug overrides (per-Plug enable/disable axis). Keyed by
    /// `PlugId` string form; the actual `PlugId` validation happens at
    /// the `Bundle::install` boundary.
    #[serde(default)]
    ...
    /// Runtime ceiling / egress overrides. Operator can tighten
    /// manifest-declared limits via `[bundles.<name>.runtime.limits]`
    /// or `[bundles.<name>.runtime.egress]`. **Currently parsed only** —
    /// the `Bundle::install` hook enforces fail-closed-on-missing (no
    /// ceiling) and merges this override on top of the manifest value.
    #[serde(default)]
    pub runtime: Option<BundleRuntimeOverride>,
}

/// Runtime ceiling / egress overrides. Mirrors the `[bundle.runtime]`
/// shape in `bundle.toml` minus the `kind` / `entry` / `transport`
/// fields (those are manifest-owned; the operator cannot change which
/// runtime dispatches the Gadget, only its resource envelope).
#[derive(Debug, Clone, Default, Serialize, Deserialize)]
...
/// Per-Plug configuration stanza.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct PlugOverride {
    /// Deployment-wide Plug enablement.
    #[serde(default = "default_plug_enabled")]
    pub enabled: bool,

    /// Per-tenant override table. Reserved — parsed but not enforced.
    /// Any non-empty inner map requires
...
/// enums — validation happens at `GadgetRegistry::freeze()` time.
#[derive(Debug, Clone, Default, Serialize, Deserialize)]
...
/// Currently parsed but not enforced; once multi-tenant lands the router
/// will consult `AuthenticatedContext.tenant_id` and apply this flag.
#[derive(Debug, Clone, Serialize, Deserialize)]
...
/// Opt-in feature toggles. Currently only the
/// `tenant_plug_overrides_accepted_as_reserved` acknowledgement gate;
/// additional toggles land here as features evolve.
#[derive(Debug, Clone, Default, Serialize, Deserialize)]
pub struct FeaturesConfig {
    /// Operator acknowledgement that `tenant_overrides` stanzas are
    /// parsed but not enforced. Without this toggle, any non-empty
    /// `tenant_overrides` map refuses startup with CFG-045.
    #[serde(default)]
...
impl AppConfig {
    ...
    /// Enforce that `tenant_overrides` stanzas are parsed but not
    /// enforced. Any non-empty map requires the operator acknowledgement
    /// toggle `[features] tenant_plug_overrides_accepted_as_reserved = true`.
    ///
    /// Without the toggle, startup fails with `CFG-045`. With the
    /// toggle, the parser emits `tracing::warn!` per bundle/plug pairing
    /// naming the reserved stanza.
                ...
                if !ack {
                    return Err(crate::error::GadgetronError::Config(format!(
                        "CFG-045: [bundles.{bundle_name}.plugs.{plug_name}.tenant_overrides] \
                         stanza is reserved and requires [features] \
                         tenant_plug_overrides_accepted_as_reserved = true to accept. \
                         Remove the stanza or set the feature toggle."
                    )));
                }
                // Toggle ack — warn per stanza.
                tracing::warn!(
                    target: "gadgetron_config",
                    bundle = bundle_name.as_str(),
                    plug = plug_name.as_str(),
                    entries = plug.tenant_overrides.len(),
                    "tenant_overrides reserved — enforcement deferred to a future release"
                );
            }
        }
        Ok(())
...
fn migrate_legacy_penny(source: &str) -> crate::error::Result<String> {
    fn emit_deprecation(migration: &'static str) {
        tracing::warn!(
            ...
            migration,
            "deprecated field — will be removed in a future release"
        );
    }
    ...
    if penny.contains_key("claude_model") {
        tracing::error!(
            ...
            "legacy field `[penny].claude_model` is dropped — the agent \
             cannot pick its own brain model. Move brain-selection logic \
             to `[agent.brain]` and remove this field."
        );
    }
...
"#;
    ...
    /// `BundleRuntimeOverride` shape. No enforcement yet — the
    /// `Bundle::install` hook will consume this override.
    #[test]
...
"#;
        ...
        match &err {
            crate::error::GadgetronError::Config(msg) => {
                ...
                assert!(msg.contains("Remove the stanza or set the feature toggle"));
            }
    ...
    /// the validator emits `warn!` per stanza (enforcement is deferred).
    /// Captures via the shared process-global subscriber installed by
    /// `install_tracing_capture()` — avoids `tracing`'s thread-local
...
"#;
        ...
        assert!(
            event.message.contains("reserved") && event.message.contains("deferred"),
            "message must name the reservation + deferral: {}",
            event.message
        );
    }
