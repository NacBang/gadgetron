...
function defaultGroups(
  ...
  if (available.cooling) {
    // Liquid temps: 4 inlet/outlet probes + air. The legacy aggregate
    // `cooling.coolant_temp` was dropped because shipped gadgetini
    // firmware writes garbage values (negatives) into it; the inlet/
    // outlet probes are the source of truth. Delta-t lives on a
    // different scale and gets its own group below.
    groups.push({
      id: "cooling",
      label: "Liquid cooling temperatures",
      unit: "°C",
      fmt: celsiusFmt,
      series: [
        { metric: "cooling.coolant_inlet1_temp", label: "inlet1", tone: SERIES_PALETTE[0] },
        { metric: "cooling.coolant_inlet2_temp", label: "inlet2", tone: SERIES_PALETTE[1] },
        { metric: "cooling.coolant_outlet1_temp", label: "outlet1", tone: SERIES_PALETTE[7 % SERIES_PALETTE.length] },
        { metric: "cooling.coolant_outlet2_temp", label: "outlet2", tone: SERIES_PALETTE[3] },
        { metric: "cooling.air_temp", label: "air", tone: SERIES_PALETTE[2] },
      ],
    });
    groups.push({
      id: "cooling_delta",
      label: "Coolant Δt + air humidity",
      unit: "°C / %",
      fmt: celsiusFmt,
      series: [
        { metric: "cooling.coolant_delta_t", label: "Δt °C", tone: SERIES_PALETTE[6] },
        { metric: "cooling.air_humidity", label: "humidity %", tone: SERIES_PALETTE[4] },
      ],
    });
  }
