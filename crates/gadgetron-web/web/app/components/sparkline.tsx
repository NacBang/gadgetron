...
export function Sparkline({
  ...
  return (
    <div className="flex flex-col gap-0.5" data-testid="sparkline">
      {(label || current) && (
        // Lock the header to a single line so per-GPU current strings
        // ("100/0/0/0/0/0/0/0%") don't wrap and break the grid alignment
        // with adjacent NIC/cooling sparklines. Truncate label first
        // (less critical than current value); current keeps its glyphs.
        <div className="flex min-w-0 items-center justify-between gap-2 whitespace-nowrap text-[10px]">
          {label && (
            <span className="min-w-0 truncate text-zinc-500" title={label}>
              {label}
            </span>
          )}
          {current && (
            <span className="shrink-0 font-mono text-zinc-300">{current}</span>
          )}
        </div>
      )}
      {path ? (
