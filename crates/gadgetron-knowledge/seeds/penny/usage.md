---
tags = ["penny", "getting-started", "usage"]
type = "runbook"
source = "seed"
...
웹 UI(`/web`) 또는 OpenAI 호환 API로 대화합니다. model = `penny`로 요청을 보내면 Penny가 받습니다. 같은 API Bearer 키가 모든 모델 호출에 쓰입니다.
...
Penny는 **매 응답 직후** 이 대화에서 저장할 지식이 있는지 스스로 판단합니다. 사용자가 "기록해줘"라고 지시하면 즉시 저장하고, 지시 없어도 Penny가 가치 있다고 판단하면 저장합니다.
...
| `wiki.search <쿼리>` | 검색 (키워드 + 의미 하이브리드) | 사용자 질문 받으면 먼저 실행 |
...
- **틀렸으면 지적하세요.** "이 페이지 내용 틀렸어" → Penny가 수정하거나 삭제합니다
- **저장을 원하지 않으면 말하세요.** "이건 기록하지 마" → 기록 스킵
- **기존 지식을 찾으세요.** "위키에 X 있어?" → Penny가 검색해서 요약
- **계층을 요청하세요.** "이걸 runbooks/에 저장해줘" → Penny가 경로 존중
