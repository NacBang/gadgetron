...
| Claude Code CLI (2.1.206+) | `claude --version` |
| `claude login` 완료 | `~/.claude/` 디렉토리 존재 |
| Git | `git --version` |
| Node.js + npm (Web UI 빌드에 필요, 선택) | `node --version` |
| PostgreSQL 15+ with pgvector | `psql -c 'SELECT 1'` |
...
- `penny_agent_error` + `exit_code=1` + CLI flag 오류 → Claude Code 버전이 너무 낮음, 2.1.206+ 업그레이드
