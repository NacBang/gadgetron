...
Gadgetron의 집단 기억입니다. 운영자, 사용자, Kairos가 함께 쓰고 읽습니다.

## 시작 문서

- [`kairos/usage.md`](kairos/usage.md) — Kairos와 대화하는 법, 도구 목록, 기록 정책
...
| 폴더 | 용도 | 주 저자 |
|---|---|---|
| `kairos/` | Kairos 자체에 대한 메타 문서, 컨벤션 | 코어 + 사용자 |
| `operators/` | 운영자 온보딩, 장애 대응, 운영 노하우 | 운영자 + Kairos |
| `decisions/` | 팀 설계·운영 결정 기록 (ADR 스타일) | 사용자 + Kairos |
| `runbooks/` | 반복되는 작업의 절차 | 운영자 + Kairos |
...
3. **Kairos는 반복될 만한 지식을 적극 기록합니다.** 저장하지 않을 내용은 명시하세요.
4. **크레덴셜은 자동 차단.** PEM 키·AWS 키 등 명백한 시크릿이 포함된 쓰기는 거부됩니다.

## 수정·삭제

- 아무 파일이나 편집기로 직접 수정 가능. 변경은 다음 `gadgetron reindex` 또는 Kairos 다음 호출 시 인덱스에 반영.
- Kairos에게 말해서 수정/삭제도 가능: "이 페이지 지워줘", "이 내용 틀렸어, 고쳐줘".
