...
//! pgvector don't block this test suite.
//!
...
/// Sanity check: unsupported content-type (e.g. PDF, supported only via
/// the document-formats bundle) is rejected loud and clear.
#[tokio::test]
async fn wiki_import_rejects_unsupported_content_type() {
    ...
    let err = provider
        ...
        .await
        .expect_err("pdf must be rejected without the document-formats bundle");
    use gadgetron_core::agent::tools::GadgetError;
    match err {
        GadgetError::InvalidArgs(msg) => {
            assert!(
