...
fn build_wiki(dir: &TempDir) -> Arc<Wiki> {
    ...
    let cfg = WikiConfig {
        ...
        autocommit: true,
        git_author_name: "Canonical Writeback E2E".into(),
...
async fn kc1b_capture_accept_materialize_then_wiki_search_finds_it() {
    ...
    // `hits[N].path` is still `String` (KnowledgeStore-API paths remain
    // `String`; only the candidate plane narrowed to `KnowledgePath`).
    // Compare via `.as_str()`.
    let match_hit = hits
        .iter()
