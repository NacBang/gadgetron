...
//! # Role
//!
//! The knowledge plane does not talk to `Wiki` directly. `LlmWikiStore`
//! is the **canonical store** that `KnowledgeService` holds an
//! `Arc<dyn KnowledgeStore>` pointer to. All six legacy `wiki.*`
//! gadget surfaces (`list`, `get`, `write`, `delete`, `rename`, `import`)
//! now route through this adapter instead of touching `Wiki` directly.
//!
//! # Error translation
//!
//! The adapter maps every `WikiError` variant to a
//! `GadgetronError::Knowledge { kind: KnowledgeErrorKind::*, .. }` at this
//! boundary. `WikiErrorKind::PageNotFound` → `KnowledgeErrorKind::
...
impl LlmWikiStore {
    ...
    ///
    /// Callers gate this on non-empty `provenance` so legacy writes
    /// are byte-identical.
    ...
    /// future multi-wiki scenarios. Production wiring MUST use
...
impl KnowledgeStore for LlmWikiStore {
    ...
    async fn put(
        ...
        // `create_only` / `overwrite` hints are advisory — the
        // underlying `Wiki::write` always overwrites. Honoring
        // `create_only` requires a `get`-then-`put` race window that
        // is future work. For now the hints pass through but do not
        // block.
        ...
        // When `request.provenance` is non-empty, prepend a deterministic
        // YAML frontmatter block so the audit trail from the candidate
        // plane (hint_reason / hint_tags / source-bundle identifiers)
        // persists into the canonical page. Empty provenance → markdown
        // unchanged, preserving legacy `wiki.write` / RAW-import byte
        // shape.
        //
