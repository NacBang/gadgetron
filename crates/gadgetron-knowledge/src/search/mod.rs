...
//!
//! Currently exposes a single `WebSearch` trait implemented by
//! `SearxngClient`. The trait exists so future providers (Brave Search,
//! Kagi, custom retrieval) can be dropped in via `Arc<dyn WebSearch>`
//! without disturbing the MCP tool surface.
