...
//!
//! # Security — parse error sanitization
//!
...
//!
//! Future security: SSRF — the client follows whatever `searxng_url`
//! points at. The current build accepts this (operator owns the
//! config). Multi-user deployments must add an IP allow-list rejecting
//! 169.254.0.0/16 metadata endpoints, RFC-1918 ranges (unless
//! explicitly whitelisted), and loopback overrides.
