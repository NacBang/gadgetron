...
//!   token count. No TF-IDF, no BM25 — the knowledge base is personal,
//!   not web-scale.
//!
//! # Tokenization
//!
...
//!   A future pass can strip them before indexing; for now they stay in so
...
pub struct WikiSearchHit {
    ...
    pub score: f32,
    /// Optional snippet of surrounding content. Currently `None` —
    /// snippet generation is future work.
...
impl InvertedIndex {
    ...
    /// which tokens came from which page on ingest). For the current
    /// scale this is fine; the wiki's write path triggers a full rebuild
    /// anyway.
    ...
    /// for fast re-insert on re-add); this is fine at the current scale
    /// (<10k pages) and keeps the method allocation-free.
    ///
    /// Supports the incremental `KnowledgeIndex::apply` path in
    /// `keyword_index::WikiKeywordIndex`.
