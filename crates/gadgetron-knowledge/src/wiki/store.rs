...
//! by stat(2), so for the current single-user rate this is well under
//! noise.
//!
//! When multi-user lands, a sharded repo pool (N workers, each with
//! a dedicated `Mutex<Repository>`) can replace the re-open — same
//! public API.
...
impl Wiki {
    ...
    /// every call — at the current scale (<10k pages) this takes
    /// ~20-50 ms which is well below tool-call latency budget. A future
    /// optimization can cache under a `Mutex<Option<InvertedIndex>>`
