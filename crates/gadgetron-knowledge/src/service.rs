...
//! # Write algorithm
//!
//! 1. caller ACL/validation at boundary (currently a placeholder —
//!    actor is not yet consulted for ACL)
...
impl KnowledgeService {
    ...
    ///
    /// Used by the workbench gateway projection to build the bootstrap
    /// response without coupling the gateway to internal plug types.
    ///
    /// Health is currently structural (plug registered = healthy).
    /// Runtime liveness checks (stale index, backend ping) are future
    /// work.
