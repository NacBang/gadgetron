...
///
/// `pub` so `SemanticPgVectorIndex` (which holds an
/// `Arc<SemanticBackend>`) can expose a `pub fn new(...)` surface
/// without leaking visibility. All associated methods stay
/// `pub(crate)` — external callers still must route through
/// `KnowledgeService` / `SemanticPgVectorIndex` for the knowledge plane
/// surface.
#[derive(Clone)]
...
impl SemanticBackend {
    ...
    // Exposed for maintenance tooling; `KnowledgeGadgetProvider` does
    // not call this directly (fanout is always async via
    // `FuturesUnordered`), but `maintenance::run_reindex` may consult
    // the mode for dry-run vs sync decisions.
    #[allow(dead_code)]
