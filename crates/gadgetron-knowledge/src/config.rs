...
pub struct EmbeddingConfig {
    /// Provider name. Currently supports only `openai_compat`.
...
pub struct ReindexConfig {
    ...
    /// Startup behavior. `async` is the default.
    #[serde(default)]
...
///
/// Defaults are chosen so the capture plane stays active even when
/// operator config omits `[knowledge.curation]` entirely.
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(default)]
pub struct KnowledgeCurationConfig {
    ...
    /// Audit capture cannot be disabled here.
    pub enabled: bool,
    ...
    /// accept unilaterally). Free-form strings for now; future work will
    /// narrow this to an enum once the canonical vocabulary is finalized.
...
impl KnowledgeCurationConfig {
    /// Validate curation config. Rules:
    ///
    ...
    ///   root escape" rule.
    /// - `require_user_confirmation_for` accepts any non-empty string;
    ///   future work narrows this to an enum.
    pub fn validate(&self) -> Result<(), String> {
...
pub struct KnowledgeConfig {
    ...
    /// # Single-wiki shape
    ///
    /// The current single-user MVP has `[knowledge]` declare exactly one
    /// wiki root here. This shape will grow into a named registry in a
    /// future release:
    ///
    /// ```toml
    ...
    ///
    /// The multi-tenant audit columns (`owner_id`, `tenant_id` on
    /// `ToolAuditEvent::ToolCallCompleted`) exist specifically so the
    /// audit trail can grow into this registry without a schema
    /// migration. Wiki scoping is the matching write-side evolution —
    /// the `WikiScope` enum is the piece that ties
    /// `owner_id`/`tenant_id` back to which pages each principal can
    /// see.
    ...
    /// Candidate-lifecycle / curation policy. Defaults keep the capture
    /// plane active and allow up to 8 candidates per request; operators
    /// disable the curation loop by setting `enabled = false` (the audit
    /// plane stays on regardless).
    #[serde(default)]
