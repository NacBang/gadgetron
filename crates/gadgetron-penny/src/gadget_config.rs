//! MCP config JSON tempfile writer — bridges Claude Code to the
//! Gadgetron stdio Gadget server.
//!
//! Every Claude Code subprocess invocation writes a fresh JSON tempfile
//! pointing at the `gadgetron gadget serve` stdio subcommand (the CLI
//! keeps a deprecation shim for the old `gadgetron mcp serve` name
//! through v0.4). Claude Code reads the file via `--mcp-config <path>`
//! at startup and launches the stdio MCP server as its own child
//! process. The tempfile is held by the caller (`ClaudeCodeSession`)
//! and removed on drop when the subprocess exits.
//!
...
//!
//! Non-Unix targets fail compilation with a clear message. The current
//! scope is Linux/macOS only.

#[cfg(not(unix))]
compile_error!(
    "gadgetron-penny requires a Unix target (uses mkstemp via the tempfile crate). \
     Windows / WASI support is future work."
);
...
/// `gadgetron serve`.
///
/// The invocation uses the canonical `gadget serve` verb; the old
/// `mcp serve` form remains available as a deprecation shim until v0.4.
///
...
"#,
    ...
    fn tmpfile_has_0600_permissions() {
        // mkstemp atomically sets 0600 on POSIX. Lock it in so a future
        // refactor that moves to `File::create` (which would honor umask
        // and likely produce 0644) fails this test.
        let tmp = write_config_file(None).expect("write");
