...
fn spawn_claude_process(
    ...
    // 4. Concurrent stderr drain. Spawned BEFORE we start reading
    //    stdout so neither pipe can block the other.
...
async fn drive_attempt(
    ...
    .await?
    {
        ...
        StreamLoopOutcome::TimedOut => {
            let _ = process.child.start_kill();
            let _ = process.child.wait().await;
            // `stderr_handle.await` can hang indefinitely if Claude
            // Code does not flush stderr on
...
mod tests {
    ...
    // ---- stderr handle timeout regression lock ----
    ...
    fn stderr_handle_timeout_unblocks_drive_task_on_sigterm_noop() {
        ...
        assert!(
            ...
            "regression: the timed_out cleanup path must wrap \
             `stderr_handle.await` in `tokio::time::timeout(Duration::from_secs(2), \
             stderr_handle).await`. Without the wrapper the drive \
             task hangs on SIGTERM-noop subprocesses."
        );
    }
