...
//! It is wired at the request ingress boundary.
//!
//! # Candidate stubs
//!
//! `workbench.candidates_pending` always returns `Ok([])` in this build.
//! `workbench.candidate_decide` always returns `Err(Denied)` in this build.
...
/// One is constructed per Penny request; it MUST NOT be cached globally.
...
impl<S: WorkbenchService> WorkbenchAwarenessGadgetProvider<S> {
    ...
    async fn call_candidates_pending(&self, args: &Value) -> Result<GadgetResult, GadgetError> {
        ...
        // The candidate query is currently a stub.
        let candidates = self
            .service
    ...
    async fn call_candidate_decide(&self, args: &Value) -> Result<GadgetResult, GadgetError> {
        ...
        // Always Err(Denied) until the real coordinator is wired.
        self.service
...
mod tests {
            ...
            Box::pin(async {
                Err(GadgetronError::Penny {
                    kind: PennyErrorKind::ToolDenied {
                        reason:
                            "candidate-coordinator wiring is pending: candidate decisions are not yet wired"
                                .to_string(),
                    },
    ...
    async fn candidate_decide_returns_denied_result() {
        ...
        assert!(
            result.is_err(),
            "decide should fail with Denied when the coordinator is unwired"
        );
        match result.unwrap_err() {
            GadgetError::Denied { reason } => {
                assert!(
                    reason.contains("candidate-coordinator wiring"),
                    "reason must mention coordinator wiring: {reason}"
                );
            }
