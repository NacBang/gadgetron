...
//!
//! MCP is the wire protocol; Gadgets are the payload.
//!
//! # Protocol
//!
...
//! subprocess model: one `claude -p` invocation → one `gadgetron
//! gadget serve` child → exits together.
//!
//! # Why manual JSON-RPC, not `rmcp`?
//!
//! The `rmcp` Rust SDK was evaluated and its API was unstable at the
//! time of writing. The manual implementation is ~100 lines and only
//! needs to handle four MCP methods, so the complexity cost of the
//! dependency was judged higher than the protocol code itself.
