        ...
        Box::pin(async {
            Err(GadgetronError::Penny {
                kind: PennyErrorKind::ToolDenied {
                    reason: "candidate-coordinator wiring is pending: candidate decisions are not yet wired"
                        .to_string(),
                },
...
fn make_registry() -> GadgetRegistry {
    ...
    // then reaches the test assertions.
...
async fn candidate_decide_returns_denied_through_registry() {
    ...
    // Either way the content or error must indicate denied.
    match result {
