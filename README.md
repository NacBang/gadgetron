...
**Version**: `0.5.3` — **EPIC 1 (Workbench MVP) CLOSED `v0.3.0` (PR #208); EPIC 2 (Agent autonomy) CLOSED `v0.4.0` (PR #209); EPIC 3 (Plugin platform) CLOSED `v0.5.0` (PR #228); EPIC 4 (Multi-tenant business ops / XaaS) ACTIVE** (promoted 2026-04-20) — ISSUE 11 TASK 11.1 shipped as PR #230 / 0.5.1 (structured 429 + `Retry-After: 60` + `retry_after_seconds: 60`); **ISSUE 11 TASK 11.2 shipped as PR #231 / 0.5.2** — per-tenant `TokenBucketRateLimiter` wrapped by `RateLimitedQuotaEnforcer`, opt-in via `[quota_rate_limit]`; **ISSUE 11 TASK 11.3 shipped as PR #232 / 0.5.3** — Postgres-backed daily + monthly spend tracking; `record_post` now runs one UPDATE against `quota_configs` with CASE-expression rollover (daily zeros at UTC midnight, monthly at first-of-month) — no background job, rollover happens on the first post-boundary request; new migration `20260420000001_quota_usage_day.sql` adds `usage_day DATE DEFAULT CURRENT_DATE` to `quota_configs` (default today for existing rows so first post-migration request doesn't spuriously zero monthly usage); new `PgQuotaEnforcer` selected by CLI when `pg_pool.is_some()`, else falls back to `InMemoryQuotaEnforcer`; both are wrapped by `RateLimitedQuotaEnforcer` when `[quota_rate_limit]` is configured; fire-and-forget on UPDATE failure (request already succeeded, error logged via tracing::error but never propagated). Pipeline order: rate-limit check → pg cost check → dispatch → pg record_post. TASK 11.4 (`/web` UI surface for the 429 path — quota status banner, retry countdown) remains. Three complete EPICs shipped on trunk. EPIC 3 landed all three planned ISSUEs: **ISSUE 8 (DescriptorCatalog hot-reload, 5 TASKs, v0.4.1–v0.4.5)** — ArcSwap substrate → admin HTTP reload → `CatalogSnapshot { catalog, validators }` bundling → file-based TOML source via `[web] catalog_path` → POSIX `SIGHUP` reloader sharing `perform_catalog_reload()` helper with HTTP path. **ISSUE 9 (real bundle manifests, 3 TASKs, v0.4.6–v0.4.8)** — `BundleMetadata { id, version }` + optional `[bundle]` TOML table → first-party `bundles/gadgetron-core/bundle.toml` with drift-test invariant vs `seed_p2b()` → multi-bundle aggregation via `[web] bundles_dir` + `DescriptorCatalog::from_bundle_dir()` with hard duplicate-id collisions + `bundles: Vec<BundleMetadata>` response field + 3-level precedence → bundle-driven harness default with `source` enum `{"seed_p2b", "config_file", "bundles_dir"}`. **ISSUE 10 (bundle marketplace, 4 TASKs, v0.4.9–v0.4.12)** — `GET /admin/bundles` read-only discovery → `POST /admin/bundles` + `DELETE /admin/bundles/{bundle_id}` install/uninstall with `validate_bundle_id()` path-traversal regex + no-silent-overwrite collision policy → per-bundle `required_scope` inheritance (narrower-wins, zero-overhead default) → Ed25519 signed manifests with `[web.bundle_signing]` trust-anchor config + signer-leak-prevention-via-error-envelope-unification. Harness 91 PASS, 0 FAIL. **EPIC 4 (Multi-tenant business ops / XaaS — integer-cent billing, HuggingFace catalog, tenant self-service, quotas + SLA enforcement) is now ACTIVE (promoted 2026-04-20)** toward `v1.0.0` (first production release); ISSUE 11 (quotas + rate limits) in-flight with TASK 11.1 shipped, TASK 11.2 (token-bucket enforcer replacing `InMemoryQuotaEnforcer`) pending. `0.5.x` patch bumps accumulate per-ISSUE until EPIC 4 closes with the minor bump to `v1.0.0`. EPIC 5 (Cluster platform, PLANNED post-1.0) follows with `v2.0.0`. Three complete EPICs + twelve TASKs shipped across EPIC 3. `POST /v1/chat/completions` + Penny runtime + embedded Web UI + browser-driven wiki CRUD (`/web/wiki`) + direct-action audit + approval flow (ISSUE 3, PR #188) + operator observability (`/usage/summary` + `/events/ws` WebSocket + `/web/dashboard`, ISSUE 4, PR #194) + Penny tool-call audit persistence + `/audit/tool-events` query endpoint + `ActivityEvent::ToolCallCompleted` fan-out (ISSUE 5, PR #199) + Penny-attributed activity feed (`GadgetAuditEventWriter.with_coordinator` + `CapturedActivityEvent { origin: Penny, kind: GadgetToolCall }` fan-out to `/workbench/activity`, ISSUE 6, PR #201) + first-class MCP server (`GET /v1/tools` discovery + `POST /v1/tools/{name}/invoke` dispatch + cross-session audit landing `tool_audit_events` rows with authenticated-actor `owner_id`/`tenant_id`, ISSUE 7 TASKs 7.1/7.2/7.3, PRs #204/#205/#207) all ship on trunk. Next milestones: **EPIC 3 (Plugin platform) → tag `v0.5.0`**, **EPIC 4 (Multi-tenant XaaS) → tag `v1.0.0`**, **EPIC 5 (Cluster platform) → tag `v2.0.0`**. See [`docs/adr/ADR-P2A-06`](docs/adr/ADR-P2A-06-approval-flow-deferred-to-p2b.md) for the Penny-side approval items that remain future work. Canonical plan: [`docs/ROADMAP.md`](docs/ROADMAP.md).
...
### Next
Both EPIC 1 (Workbench MVP) and EPIC 2 (Agent autonomy) are closed. Next active EPIC candidates per `docs/ROADMAP.md`:
- **EPIC 3 — Plugin platform** (ISSUEs 8/9/10, close → `v0.5.0`): `DescriptorCatalog` hot-reload, real bundle manifests, bundle marketplace.
- **EPIC 4 — Multi-tenant / XaaS** (ISSUEs 11/12/13/14, close → `v1.0.0`): per-tenant quotas/rate-limits, integer-cent billing, HuggingFace catalog, tenant self-service.
- **EPIC 5 — Cluster platform** (post-1.0, close → `v2.0.0`): multi-node VRAM scheduling, NUMA, MIG, K8s operator.
