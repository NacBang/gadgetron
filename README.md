...
**Version**: `0.4.9` — **EPIC 1 (Workbench MVP) CLOSED `v0.3.0` (PR #208); EPIC 2 (Agent autonomy) CLOSED `v0.4.0` (PR #209); EPIC 3 (Plugin platform) ACTIVE — ISSUE 8 + ISSUE 9 BOTH functionally complete; ISSUE 10 (bundle marketplace) STARTED**. ISSUE 8 five TASKs: 8.1 `Arc<ArcSwap<_>>` substrate (PR #211), 8.2 `POST /admin/reload-catalog` Management-scoped endpoint (PR #213), 8.3 `CatalogSnapshot { catalog, validators }` atomic bundling (PR #214), 8.4 file-based TOML source via `[web] catalog_path` (PR #216), 8.5 POSIX `SIGHUP` reloader (PR #217). ISSUE 9 three TASKs: 9.1 bundle metadata + first-party `bundles/gadgetron-core/bundle.toml` (PR #219), 9.2 multi-bundle aggregation via `[web] bundles_dir` + `from_bundle_dir()` with hard duplicate-id collision + `bundles: Vec<BundleMetadata>` response + 3-level precedence (PR #220), 9.3 bundle-driven harness default + `source` enum expands to `{seed_p2b, config_file, bundles_dir}` (PR #222). **ISSUE 10 TASK 10.1 shipped (PR #223 — `GET /api/v1/web/workbench/admin/bundles` Management-scoped bundle-discovery endpoint; returns `{bundles_dir, count, bundles: [{bundle?, source_path, action_count, view_count}, ...]}` scanning each `<bundles_dir>/<subdir>/bundle.toml`; read-only, does NOT ArcSwap the live catalog; deterministic subdir-sorted ordering matches `from_bundle_dir`; 503 `Config` when `bundles_dir` isn't wired; harness PASS 84 → 86 via new Gates 7q.4 shape + gadgetron-core enumeration + 7q.5 RBAC 403)**. TASK 10.2 (install/uninstall runtime), 10.3 (per-bundle scope isolation), 10.4 (Ed25519-signed manifests) pending before EPIC 3 close → tag `v0.5.0`. Two complete EPICs + nine TASKs shipped. `POST /v1/chat/completions` + Penny runtime + embedded Web UI + browser-driven wiki CRUD (`/web/wiki`) + direct-action audit + approval flow (ISSUE 3, PR #188) + operator observability (`/usage/summary` + `/events/ws` WebSocket + `/web/dashboard`, ISSUE 4, PR #194) + Penny tool-call audit persistence + `/audit/tool-events` query endpoint + `ActivityEvent::ToolCallCompleted` fan-out (ISSUE 5, PR #199) + Penny-attributed activity feed (`GadgetAuditEventWriter.with_coordinator` + `CapturedActivityEvent { origin: Penny, kind: GadgetToolCall }` fan-out to `/workbench/activity`, ISSUE 6, PR #201) + first-class MCP server (`GET /v1/tools` discovery + `POST /v1/tools/{name}/invoke` dispatch + cross-session audit landing `tool_audit_events` rows with authenticated-actor `owner_id`/`tenant_id`, ISSUE 7 TASKs 7.1/7.2/7.3, PRs #204/#205/#207) all ship on trunk. Next milestones: **EPIC 3 (Plugin platform) → tag `v0.5.0`**, **EPIC 4 (Multi-tenant XaaS) → tag `v1.0.0`**, **EPIC 5 (Cluster platform) → tag `v2.0.0`**. See [`docs/adr/ADR-P2A-06`](docs/adr/ADR-P2A-06-approval-flow-deferred-to-p2b.md) for the Penny-side approval items that remain future work. Canonical plan: [`docs/ROADMAP.md`](docs/ROADMAP.md).
