...
**Version**: `0.4.8` — **EPIC 1 (Workbench MVP) CLOSED `v0.3.0` (PR #208); EPIC 2 (Agent autonomy) CLOSED `v0.4.0` (PR #209); EPIC 3 (Plugin platform) ACTIVE — ISSUE 8 (DescriptorCatalog hot-reload) + ISSUE 9 (real bundle manifests) BOTH functionally complete**. ISSUE 8 five TASKs: 8.1 `Arc<ArcSwap<_>>` substrate (PR #211), 8.2 `POST /admin/reload-catalog` Management-scoped endpoint (PR #213), 8.3 `CatalogSnapshot { catalog, validators }` atomic bundling (PR #214), 8.4 file-based TOML source via `[web] catalog_path` (PR #216), 8.5 POSIX `SIGHUP` reloader (PR #217 — fs-watcher deferred to TASK 8.6). ISSUE 9 three TASKs: 9.1 bundle metadata + first-party `bundles/gadgetron-core/bundle.toml` (PR #219), 9.2 multi-bundle aggregation via `[web] bundles_dir` + `DescriptorCatalog::from_bundle_dir()` with hard duplicate-id collision errors + `bundles: Vec<BundleMetadata>` response field + 3-level precedence (PR #220), 9.3 bundle-driven harness default (PR #222 — E2E harness now boots against in-tree `bundles/` via the from_bundle_dir path, Gate 7q.1 asserts `source == "bundles_dir"` end-to-end, new Gate 7q.3 pins `.bundles[0].id == "gadgetron-core"`; seed_p2b stays alive as a unit-test fixture + drift-guard reference but production code no longer defaults to it through the harness config). **ISSUE 10 (bundle marketplace — discovery + install/uninstall API + per-bundle scope isolation + signed manifests) is the only work remaining before EPIC 3 close → tag `v0.5.0`**. Two complete EPICs + eight TASKs shipped. `POST /v1/chat/completions` + Penny runtime + embedded Web UI + browser-driven wiki CRUD (`/web/wiki`) + direct-action audit + approval flow (ISSUE 3, PR #188) + operator observability (`/usage/summary` + `/events/ws` WebSocket + `/web/dashboard`, ISSUE 4, PR #194) + Penny tool-call audit persistence + `/audit/tool-events` query endpoint + `ActivityEvent::ToolCallCompleted` fan-out (ISSUE 5, PR #199) + Penny-attributed activity feed (`GadgetAuditEventWriter.with_coordinator` + `CapturedActivityEvent { origin: Penny, kind: GadgetToolCall }` fan-out to `/workbench/activity`, ISSUE 6, PR #201) + first-class MCP server (`GET /v1/tools` discovery + `POST /v1/tools/{name}/invoke` dispatch + cross-session audit landing `tool_audit_events` rows with authenticated-actor `owner_id`/`tenant_id`, ISSUE 7 TASKs 7.1/7.2/7.3, PRs #204/#205/#207) all ship on trunk. Next milestones: **EPIC 3 (Plugin platform) → tag `v0.5.0`**, **EPIC 4 (Multi-tenant XaaS) → tag `v1.0.0`**, **EPIC 5 (Cluster platform) → tag `v2.0.0`**. See [`docs/adr/ADR-P2A-06`](docs/adr/ADR-P2A-06-approval-flow-deferred-to-p2b.md) for the Penny-side approval items that remain future work. Canonical plan: [`docs/ROADMAP.md`](docs/ROADMAP.md).
