...
**Version**: `0.5.15` — **EPIC 1 (Workbench MVP) CLOSED `v0.3.0` (PR #208); EPIC 2 (Agent autonomy) CLOSED `v0.4.0` (PR #209); EPIC 3 (Plugin platform) CLOSED `v0.5.0` (PR #228); EPIC 4 (Multi-tenant business ops / XaaS) ACTIVE** (promoted 2026-04-20) — **ISSUE 11 + ISSUE 12 (telemetry scope) + ISSUE 14 (tenant self-service, PR #246 / 0.5.7) + ISSUE 15 (cookie-session login API, PR #248 / 0.5.8) + ISSUE 16 (unified Bearer/cookie auth middleware, PR #259 / 0.5.9) + ISSUE 17 (ValidatedKey.user_id, PR #260 / 0.5.10) + ISSUE 19 (AuditEntry actor fields, PR #262 / 0.5.11) + ISSUE 20 (TenantContext → AuditEntry plumbing, PR #263 / 0.5.12) + ISSUE 21 (pg audit_log consumer, PR #267 / 0.5.13) + ISSUE 22 (admin `GET /admin/audit/log` query endpoint, PR #269 / 0.5.14) + ISSUE 23 (billing_events.actor_user_id + tenant-first composite index, PR #271 / 0.5.15) all complete** (TASKs 12.1 + 12.2 shipped; TASKs 12.3 / 12.4 / 12.5 DEFERRED per 2026-04-20 commercialization-layer direction): TASK 11.1 structured 429 + Retry-After (PR #230 / 0.5.1), TASK 11.2 token-bucket rate limiter via `[quota_rate_limit]` (PR #231 / 0.5.2), TASK 11.3 Postgres-backed daily + monthly spend tracking via `PgQuotaEnforcer` + CASE rollover (PR #232 / 0.5.3), **TASK 11.4 `GET /api/v1/web/workbench/quota/status` tenant introspection endpoint (PR #234 / 0.5.4)** — OpenAiCompat scope so users can check their own usage without Management rights; response shape `{usage_day, daily: {used_cents, limit_cents, remaining_cents}, monthly: {used_cents, limit_cents, remaining_cents}}` with the CASE-rollover SQL inline so boundary crossings are already reflected; when `quota_configs` has no row for the tenant (bootstrap gap), handler falls back to schema defaults (1_000_000 cents daily, 10_000_000 cents monthly, usage_day = today) so the UI renders "fresh tenant, full quota" instead of a 400 while tenant provisioning catches up. New harness gate 7k.5 pins the response shape + fallback path. Full EPIC 4 pipeline now: rate-limit check → pg cost check → dispatch → pg record_post → tenant introspection via /quota/status → rejections surface as structured 429 + Retry-After. **ISSUE 12 (integer-cent billing — closed at telemetry scope via PR #241 / 0.5.6)**: TASK 12.1 (PR #236 / 0.5.5) shipped the append-only `billing_events` ledger table (integer cents, `event_kind ∈ {chat, tool, action}` CHECK constraint, FK-less `source_event_id` per Stripe-style writer-independence) + `PgQuotaEnforcer::record_post` chat INSERT + `GET /api/v1/web/workbench/admin/billing/events` Management-scoped ledger query (newest-first, `limit.clamp(1, 500)`, optional `since` lower bound; handler pins tenant boundary so cross-tenant reads are impossible regardless of query params). TASK 12.2 (PR #241 / 0.5.6) widened emission to `/v1/tools/{name}/invoke` success + workbench direct-action + approved-action terminals — each emits one `billing_events` row (kind=tool / kind=action; action rows carry `source_event_id = audit_event_id` for clean audit↔ledger joins; harness gates 7i.5 + 7h.1c). **Scope direction (2026-04-20)**: TASKs 12.3 (invoice materialization) / 12.4 (counter/ledger reconciliation) / 12.5 (Stripe ingest) + ISSUE 13 (HF catalog) + ISSUE 14 (tenant self-service) are the **commercialization layer** — DEFERRED, not gate-blockers for `v1.0.0`. EPIC 4 "close for 1.0" = ISSUE 11 + core product surfaces (knowledge + Penny + bundle/plug + multi-user Google OAuth + observability) meeting production bar; 12.3 / 12.4 / 12.5 / 13 / 14 can ship post-1.0 as patch / minor bumps once market pull justifies. Three complete EPICs shipped on trunk. EPIC 3 landed all three planned ISSUEs: **ISSUE 8 (DescriptorCatalog hot-reload, 5 TASKs, v0.4.1–v0.4.5)** — ArcSwap substrate → admin HTTP reload → `CatalogSnapshot { catalog, validators }` bundling → file-based TOML source via `[web] catalog_path` → POSIX `SIGHUP` reloader sharing `perform_catalog_reload()` helper with HTTP path. **ISSUE 9 (real bundle manifests, 3 TASKs, v0.4.6–v0.4.8)** — `BundleMetadata { id, version }` + optional `[bundle]` TOML table → first-party `bundles/gadgetron-core/bundle.toml` with drift-test invariant vs `seed_p2b()` → multi-bundle aggregation via `[web] bundles_dir` + `DescriptorCatalog::from_bundle_dir()` with hard duplicate-id collisions + `bundles: Vec<BundleMetadata>` response field + 3-level precedence → bundle-driven harness default with `source` enum `{"seed_p2b", "config_file", "bundles_dir"}`. **ISSUE 10 (bundle marketplace, 4 TASKs, v0.4.9–v0.4.12)** — `GET /admin/bundles` read-only discovery → `POST /admin/bundles` + `DELETE /admin/bundles/{bundle_id}` install/uninstall with `validate_bundle_id()` path-traversal regex + no-silent-overwrite collision policy → per-bundle `required_scope` inheritance (narrower-wins, zero-overhead default) → Ed25519 signed manifests with `[web.bundle_signing]` trust-anchor config + signer-leak-prevention-via-error-envelope-unification. Harness 91 PASS, 0 FAIL. EPIC 4 scope spans integer-cent billing, HuggingFace catalog, tenant self-service, and quotas + SLA enforcement toward `v1.0.0` (first production release); `0.5.x` patch bumps accumulate per-ISSUE until EPIC 4 closes with the minor bump to `v1.0.0`. EPIC 5 (Cluster platform, PLANNED post-1.0) follows with `v2.0.0`. Three complete EPICs + twelve TASKs shipped across EPIC 3. `POST /v1/chat/completions` + Penny runtime + embedded Web UI + browser-driven wiki CRUD (`/web/wiki`) + direct-action audit + approval flow (ISSUE 3, PR #188) + operator observability (`/usage/summary` + `/events/ws` WebSocket + `/web/dashboard`, ISSUE 4, PR #194) + Penny tool-call audit persistence + `/audit/tool-events` query endpoint + `ActivityEvent::ToolCallCompleted` fan-out (ISSUE 5, PR #199) + Penny-attributed activity feed (`GadgetAuditEventWriter.with_coordinator` + `CapturedActivityEvent { origin: Penny, kind: GadgetToolCall }` fan-out to `/workbench/activity`, ISSUE 6, PR #201) + first-class MCP server (`GET /v1/tools` discovery + `POST /v1/tools/{name}/invoke` dispatch + cross-session audit landing `tool_audit_events` rows with authenticated-actor `owner_id`/`tenant_id`, ISSUE 7 TASKs 7.1/7.2/7.3, PRs #204/#205/#207) all ship on trunk. Next milestones: **EPIC 4 (Multi-tenant XaaS) → tag `v1.0.0`**, **EPIC 5 (Cluster platform) → tag `v2.0.0`**. See [`docs/adr/ADR-P2A-06`](docs/adr/ADR-P2A-06-approval-flow-deferred-to-p2b.md) for the Penny-side approval items that remain future work. Canonical plan: [`docs/ROADMAP.md`](docs/ROADMAP.md).
...
- **EPIC 4 — Multi-tenant / XaaS** (ISSUEs 11/12/14/15 ✅; 13 + 16 remain; close → `v1.0.0`, **ACTIVE** 2026-04-20): per-tenant quotas, billing telemetry, tenant self-service, cookie-session login API. **ISSUE 11 ✅** (0.5.1–0.5.4 / PRs #230/#231/#232/#234), **ISSUE 12 ✅** at telemetry scope (0.5.5/0.5.6 / PRs #236/#241 — invoicing 12.3-12.5 DEFERRED), **ISSUE 14 ✅** (PR #246 / 0.5.7 — users + teams + key rotation + CLI), **ISSUE 15 ✅** (PR #248 / 0.5.8 — `/auth/{login,logout,whoami}` cookie sessions). Remaining: ISSUE 13 HF catalog discovery/pinning, ISSUE 16 web UI login form + unified Bearer/cookie middleware.
