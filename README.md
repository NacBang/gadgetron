...
**Version**: `0.4.11` — **EPIC 1 (Workbench MVP) CLOSED `v0.3.0` (PR #208); EPIC 2 (Agent autonomy) CLOSED `v0.4.0` (PR #209); EPIC 3 (Plugin platform) ACTIVE — ISSUE 8 + ISSUE 9 BOTH functionally complete; ISSUE 10 (bundle marketplace) in-flight with TASKs 10.1 + 10.2 + 10.3 shipped**. TASK 10.1 (discovery endpoint, PR #223 / 0.4.9) + TASK 10.2 (install + uninstall runtime, PR #224 / 0.4.10) already landed. **TASK 10.3 shipped (PR #226 / 0.4.11 — per-bundle scope isolation)**: `BundleMetadata.required_scope: Option<Scope>` new serde-optional TOML field at the `[bundle]` level. `DescriptorCatalog::from_toml_file` post-parse pass walks every view + action; descriptors without their own `required_scope` inherit `bundle.required_scope` as a floor — actors without that scope see NONE of the bundle's descriptors. Descriptors with explicit scopes keep theirs (narrower wins). Zero-overhead for bundles that don't declare a scope (default `None`, no inheritance pass). `from_bundle_dir` inherits the same behavior via its per-manifest `from_toml_file` call. Design motivation: a bundle with N actions for operators only used to require `required_scope = "Management"` on every descriptor — now one declaration at the bundle level covers the whole set AND travels with the manifest file, so operators installing via TASK 10.2 automatically inherit the scope floor. TASK 10.4 (Ed25519-signed manifests on install) pending before EPIC 3 close → tag `v0.5.0`. Two complete EPICs + eleven TASKs shipped. `POST /v1/chat/completions` + Penny runtime + embedded Web UI + browser-driven wiki CRUD (`/web/wiki`) + direct-action audit + approval flow (ISSUE 3, PR #188) + operator observability (`/usage/summary` + `/events/ws` WebSocket + `/web/dashboard`, ISSUE 4, PR #194) + Penny tool-call audit persistence + `/audit/tool-events` query endpoint + `ActivityEvent::ToolCallCompleted` fan-out (ISSUE 5, PR #199) + Penny-attributed activity feed (`GadgetAuditEventWriter.with_coordinator` + `CapturedActivityEvent { origin: Penny, kind: GadgetToolCall }` fan-out to `/workbench/activity`, ISSUE 6, PR #201) + first-class MCP server (`GET /v1/tools` discovery + `POST /v1/tools/{name}/invoke` dispatch + cross-session audit landing `tool_audit_events` rows with authenticated-actor `owner_id`/`tenant_id`, ISSUE 7 TASKs 7.1/7.2/7.3, PRs #204/#205/#207) all ship on trunk. Next milestones: **EPIC 3 (Plugin platform) → tag `v0.5.0`**, **EPIC 4 (Multi-tenant XaaS) → tag `v1.0.0`**, **EPIC 5 (Cluster platform) → tag `v2.0.0`**. See [`docs/adr/ADR-P2A-06`](docs/adr/ADR-P2A-06-approval-flow-deferred-to-p2b.md) for the Penny-side approval items that remain future work. Canonical plan: [`docs/ROADMAP.md`](docs/ROADMAP.md).
