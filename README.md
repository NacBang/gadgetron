...
**Version**: `0.4.12` — **EPIC 1 (Workbench MVP) CLOSED `v0.3.0` (PR #208); EPIC 2 (Agent autonomy) CLOSED `v0.4.0` (PR #209); EPIC 3 (Plugin platform) ACTIVE — ISSUE 8 + ISSUE 9 + ISSUE 10 ALL functionally complete; EPIC 3 close → `v0.5.0` is the natural next step**. ISSUE 10 four TASKs: 10.1 bundle-discovery (PR #223 / 0.4.9), 10.2 install + uninstall (PR #224 / 0.4.10), 10.3 per-bundle scope isolation via `BundleMetadata.required_scope` inheritance (PR #226 / 0.4.11). **TASK 10.4 shipped (PR #227 / 0.4.12 — Ed25519 signed manifests)**: closes ISSUE 10 and covers the supply-chain surface opened by TASK 10.2. `[web.bundle_signing] { public_keys_hex: Vec<String>, require_signature: bool }` config block declares trusted publisher pubkeys; `InstallBundleRequest` widens with `signature_hex: Option<String>`; `verify_bundle_signature` runs BEFORE `toml::from_str` (so a signed-malformed file takes the same error path as an unsigned-malformed one — no signer leak via error text). Policy matrix: unsigned-allowed → accept, unsigned-required → reject, sig-valid → accept, sig-tampered → reject, sig-unknown-key → reject, signature-without-trust-anchors → reject loud (better to fail than silently accept). Default config preserves TASK 10.2 unsigned behavior so deployments that haven't rotated to signed bundles keep working. 6 unit tests pin each policy branch. Workspace picks up `ed25519-dalek = "2"` + `hex = "0.4"`. Two complete EPICs + twelve TASKs shipped; EPIC 3 functionally complete, ISSUE 8 / 9 / 10 all landed. `POST /v1/chat/completions` + Penny runtime + embedded Web UI + browser-driven wiki CRUD (`/web/wiki`) + direct-action audit + approval flow (ISSUE 3, PR #188) + operator observability (`/usage/summary` + `/events/ws` WebSocket + `/web/dashboard`, ISSUE 4, PR #194) + Penny tool-call audit persistence + `/audit/tool-events` query endpoint + `ActivityEvent::ToolCallCompleted` fan-out (ISSUE 5, PR #199) + Penny-attributed activity feed (`GadgetAuditEventWriter.with_coordinator` + `CapturedActivityEvent { origin: Penny, kind: GadgetToolCall }` fan-out to `/workbench/activity`, ISSUE 6, PR #201) + first-class MCP server (`GET /v1/tools` discovery + `POST /v1/tools/{name}/invoke` dispatch + cross-session audit landing `tool_audit_events` rows with authenticated-actor `owner_id`/`tenant_id`, ISSUE 7 TASKs 7.1/7.2/7.3, PRs #204/#205/#207) all ship on trunk. Next milestones: **EPIC 3 (Plugin platform) → tag `v0.5.0`**, **EPIC 4 (Multi-tenant XaaS) → tag `v1.0.0`**, **EPIC 5 (Cluster platform) → tag `v2.0.0`**. See [`docs/adr/ADR-P2A-06`](docs/adr/ADR-P2A-06-approval-flow-deferred-to-p2b.md) for the Penny-side approval items that remain future work. Canonical plan: [`docs/ROADMAP.md`](docs/ROADMAP.md).
