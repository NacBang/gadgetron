...
**Version**: `0.4.4` — **EPIC 1 (Workbench MVP) CLOSED `v0.3.0` (PR #208); EPIC 2 (Agent autonomy) CLOSED `v0.4.0` (PR #209); EPIC 3 (Plugin platform) ACTIVE — ISSUE 8 TASK 8.1 substrate (PR #211) + TASK 8.2 `POST /api/v1/web/workbench/admin/reload-catalog` Management-scoped hot-reload endpoint (PR #213) + TASK 8.3 `CatalogSnapshot { catalog, validators }` bundling via `Arc<ArcSwap<CatalogSnapshot>>` (PR #214, closed the TASK 8.2 validator-rebuild gap) + TASK 8.4 file-based catalog source (PR #216) — operators point `[web] catalog_path` in `gadgetron.toml` at a TOML file; each `POST /admin/reload-catalog` reparses the file and atomically swaps the snapshot, with parse failures surfacing as 500 without replacing the running snapshot; response widens with `source: "config_file"` + `source_path: "/path/to/file"` when the path is configured**. Two complete EPICs + four TASKs shipped. `POST /v1/chat/completions` + Penny runtime + embedded Web UI + browser-driven wiki CRUD (`/web/wiki`) + direct-action audit + approval flow (ISSUE 3, PR #188) + operator observability (`/usage/summary` + `/events/ws` WebSocket + `/web/dashboard`, ISSUE 4, PR #194) + Penny tool-call audit persistence + `/audit/tool-events` query endpoint + `ActivityEvent::ToolCallCompleted` fan-out (ISSUE 5, PR #199) + Penny-attributed activity feed (`GadgetAuditEventWriter.with_coordinator` + `CapturedActivityEvent { origin: Penny, kind: GadgetToolCall }` fan-out to `/workbench/activity`, ISSUE 6, PR #201) + first-class MCP server (`GET /v1/tools` discovery + `POST /v1/tools/{name}/invoke` dispatch + cross-session audit landing `tool_audit_events` rows with authenticated-actor `owner_id`/`tenant_id`, ISSUE 7 TASKs 7.1/7.2/7.3, PRs #204/#205/#207) all ship on trunk. Next milestones: **EPIC 3 (Plugin platform) → tag `v0.5.0`**, **EPIC 4 (Multi-tenant XaaS) → tag `v1.0.0`**, **EPIC 5 (Cluster platform) → tag `v2.0.0`**. See [`docs/adr/ADR-P2A-06`](docs/adr/ADR-P2A-06-approval-flow-deferred-to-p2b.md) for the Penny-side approval items that remain future work. Canonical plan: [`docs/ROADMAP.md`](docs/ROADMAP.md).
