...
**Version**: `0.4.7` — **EPIC 1 (Workbench MVP) CLOSED `v0.3.0` (PR #208); EPIC 2 (Agent autonomy) CLOSED `v0.4.0` (PR #209); EPIC 3 (Plugin platform) ACTIVE — ISSUE 8 (DescriptorCatalog hot-reload) functionally complete with all five shipped TASKs: 8.1 `Arc<ArcSwap<_>>` substrate (PR #211), 8.2 `POST /admin/reload-catalog` Management-scoped endpoint (PR #213), 8.3 `CatalogSnapshot { catalog, validators }` atomic bundling (PR #214 — closed the TASK 8.2 validator-rebuild gap), 8.4 file-based TOML catalog source via `[web] catalog_path` + `DescriptorCatalog::from_toml_file()` with parse-failure guarantee (PR #216), 8.5 POSIX `SIGHUP` reloader that reuses the same `perform_catalog_reload()` code path as the HTTP endpoint — operator workflow: edit `catalog_path` + `kill -HUP <pid>`, Unix-only with fs-watcher deferred to future TASK 8.6 if demand emerges (PR #217). **ISSUE 9 (real bundle manifests) STARTED** — TASK 9.1 shipped (PR #219 — `BundleMetadata { id, version }` + optional `[bundle]` TOML table; reload response widens with `bundle: Option<BundleMetadata>`; first-party bundle file at `bundles/gadgetron-core/bundle.toml` mirroring `seed_p2b()` exactly, guarded by drift-test). TASK 9.2 shipped (PR #220 — multi-bundle aggregation; new `[web] bundles_dir` config key + `DescriptorCatalog::from_bundle_dir()` scans every `<dir>/<name>/bundle.toml` and merges into one catalog with deterministic alphabetical order; duplicate action/view ids across bundles surface as a hard `Config` error naming both bundles — no silent winners; `allow_direct_actions` OR-folded across bundles; reload response gains `bundles: Vec<BundleMetadata>` listing every contributing bundle; precedence: `bundles_dir` > `catalog_path` > `seed_p2b` fallback). TASK 9.3 (retire `seed_p2b()` in favor of shipping `bundles/gadgetron-core/` as default source) + ISSUE 10 (bundle marketplace) pending before EPIC 3 close → tag `v0.5.0`**. Two complete EPICs + seven TASKs shipped. `POST /v1/chat/completions` + Penny runtime + embedded Web UI + browser-driven wiki CRUD (`/web/wiki`) + direct-action audit + approval flow (ISSUE 3, PR #188) + operator observability (`/usage/summary` + `/events/ws` WebSocket + `/web/dashboard`, ISSUE 4, PR #194) + Penny tool-call audit persistence + `/audit/tool-events` query endpoint + `ActivityEvent::ToolCallCompleted` fan-out (ISSUE 5, PR #199) + Penny-attributed activity feed (`GadgetAuditEventWriter.with_coordinator` + `CapturedActivityEvent { origin: Penny, kind: GadgetToolCall }` fan-out to `/workbench/activity`, ISSUE 6, PR #201) + first-class MCP server (`GET /v1/tools` discovery + `POST /v1/tools/{name}/invoke` dispatch + cross-session audit landing `tool_audit_events` rows with authenticated-actor `owner_id`/`tenant_id`, ISSUE 7 TASKs 7.1/7.2/7.3, PRs #204/#205/#207) all ship on trunk. Next milestones: **EPIC 3 (Plugin platform) → tag `v0.5.0`**, **EPIC 4 (Multi-tenant XaaS) → tag `v1.0.0`**, **EPIC 5 (Cluster platform) → tag `v2.0.0`**. See [`docs/adr/ADR-P2A-06`](docs/adr/ADR-P2A-06-approval-flow-deferred-to-p2b.md) for the Penny-side approval items that remain future work. Canonical plan: [`docs/ROADMAP.md`](docs/ROADMAP.md).
