...
**Version**: `0.4.10` — **EPIC 1 (Workbench MVP) CLOSED `v0.3.0` (PR #208); EPIC 2 (Agent autonomy) CLOSED `v0.4.0` (PR #209); EPIC 3 (Plugin platform) ACTIVE — ISSUE 8 + ISSUE 9 BOTH functionally complete; ISSUE 10 (bundle marketplace) in-flight**. ISSUE 8 five TASKs + ISSUE 9 three TASKs + ISSUE 10 TASK 10.1 (bundle-discovery `GET /admin/bundles`, PR #223) already landed. **ISSUE 10 TASK 10.2 shipped (PR #224 / 0.4.10 — install + uninstall runtime)**: `POST /api/v1/web/workbench/admin/bundles` accepts `{bundle_toml: "<text>"}`, parses + validates (manifest must declare `[bundle]` with id matching `[a-zA-Z0-9_-]{1,64}` for path-traversal safety), writes manifest verbatim to `{bundles_dir}/{id}/bundle.toml`, returns `{installed, bundle_id, manifest_path, reload_hint}`. `DELETE /api/v1/web/workbench/admin/bundles/{bundle_id}` removes the directory via `std::fs::remove_dir_all` with the same id regex guard. Composable with reload: install writes to disk but does NOT swap the live catalog — operator must trigger `POST /admin/reload-catalog` or SIGHUP to activate. Collision policy: installing over an existing id is rejected with 4xx `Config` (operator uninstalls first) — matches `from_bundle_dir` duplicate-id hard-error semantics. Both Management-scoped. Harness PASS 86 → 91 via new Gates 7q.6 (install → discovery lists it), 7q.7 (`id = "../etc/passwd"` → 4xx rejection), 7q.8 (uninstall → discovery no longer lists it). TASK 10.3 (per-bundle scope isolation — bundles declare required scopes, aggregator filters visible actions per actor) + TASK 10.4 (Ed25519-signed manifests to cover the supply-chain surface opened by letting HTTP write arbitrary TOML) pending before EPIC 3 close → tag `v0.5.0`. Two complete EPICs + ten TASKs shipped. `POST /v1/chat/completions` + Penny runtime + embedded Web UI + browser-driven wiki CRUD (`/web/wiki`) + direct-action audit + approval flow (ISSUE 3, PR #188) + operator observability (`/usage/summary` + `/events/ws` WebSocket + `/web/dashboard`, ISSUE 4, PR #194) + Penny tool-call audit persistence + `/audit/tool-events` query endpoint + `ActivityEvent::ToolCallCompleted` fan-out (ISSUE 5, PR #199) + Penny-attributed activity feed (`GadgetAuditEventWriter.with_coordinator` + `CapturedActivityEvent { origin: Penny, kind: GadgetToolCall }` fan-out to `/workbench/activity`, ISSUE 6, PR #201) + first-class MCP server (`GET /v1/tools` discovery + `POST /v1/tools/{name}/invoke` dispatch + cross-session audit landing `tool_audit_events` rows with authenticated-actor `owner_id`/`tenant_id`, ISSUE 7 TASKs 7.1/7.2/7.3, PRs #204/#205/#207) all ship on trunk. Next milestones: **EPIC 3 (Plugin platform) → tag `v0.5.0`**, **EPIC 4 (Multi-tenant XaaS) → tag `v1.0.0`**, **EPIC 5 (Cluster platform) → tag `v2.0.0`**. See [`docs/adr/ADR-P2A-06`](docs/adr/ADR-P2A-06-approval-flow-deferred-to-p2b.md) for the Penny-side approval items that remain future work. Canonical plan: [`docs/ROADMAP.md`](docs/ROADMAP.md).
