...
- **ISSUE 25 ⏳ `AuthenticatedContext` rename + audit-log contamination fix + billing-insert SLO** (planned, post-v0.5.16; not a `v1.0.0` gate)
  - Goal: close the type-confusion landmine that ISSUE 24 left in place — `AuthenticatedContext.user_id` is still an api_key_id typed as a user UUID, waiting for a misread at ~14 reader call sites.
  - Sketch: rename `AuthenticatedContext.user_id` → `api_key_id` (14 reader call sites); rename `real_user_id` → `user_id` so the canonical field has the canonical name; fix `action_service.rs` audit sink emitting legacy `actor.user_id` (api_key_id) into `ActionAuditEvent.actor_user_id` — same type-confusion bug as billing had pre-ISSUE-24, scoped to audit_log here; add `billing_insert_failures_total{reason}` counter + SLO alert (security-review Q3 "fix in this PR or next" deferred from ISSUE 24).
  - Harness addition: Gate 7v.N asserting `audit_log.actor_user_id` matches `billing_events.actor_user_id` for the same request_id — would catch the contamination drift if it recurs.
  - Not a `v1.0.0` gate. Type-confusion-hardening follow-up to ISSUE 24.
