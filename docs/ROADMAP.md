...
- **ISSUE 27 ⏳ `AuthenticatedContext.real_user_id` → `user_id` rename** (planned, post-v0.5.17; not a `v1.0.0` gate)
  ...
  - Not a `v1.0.0` gate.
- **ISSUE 28 ⏳ `/web` pre-auth landing page polish + status-bar format** (3 of 4 TASKs remaining, post-v0.5.19; not a `v1.0.0` gate)
  - **Operator bug report (2026-04-20)**: visiting `http://10.100.1.5:18080/web` as an unauthenticated browser renders a broken pre-auth page. Screenshot attached to the PR that opens this ISSUE. Observable problems:
    - **TASK 28.1 ✅ Web asset bundle not loading** (closed via PR #304, v0.5.19). The sidebar icons rendered as empty button rectangles and `Chat` / `Wiki` / `Dashboard` / `Knowledge` / `Bundles` nav items as unstyled underlined blue text because (a) CSP `upgrade-insecure-requests` directive forced browsers to auto-upgrade every `/web/_next/static/...` subresource request from HTTP to HTTPS on a plain-HTTP `0.0.0.0:18080` bind (100 % load failure), and (b) stale `web/dist/index.html` referenced a CSS hash that had been renamed in the `web/out/` export, so even without the CSP issue a subset of deployments 404'd on the stylesheet. Four-layer fix: (1) `WebConfig.upgrade_insecure_requests: bool` config knob (default `false`, opt-in via `[web].upgrade_insecure_requests = true` for HTTPS-only deploys) so `gadgetron-gateway/src/web_csp.rs::build_csp` emits the directive only when explicitly asked; (2) compile-time `verify_asset_consistency()` in `crates/gadgetron-web/build_logic.rs` scans every `*.html` under `web/dist/`, extracts `/web/_next/static/...` refs, and fails `cargo build` with a fix recipe if any reference has no matching file; (3) harness Gate 11b.5 (asserts default CSP omits `upgrade-insecure-requests`) + Gate 11g (runtime asset probe — extracts refs from served HTML, curls each, asserts 200); (4) prevention docs in `crates/gadgetron-web/README.md` §Asset consistency + `docs/manual/configuration.md` `[web].upgrade_insecure_requests` field with explicit "plain-HTTP deployments MUST leave this off" callout.
  ...
  - Reproduction context: `10.100.1.5:18080/web`, Chrome browser, no cookie + no Bearer pre-load. Binary at v0.5.18 (post-PR-#299). TASK 28.1 now requires v0.5.19 (post-PR-#304).
- **ISSUE 29 ⏳ `/web` shell unification refactor** (planned, post-v0.5.19; not a `v1.0.0` gate)
  - **Operator bug reports (2026-04-20)**: two related observations on the post-auth UI. (a) Each of `/web`, `/web/wiki`, `/web/dashboard` implements its own header / frame — the left rail (`Chat / Wiki / Dashboard / Knowledge / Bundles`) is present on chat but disappears on wiki and dashboard, so clicking `Dashboard` from chat navigates to a page with no left rail back to chat. (b) Starting a chat generation and switching tabs (clicking `Dashboard` from `LeftRail`) unmounts the `AssistantRuntimeProvider`; navigating back resets the in-flight chat. Both are rooted in the same structural problem — each page is a top-level Next.js route with its own layout tree; nothing is shared across routes.
    - **TASK 29.1 — Introduce `app/(shell)/layout.tsx` route-group layout.** Create a Next.js Route Group `(shell)` whose `layout.tsx` wraps all post-auth pages in a single `WorkbenchShell`. Pages under the group (`page.tsx`, `wiki/page.tsx`, `dashboard/page.tsx`) move into `(shell)/`. URL path is unaffected (Route Groups don't segment the URL). Result: navigating between Chat / Wiki / Dashboard keeps the shell's React tree mounted; only the page slot re-renders.
    - **TASK 29.2 — Hoist `AssistantRuntimeProvider` into `(shell)/layout.tsx`.** Today the provider lives inside `app/page.tsx`, so leaving `/web` unmounts the chat runtime + empties `useChatRuntime` state. Moving the provider up one level keeps the thread alive across navigation. Gate 11 (playwright `/web/dashboard` smoke) must still pass with the provider now rendered before Dashboard does any work.
    - **TASK 29.3 — `WorkbenchShell` slot API + single auth gate.** Refactor `WorkbenchShell` to accept `header` / `rightRail` slot props so each page supplies its own page-level header (e.g. `"Wiki Workbench · N pages + New/Refresh"` or `"Operator Dashboard · last 24h"`) inside the shared chrome. Lift the three independent API-key entry forms from `page.tsx` / `wiki/page.tsx` / `dashboard/page.tsx` into the shell's pre-auth branch so there is exactly one login UI.
    - **TASK 29.4 — `LeftRail.activeTab` from `usePathname()`.** Drop the `activeTab: LeftRailTab` prop; derive it from the current route via `usePathname()` so the correct tab highlights without parent plumbing. Remove the now-unused `activeTab` state from `WorkbenchShell`. Stubbed tabs (`knowledge`, `bundles`) stay `functional: false` and show the P2B notice when `usePathname()` matches `/` (chat default).
    - **TASK 29.5 — `crypto.randomUUID` fallback for non-secure contexts.** `app/wiki/page.tsx::invokeAction` calls `crypto.randomUUID()`, which is only defined in secure contexts (HTTPS or `localhost`). On a plain-HTTP bind like `10.100.1.5:18080`, `crypto.randomUUID is not a function` crashes the wiki page (observed 2026-04-20). Add a `safeRandomUuid()` helper that prefers `crypto.randomUUID` when available and falls back to `Math.random()`-based v4 spelling otherwise. Move the helper into `app/lib/uuid.ts` so the dashboard action-invoke path can reuse it.
    - **TASK 29.6 — Harness `/web/wiki` + `/web/dashboard` navigation smoke.** Extend Gate 11 (playwright) to sign in, click `Wiki` → `Dashboard` → `Chat` via the `LeftRail` links, and assert the StatusStrip + LeftRail DOM nodes remain mounted across the three transitions (`data-testid="status-strip"` + `data-testid="left-rail"` never detach). Catches regression of TASK 29.1 if someone reverts to per-page layouts.
  - Overlaps with **TASK 28.2** + **TASK 28.4**: the slot-API + single-auth-gate refactor resolves "hide nav chrome pre-auth" and "hide evidence pane pre-auth" as a side effect of having one shell in charge of pre- vs post-auth rendering. Mark those two TASKs closed when ISSUE 29 ships.
  - Not a `v1.0.0` gate (UI refactor, operator-visible but non-blocking).
  - Reproduction context: `10.100.1.5:18080/web`, Chrome browser. Binary at v0.5.19 (post-PR-#304).
