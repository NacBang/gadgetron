...
| **EPIC 4 XaaS rate-limit surface (ACTIVE, ISSUE 11 TASK 11.1 + 11.2 shipped, 0.5.0 → 0.5.2)** | [`manual/api-reference.md`](manual/api-reference.md) §Foundational error codes `quota_exceeded` row (documents the `Retry-After: 60` header + `retry_after_seconds: 60` body field landed by PR #230), [`manual/configuration.md`](manual/configuration.md) §`[quota_rate_limit]` (TASK 11.2 opt-in token-bucket config) | [`ROADMAP.md`](ROADMAP.md) §EPIC 4 ACTIVE block. TASK 11.1 (PR #230 / 0.5.1): structured 429 + Retry-After header. TASK 11.2 (PR #231 / 0.5.2): `TokenBucketRateLimiter` (DashMap-sharded, lazy refill, Instant clock, fractional tokens) wrapped by `RateLimitedQuotaEnforcer` composite — rate check runs FIRST (fail fast) before delegating to `InMemoryQuotaEnforcer` for cost; opt-in via `[quota_rate_limit] requests_per_minute = N, burst = M` (default `requests_per_minute = 0` = no-op, preserves pre-PR-#231 behavior). TASK 11.3 (Postgres-backed daily/monthly spend tracking that actually increments on `record_post` — `InMemoryQuotaEnforcer` never writes back today) + TASK 11.4 (`/web` UI surface for the 429 path — quota status banner, retry countdown) + ISSUE 12 billing + ISSUE 13 HF catalog + ISSUE 14 self-service remaining before EPIC 4 close → `v1.0.0`. |
