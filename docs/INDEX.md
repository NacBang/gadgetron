...
| **EPIC 4 XaaS rate-limit + spend tracking + quota introspection + billing ledger + tenant self-service + cookie-session login (ACTIVE, ISSUE 11 + ISSUE 12 telemetry + ISSUE 14 + ISSUE 15 complete, 0.5.0 → 0.5.8)** | [`manual/api-reference.md`](manual/api-reference.md) §Foundational error codes `quota_exceeded` row (PR #230) + §GET /api/v1/web/workbench/quota/status (PR #234 OpenAiCompat tenant introspection), [`manual/configuration.md`](manual/configuration.md) §`[quota_rate_limit]` (PR #231 opt-in token-bucket config), [`modules/xaas-platform.md`](modules/xaas-platform.md) §5.6 EPIC 4 ISSUE 11 enforcement stack | [`ROADMAP.md`](ROADMAP.md) §EPIC 4 ACTIVE / ISSUE 11 Completed. TASK 11.1 (PR #230 / 0.5.1): structured 429 + Retry-After header. TASK 11.2 (PR #231 / 0.5.2): `TokenBucketRateLimiter` (DashMap-sharded, lazy refill, Instant clock, fractional tokens) wrapped by `RateLimitedQuotaEnforcer` composite — rate check runs FIRST before delegating to inner cost enforcer; opt-in via `[quota_rate_limit]`. TASK 11.3 (PR #232 / 0.5.3): new `PgQuotaEnforcer` runs one UPDATE per request against `quota_configs` with CASE-expression rollover (daily zeros at UTC midnight, monthly at first-of-month — no background job); new migration `20260420000001_quota_usage_day.sql`; CLI picks `PgQuotaEnforcer` when `pg_pool.is_some()` else `InMemoryQuotaEnforcer`; fire-and-forget. TASK 11.4 (PR #234 / 0.5.4): `GET /quota/status` OpenAiCompat tenant introspection — response `{usage_day, daily, monthly}` each `{used_cents, limit_cents, remaining_cents}`, SQL CASE rollover inline, bootstrap-gap fallback to schema defaults when `quota_configs` row missing, new Gate 7k.5 exercises the fallback. Pipeline: rate-limit check → pg cost check → dispatch → pg `record_post` (quota_configs counter + billing_events ledger INSERT) → tenant introspection via /quota/status → rejections carry structured 429 + Retry-After. **TASK 12.1 (PR #236 / 0.5.5)**: new `billing_events` append-only ledger table (integer cents, `event_kind ∈ {chat, tool, action}`, Stripe-style FK-less `source_event_id`), `PgQuotaEnforcer::record_post` emits one INSERT per chat completion alongside the quota counter (fire-and-forget — failures log but don't propagate; reconciliation is TASK 12.4), Management-scoped `GET /api/v1/web/workbench/admin/billing/events` ledger query (newest-first, `limit.clamp(1, 500)`, optional `since` lower bound, handler pins tenant boundary). **TASK 12.2 (PR #241 / 0.5.6)** widened emission to tool + action success paths (`/v1/tools/{name}/invoke`, workbench direct-action, approved-action terminals) — each non-chat terminal now emits one `billing_events` row (kind=tool / kind=action; action rows carry `source_event_id = audit_event_id`); harness gates 7i.5 + 7h.1c. **ISSUE 12 closed at telemetry scope**. TASKs 12.3 (invoice materialization), 12.4 (reconciliation), 12.5 (Stripe ingest) + ISSUE 13 HF catalog cost-attribution are **commercialization layer, DEFERRED per 2026-04-20 scope direction** — they do NOT gate `v1.0.0`. **ISSUE 14 (PR #246 / 0.5.7)** shipped tenant self-service: new migration `20260420000004_users_teams_sessions.sql` (users / teams / team_members / user_sessions + api_keys user_id/label + audit_log actor cols), `[auth.bootstrap]` first-admin flow via argon2id + `GADGETRON_BOOTSTRAP_ADMIN_PASSWORD` env, Management-scoped `/admin/users` + `/admin/teams` CRUD (single-admin guard in delete path, tenant-boundary guard on team-member add), OpenAiCompat `/keys` user self-service (scope narrowing, idempotent revoke), `gadgetron {user,team} {create,list,delete}` CLI. Harness 115 PASS. **ISSUE 15 (PR #248 / 0.5.8)** shipped cookie-session login API on public routes: `POST /api/v1/auth/login` (email/password → argon2id verify → SHA-256-hashed session cookie), `POST /auth/logout`, `GET /auth/whoami` — 24h TTL, idle rotation via `last_active_at`, HttpOnly + SameSite=Lax (Secure via proxy). Harness 126 PASS. |
...
| **Plugin platform — DescriptorCatalog hot-reload + bundle manifests + bundle marketplace (EPIC 3 CLOSED `v0.5.0`, 2026-04-20, PR #228; 0.4.1 → 0.5.0)** | [`manual/api-reference.md`](manual/api-reference.md) §POST /api/v1/web/workbench/admin/reload-catalog (wire shape `{reloaded, action_count, view_count, source, source_path, bundle?, bundles?}`, `source` enum `{"seed_p2b", "config_file", "bundles_dir"}`, `bundle` carries optional `required_scope`) + §GET /admin/bundles (read-only enumeration) + §POST /admin/bundles (install with `{bundle_toml, signature_hex?}` body) + §DELETE /admin/bundles/{bundle_id} (uninstall), [`manual/configuration.md`](manual/configuration.md) §`[web]` (`catalog_path` / optional `[bundle]` table with `required_scope` / `bundles_dir` keys) + §`[web.bundle_signing]` (Ed25519 trust anchors + `require_signature`) | `Arc<ArcSwap<CatalogSnapshot>>` (catalog + pre-compiled JSON-schema validators 번들) — full path: ISSUE 8 TASK 8.1–8.5 (5 TASKs, PRs #211/#213/#214/#216/#217 — ArcSwap substrate → admin HTTP reload → CatalogSnapshot bundling → file-based TOML source → SIGHUP reloader) + ISSUE 9 TASK 9.1–9.3 (3 TASKs, PRs #219/#220/#222 — BundleMetadata + first-party bundle file + multi-bundle aggregation + bundle-driven harness default) + ISSUE 10 TASK 10.1–10.4 (4 TASKs, PRs #223/#224/#226/#227 — discovery + install/uninstall with `validate_bundle_id()` path-traversal regex + per-bundle `required_scope` inheritance + Ed25519 signed manifests with `[web.bundle_signing]` trust anchors), Management-scope 격리 규칙 (`scope_guard_middleware`); [`architecture/platform-architecture.md`](architecture/platform-architecture.md) §2.E.2 `/api/v1/web/workbench/admin/*` 안정성 행 + Axis B gadgetron-gateway 행; [`ROADMAP.md`](ROADMAP.md) §EPIC 3 CLOSED (fs-watcher deferred as optional TASK 8.6 if demand materializes); harness Gate 7q.1–7q.8 (8 gates covering reload + admin/bundles discovery + install + path-traversal + uninstall; 91 PASS, 0 FAIL). Released as `v0.5.0`. |
