...
| **EPIC 4 XaaS rate-limit surface (ACTIVE, ISSUE 11 TASK 11.1 shipped 0.5.1)** | [`manual/api-reference.md`](manual/api-reference.md) §Foundational error codes `quota_exceeded` row (documents the `Retry-After: 60` header + `retry_after_seconds: 60` body field landed by PR #230) | [`ROADMAP.md`](ROADMAP.md) §EPIC 4 ACTIVE block (ISSUE 11 TASK 11.2 token-bucket enforcer + ISSUE 12 billing + ISSUE 13 HF catalog + ISSUE 14 self-service remaining before EPIC 4 close → `v1.0.0`). `InMemoryQuotaEnforcer` in `gadgetron-xaas` still runs the trigger; TASK 11.2 replaces it with per-tenant token buckets. |
