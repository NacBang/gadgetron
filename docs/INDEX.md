...
| **Plugin platform — DescriptorCatalog hot-reload + bundle manifests (EPIC 3 ACTIVE; ISSUE 8 functionally complete + ISSUE 9 TASK 9.1 + 9.2 shipped, 0.4.1 → 0.4.7)** | [`manual/api-reference.md`](manual/api-reference.md) §POST /api/v1/web/workbench/admin/reload-catalog (Management-scoped HTTP path; wire shape `{reloaded, action_count, view_count, source, source_path, bundle?, bundles?}` — both bundle-related fields optional via `skip_serializing_if`), [`manual/configuration.md`](manual/configuration.md) §`[web]` 의 `catalog_path` 키 (TOML file source) + optional `[bundle]` 테이블 + `bundles_dir` 키 (multi-bundle directory source) | `Arc<ArcSwap<CatalogSnapshot>>` (catalog + pre-compiled JSON-schema validators 번들, ISSUE 8 TASK 8.1 `Arc<ArcSwap<DescriptorCatalog>>` 플러밍 → TASK 8.3 `CatalogSnapshot` 로 rev, PR #211 + #214) + TASK 8.4 file-based TOML source (`DescriptorCatalog::from_toml_file()` with parse-failure guard, PR #216) + TASK 8.5 POSIX `SIGHUP` reloader (`spawn_sighup_reloader()` Unix-only tokio task, shares `perform_catalog_reload()` with HTTP path so telemetry is identical, PR #217) + ISSUE 9 TASK 9.1 `BundleMetadata { id, version }` + optional `[bundle]` TOML 테이블 + first-party bundle file `bundles/gadgetron-core/bundle.toml` with drift-test invariant vs `seed_p2b()`, PR #219) + TASK 9.2 multi-bundle aggregation (`DescriptorCatalog::from_bundle_dir()` merges every `<bundles_dir>/<name>/bundle.toml` in alphabetical order, duplicate-id collisions → hard Config error naming both bundles, `bundles: Vec<BundleMetadata>` response field, precedence `bundles_dir` > `catalog_path` > `seed_p2b`, PR #220), Management-scope 격리 규칙 (`scope_guard_middleware`); [`architecture/platform-architecture.md`](architecture/platform-architecture.md) §2.E.2 `/api/v1/web/workbench/admin/*` 안정성 행 + Axis B gadgetron-gateway 행; [`ROADMAP.md`](ROADMAP.md) §EPIC 3 (fs-watcher = TASK 8.6 deferred, TASK 9.3 retire seed_p2b + ISSUE 10 bundle marketplace 남음); harness Gate 7q.1 (swap + read-path cross-check) + 7q.2 (OpenAiCompat → 403). EPIC 3 close → tag `v0.5.0`. |
