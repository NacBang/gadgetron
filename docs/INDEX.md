...
| **Plugin platform — DescriptorCatalog hot-reload + bundle manifests + bundle marketplace (EPIC 3 ACTIVE; ISSUE 8 + ISSUE 9 both functionally complete, ISSUE 10 TASK 10.1 + 10.2 + 10.3 shipped, 0.4.1 → 0.4.11)** | [`manual/api-reference.md`](manual/api-reference.md) §POST /api/v1/web/workbench/admin/reload-catalog (wire shape `{reloaded, action_count, view_count, source, source_path, bundle?, bundles?}`, `source` enum `{"seed_p2b", "config_file", "bundles_dir"}`, `bundle` now carries optional `required_scope`) + §GET /admin/bundles (read-only enumeration) + §POST /admin/bundles (install with `{bundle_toml}` body) + §DELETE /admin/bundles/{bundle_id} (uninstall), [`manual/configuration.md`](manual/configuration.md) §`[web]` 의 `catalog_path` / optional `[bundle]` 테이블 (now includes `required_scope`) / `bundles_dir` 키 (multi-bundle directory source, **harness + shipped default**) | `Arc<ArcSwap<CatalogSnapshot>>` (catalog + pre-compiled JSON-schema validators 번들, ISSUE 8 TASK 8.1 `Arc<ArcSwap<DescriptorCatalog>>` 플러밍 → TASK 8.3 `CatalogSnapshot` 로 rev, PR #211 + #214) + TASK 8.4 file-based TOML source (`DescriptorCatalog::from_toml_file()` with parse-failure guard, PR #216) + TASK 8.5 POSIX `SIGHUP` reloader (`spawn_sighup_reloader()` shares `perform_catalog_reload()` with HTTP path, PR #217) + ISSUE 9 TASK 9.1 `BundleMetadata { id, version }` + optional `[bundle]` TOML 테이블 + first-party bundle file + drift-test vs `seed_p2b()`, PR #219) + TASK 9.2 multi-bundle aggregation (`DescriptorCatalog::from_bundle_dir()` merges every `<bundles_dir>/<name>/bundle.toml`, duplicate-id → hard Config, `bundles: Vec<BundleMetadata>` response, precedence `bundles_dir` > `catalog_path` > `seed_p2b`, PR #220) + TASK 9.3 bundle-driven harness default (source enum 3-value, Gate 7q.1 retargeted + new Gate 7q.3, PR #222) + ISSUE 10 TASK 10.1 bundle-discovery endpoint (read-only `GET /admin/bundles`, PR #223) + TASK 10.2 install/uninstall runtime (`POST /admin/bundles` + `DELETE /admin/bundles/{bundle_id}` with `validate_bundle_id()` path-traversal-safe regex + collision-rejection + reload-hint to keep snapshot swap explicit, PR #224) + TASK 10.3 bundle-level scope inheritance (`BundleMetadata.required_scope` floors every view/action without its own, narrower wins, zero-overhead when unset, PR #226), Management-scope 격리 규칙 (`scope_guard_middleware`); [`architecture/platform-architecture.md`](architecture/platform-architecture.md) §2.E.2 `/api/v1/web/workbench/admin/*` 안정성 행 + Axis B gadgetron-gateway 행; [`ROADMAP.md`](ROADMAP.md) §EPIC 3 (fs-watcher = TASK 8.6 deferred; **TASK 10.4 signed manifests** 만 EPIC 3 close 전 남음); harness Gate 7q.1–7q.8 (8 gates covering reload + admin/bundles discovery + install + path-traversal + uninstall). EPIC 3 close → tag `v0.5.0`. |
