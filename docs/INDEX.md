...
| **EPIC 4 XaaS rate-limit + spend tracking surface (ACTIVE, ISSUE 11 TASK 11.1 + 11.2 + 11.3 shipped, 0.5.0 → 0.5.3)** | [`manual/api-reference.md`](manual/api-reference.md) §Foundational error codes `quota_exceeded` row (PR #230 `Retry-After` header + `retry_after_seconds` body field), [`manual/configuration.md`](manual/configuration.md) §`[quota_rate_limit]` (PR #231 opt-in token-bucket config) | [`ROADMAP.md`](ROADMAP.md) §EPIC 4 ACTIVE block. TASK 11.1 (PR #230 / 0.5.1): structured 429 + Retry-After header. TASK 11.2 (PR #231 / 0.5.2): `TokenBucketRateLimiter` (DashMap-sharded, lazy refill, Instant clock, fractional tokens) wrapped by `RateLimitedQuotaEnforcer` composite — rate check runs FIRST before delegating to inner cost enforcer; opt-in via `[quota_rate_limit]`. TASK 11.3 (PR #232 / 0.5.3): new `PgQuotaEnforcer` in `gadgetron-xaas::quota::enforcer` runs one UPDATE per request against `quota_configs` with CASE-expression rollover (daily zeros at UTC midnight, monthly at first-of-month — no background job; rollover happens on the first post-boundary request); new migration `20260420000001_quota_usage_day.sql` adds `usage_day DATE DEFAULT CURRENT_DATE`; CLI picks `PgQuotaEnforcer` when `pg_pool.is_some()` else `InMemoryQuotaEnforcer`; fire-and-forget UPDATE (errors log but never propagate); pipeline = rate-limit check → pg cost check → dispatch → pg `record_post`. TASK 11.4 (`/web` UI surface for the 429 path — quota status banner, retry countdown) + ISSUE 12 billing + ISSUE 13 HF catalog + ISSUE 14 self-service remaining before EPIC 4 close → `v1.0.0`. |
