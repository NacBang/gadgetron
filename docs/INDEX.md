...
| **외부 MCP 클라이언트 surface — tool discovery + invoke (0.2.10+)** | [`manual/api-reference.md`](manual/api-reference.md) §GET /v1/tools + §POST /v1/tools/{name}/invoke | `GadgetCatalog` trait (core), L3 allowed-names gate via `Arc<dyn GadgetDispatcher>`; [`architecture/platform-architecture.md`](architecture/platform-architecture.md) Axis B gadgetron-core row (EPIC 2 / ISSUE 7 TASKs 7.1/7.2) |
