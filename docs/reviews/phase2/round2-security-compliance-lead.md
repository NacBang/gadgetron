...
|----|-------------|--------|----------|
| SEC-1 | STRIDE threat model per component | **APPROVED** | `00-overview.md` §8 STRIDE table (5 components × 7 categories); `02-kairos-agent.md` §15 (6 sub-components); trust boundary table B1-B6 present. Assets documented. Meets §1.5-A requirement. |
...
| SEC-6 | M6 — audit log stores tool names only, not arguments | **APPROVED** | `02-kairos-agent.md §6.2 stream.rs::event_to_chat_chunks` ToolUse arm logs `tool_name = %name` only via `tracing::info!`, discards `input` field. Test `tool_call_log_contains_name_not_args` enforces this. `00-overview.md §8` M6 defines `tools_called: Vec<String>`. |
| SEC-7 | Manual warning re --dangerously-skip-permissions (pre-merge gate) | **APPROVED** | `00-overview.md §10` specifies both Disclosure 1 (git permanence) and Disclosure 2 (SearXNG) as verbatim-locked text. ADR-P2A-03 designates `docs/manual/kairos.md` as a blocking pre-merge gate. ADR-P2A-02 §Consequences requires explanation of `--dangerously-skip-permissions` in the manual. Gate mechanism documented. **Note: `docs/manual/kairos.md` does not yet exist** — this is correct per the spec (written before first code PR, not before design review). |
...
| SEC-10 | P2C reopen security threat model tag | **APPROVED** | `[P2C-SECURITY-REOPEN]` tags appear in: `02-kairos-agent.md §10 config.rs` (ANTHROPIC_BASE_URL), `01-knowledge-layer.md §5.2 searxng.rs` (SSRF), `ADR-P2A-02 §Non-applicability`, `ADR-P2A-03 §For P2C`, `02-kairos-agent.md §15.5`. The tag is present in all five critical locations identified in SEC-10. |
...
- **Location**: `02-kairos-agent.md §5.1 spawn.rs::build_claude_command` (line ~434-465)
...
- **Severity**: HIGH
- **Spec location to update**: `02-kairos-agent.md §5.1 spawn.rs` + `00-overview.md §8 Appendix B` (subprocess environment section currently says "Everything else default" which must change to "env_clear + explicit allowlist")

---

### BLOCKER-2: Audit entry schema lacks `request_id` — forensic correlation impossible
- **Location**: `00-overview.md §8` audit logging section (line ~492-497); `02-kairos-agent.md §6.2` (tracing events)
- **Issue**: The specified audit fields are `kairos_dispatched: bool`, `tools_called: Vec<String>`, and `subprocess_duration_ms: i32`. The observability section (§11) mentions "Log Claude Code stderr at debug level with `request_id` correlation tag" in a tracing span, but there is no `request_id` field in the persisted `AuditEntry` struct extension. The `AuditEntry` extended for kairos cannot be correlated with the gateway's HTTP request audit record or with the tracing span. If a `wiki_write_secret_suspected` audit event fires, there is no way to identify which HTTP request, which user session, or which timestamp triggered it without parsing unstructured log lines. This violates the SOC2 CC7.2 anomaly triage requirement the spec claims to satisfy. Furthermore, the `tools_called` log in `stream.rs` is a `tracing::info!` event, not a persisted audit entry — the spec never specifies how `tools_called` names flow from the tracing event into the `AuditEntry.tools_called: Vec<String>` field. This mechanism is missing.
- **Fix required**:
  1. Add `request_id: String` to the kairos `AuditEntry` extension fields in `00-overview.md §8` and in whatever code spec accumulates them
  2. Specify the exact mechanism by which `tools_called` values are accumulated: the spec in `02-kairos-agent.md §6.2` uses `tracing::info!` only; either an in-memory accumulator in `ClaudeCodeSession` collects tool names and writes them to `AuditEntry` at session end, OR a structured tracing subscriber extracts them. The current spec leaves this gap.
...
- **Location**: `02-kairos-agent.md §8 redact.rs::REDACTION_PATTERNS` (line ~678-682)
...
`02-kairos-agent.md §8` has `fn is_idempotent()` unit test listed but no proptest for idempotency. Add a proptest: `prop_redact_is_idempotent(s in "\\PC*") { assert_eq!(redact_stderr(&redact_stderr(&s)), redact_stderr(&s)); }`.
...
3. **`02-kairos-agent.md §10 config.rs` — `claude_binary` validation underspecified**: Validation is `!is_empty()`. An implementer cannot determine from this spec whether relative paths, paths with `.`, or shell metacharacters are valid. BLOCKER-3 covers this.
...
**v2 successfully resolves all 10 v1 blockers at the specification level.** The STRIDE models (per `00-overview.md §8` and `02-kairos-agent.md §15`) meet the Round 1.5 requirements: assets, trust boundaries, and per-component threat tables are present and coherent. The three ADRs are structurally sound and appropriately scope the P2A risk acceptance.
...
**Readiness for implementation**: NOT READY. The four blockers above must be addressed in v2.1 of the design docs before any `gadgetron-kairos` or `gadgetron-knowledge` code is written. The ADR-P2A-01 behavioral verification (M4) also remains pending and must be completed before kairos code starts. Once BLOCKER-1..4 are resolved and M4 verified, the security posture for P2A single-user local deployment is coherent and implementation may proceed.
