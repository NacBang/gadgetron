...
> **버전**: `0.5.0` (Phase 2 EPIC 1 + EPIC 2 + **EPIC 3 모두 CLOSED**) · EPIC 3 (Plugin platform) 이 PR #228 에서 close 되어 `v0.5.0` tag 가 찍혔음. EPIC 3 는 ISSUE 8 (DescriptorCatalog hot-reload, 5 TASKs, v0.4.1–v0.4.5) + ISSUE 9 (real bundle manifests, 3 TASKs, v0.4.6–v0.4.8) + ISSUE 10 (bundle marketplace, 4 TASKs, v0.4.9–v0.4.12) 세 ISSUE 를 landed — operator 는 이제 HTTP + SIGHUP reload + file-based `bundles_dir` TOML source + Ed25519 signed manifest install 을 프로덕션 코드 경로로 사용 가능. Harness 91 PASS, 0 FAIL. **EPIC 4 (Multi-tenant business ops / XaaS — integer-cent billing ADR-D-8, HuggingFace catalog, tenant self-service, quotas + SLA enforcement) 이 다음 active target 으로 `v1.0.0` (첫 production release) 을 향함. EPIC 5 (Cluster platform) 이 그 다음 `v2.0.0`**. 이전 tag `v0.5.0` (EPIC 3 closure, 2026-04-20), `v0.4.0` (EPIC 2 closure), `v0.3.0` (EPIC 1 closure), `v0.1.0-phase1` (역사) · 버저닝 정책 `docs/process/06-versioning-policy.md`
> **에디션**: 2021
> **라이선스**: MIT
> **최소 Rust**: 1.80
> **바이너리 이름**: `gadgetron`
