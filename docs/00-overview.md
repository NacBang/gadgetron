...
> **버전**: `0.4.4` (Phase 2 EPIC 1 + EPIC 2 CLOSED; Phase 2 EPIC 3 **Plugin platform ACTIVE** — ISSUE 8 TASK 8.1 shipped as PR #211 `Arc<ArcSwap<DescriptorCatalog>>` 플러밍 substrate (0.4.1), TASK 8.2 shipped as PR #213 `POST /api/v1/web/workbench/admin/reload-catalog` Management-scoped hot-reload endpoint (0.4.2), TASK 8.3 shipped as PR #214 `CatalogSnapshot { catalog, validators }` bundling (0.4.3) — handle widened to `Arc<ArcSwap<CatalogSnapshot>>` so a reload swaps catalog + pre-compiled JSON-schema validators in lockstep, closing the TASK 8.2 validator-rebuild gap; TASK 8.4 shipped as PR #216 file-based catalog source (0.4.4) — new `[web] catalog_path` config key points the reload handler at a TOML file via `DescriptorCatalog::from_toml_file()`, parse failures 500 without replacing running snapshot, response widens with `source: "config_file"` + `source_path`; TASK 8.5 (fs-watcher + SIGHUP handler) 남음) · 이전 tag `v0.4.0` (EPIC 2 closure), `v0.3.0` (EPIC 1 closure), `v0.1.0-phase1` (역사) · EPIC 3 close → `v0.5.0` tag, EPIC 4 close → `v1.0.0` (first production release) · 버저닝 정책 `docs/process/06-versioning-policy.md`
> **에디션**: 2021
> **라이선스**: MIT
> **최소 Rust**: 1.80
> **바이너리 이름**: `gadgetron`
