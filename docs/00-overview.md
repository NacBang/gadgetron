...
> **버전**: `0.4.11` (Phase 2 EPIC 1 + EPIC 2 CLOSED; Phase 2 EPIC 3 **Plugin platform ACTIVE** — **ISSUE 8 + ISSUE 9 both functionally complete; ISSUE 10 (bundle marketplace) in-flight** with TASKs 10.1 + 10.2 + 10.3 shipped. ISSUE 8 (5 TASKs, PRs #211/#213/#214/#216/#217): `Arc<ArcSwap<_>>` 플러밍 substrate → `CatalogSnapshot` 번들링, `POST /admin/reload-catalog` Management-scoped HTTP endpoint, file-based TOML source via `[web] catalog_path`, POSIX `SIGHUP` reloader via `perform_catalog_reload()` 공유 helper. ISSUE 9 (3 TASKs, PRs #219/#220/#222): `BundleMetadata` + first-party `bundles/gadgetron-core/bundle.toml`, multi-bundle aggregation via `[web] bundles_dir` + `from_bundle_dir()` + 3-level precedence + 3-value source enum `{seed_p2b, config_file, bundles_dir}` + bundle-driven harness default. **ISSUE 10 TASKs 10.1 + 10.2 + 10.3 shipped (PRs #223 / #224 / #226)** — TASK 10.1 `GET /admin/bundles` read-only bundle-discovery; TASK 10.2 `POST /admin/bundles` (install with `{bundle_toml}` body, id regex `[a-zA-Z0-9_-]{1,64}` path-traversal safe, collision = 4xx Config) + `DELETE /admin/bundles/{bundle_id}` (uninstall via `remove_dir_all` with same id guard), both composable with reload (install writes to disk but does NOT swap live catalog, `reload_hint` in response points at reload endpoint); TASK 10.3 bundle-level scope inheritance — `BundleMetadata.required_scope: Option<Scope>` at `[bundle]` level floors every view/action the bundle ships, narrower explicit scopes keep theirs, zero-overhead default. Harness PASS 84 → 91 (Gates 7q.4/7q.5 TASK 10.1 + 7q.6/7q.7/7q.8 TASK 10.2; TASK 10.3 adds no new gates). TASK 10.4 (Ed25519-signed manifests) 가 EPIC 3 close 전 남은 유일한 일) · 이전 tag `v0.4.0` (EPIC 2 closure), `v0.3.0` (EPIC 1 closure), `v0.1.0-phase1` (역사) · EPIC 3 close → `v0.5.0` tag, EPIC 4 close → `v1.0.0` (first production release) · 버저닝 정책 `docs/process/06-versioning-policy.md`
> **에디션**: 2021
> **라이선스**: MIT
> **최소 Rust**: 1.80
> **바이너리 이름**: `gadgetron`
