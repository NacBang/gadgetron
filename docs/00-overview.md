...
Gadgetron은 지식 협업 플랫폼이다. 사용자의 로컬 지식과 웹 정보를 **지식 레이어**에 쌓고, Penny 가 그 레이어를 근거로 사용자의 요청을 해결한다. 기능은 **MCP 플러그인**으로 확장한다.
...
Penny (Penny Brown) 는 사용자와 대화하며 Gadgetron 의 지식 레이어와 플러그인을 도구로 삼아 요청을 해결한다.
...
| `gadgetron-provider` | 6종 LLM 프로바이더 어댑터 (OpenAI / Anthropic / Gemini / Ollama / vLLM / SGLang) + 프로토콜 변환 | [`docs/modules/model-serving.md`](modules/model-serving.md) §2–3 |
| `gadgetron-router` | 6종 라우팅 전략(RoundRobin / CostOptimal / LatencyOptimal / QualityOptimal / Fallback / Weighted) + lock-free `MetricsStore` | [`docs/modules/gateway-routing.md`](modules/gateway-routing.md) §4 |
...
| `gadgetron-scheduler` | VRAM 인식 모델 배포 + LRU / Priority eviction + 노드 등록 관리 | [`docs/modules/gpu-resource-manager.md`](modules/gpu-resource-manager.md) §6 · [`docs/modules/model-serving.md`](modules/model-serving.md) §5 |
| `gadgetron-node` | `NodeAgent`(엔진별 프로세스 시작/정지) + `ResourceMonitor`(sysinfo + NVML feature gate) | [`docs/modules/gpu-resource-manager.md`](modules/gpu-resource-manager.md) §1 · [`docs/modules/deployment-operations.md`](modules/deployment-operations.md) |
...
| `gadgetron-knowledge` *(P2A)* | wiki (md + git2 + Obsidian `[[link]]`) + SearXNG 프록시 + `KnowledgeToolProvider` | [`docs/design/phase2/01-knowledge-layer.md`](design/phase2/01-knowledge-layer.md) |
| `gadgetron-penny` *(P2A)* | Claude Code subprocess 브릿지 + `McpToolRegistry` + `LlmProvider` 구현 (model = `penny`) | [`docs/design/phase2/02-penny-agent.md`](design/phase2/02-penny-agent.md) · [`docs/design/phase2/04-mcp-tool-registry.md`](design/phase2/04-mcp-tool-registry.md) |
...
**목표**: Phase 1 substrate 위에 지식 레이어(LLM Wiki + 웹 조사 + RAW 입수 + 검색 인덱스), Penny, MCP 플러그인 확장 surface를 올려 지식 협업 플랫폼을 구성.
