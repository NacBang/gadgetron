...
> **버전**: `0.4.9` (Phase 2 EPIC 1 + EPIC 2 CLOSED; Phase 2 EPIC 3 **Plugin platform ACTIVE** — **ISSUE 8 + ISSUE 9 both functionally complete; ISSUE 10 (bundle marketplace) in-flight** with TASK 10.1 shipped. ISSUE 8 (5 TASKs, PRs #211/#213/#214/#216/#217): `Arc<ArcSwap<_>>` 플러밍 substrate → `CatalogSnapshot` 번들링, `POST /admin/reload-catalog` Management-scoped HTTP endpoint, file-based TOML source via `[web] catalog_path`, POSIX `SIGHUP` reloader via `perform_catalog_reload()` 공유 helper. ISSUE 9 (3 TASKs, PRs #219/#220/#222): `BundleMetadata` + first-party `bundles/gadgetron-core/bundle.toml`, multi-bundle aggregation via `[web] bundles_dir` + `from_bundle_dir()` 가 subdirectory 별 `bundle.toml` 을 merge + 중복 id 는 hard Config error + `bundles: Vec<BundleMetadata>` response 확장 + 3-level precedence, bundle-driven harness default 로 source enum 이 3 값 `{seed_p2b, config_file, bundles_dir}` 로 확장 + seed_p2b 는 unit-test fixture + drift-guard 로만 존속. **ISSUE 10 TASK 10.1 shipped as PR #223 / 0.4.9** — `GET /api/v1/web/workbench/admin/bundles` Management-scoped bundle-discovery endpoint (read-only, does NOT ArcSwap live catalog, subdir-sorted deterministic ordering matches `from_bundle_dir`, 503 Config 503 when bundles_dir unwired), response shape `{bundles_dir, count, bundles: [{bundle?, source_path, action_count, view_count}, ...]}`, harness PASS 84 → 86 via new Gates 7q.4 (shape + gadgetron-core enumeration) + 7q.5 (RBAC 403). TASK 10.2 (install/uninstall runtime) + TASK 10.3 (per-bundle scope isolation) + TASK 10.4 (Ed25519-signed manifests) 가 EPIC 3 close 전 남은 일) · 이전 tag `v0.4.0` (EPIC 2 closure), `v0.3.0` (EPIC 1 closure), `v0.1.0-phase1` (역사) · EPIC 3 close → `v0.5.0` tag, EPIC 4 close → `v1.0.0` (first production release) · 버저닝 정책 `docs/process/06-versioning-policy.md`
> **에디션**: 2021
> **라이선스**: MIT
> **최소 Rust**: 1.80
> **바이너리 이름**: `gadgetron`
...
### Phase 2 — Knowledge Layer + Penny + Bundle Surfaces (v0.2 → **v0.4.2**, EPIC 1 + EPIC 2 CLOSED; EPIC 3 Plugin platform **ACTIVE**)
