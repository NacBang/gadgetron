...
> - **ISSUE 24** thread real `user_id` through `QuotaToken` + `AuthenticatedContext` ⏳ planned — closes the chat + action NULL `actor_user_id` from ISSUE 23. Not a `v1.0.0` gate. Full sketch + Gate 7k.6b flip in [`ROADMAP.md §EPIC 4 ISSUE 24`](ROADMAP.md).
...
- EPIC 4 (Multi-tenant business ops / XaaS) — **ACTIVE (2026-04-20 승격, `v1.0.0` target)**. ISSUEs 11 / 12 (telemetry) / 14 / 15 / 16 / 17 / 19 / 20 / 21 / 22 / 23 모두 완료 (0.5.0 → 0.5.15). Remaining multi-user scope: **ISSUE 18 (web UI login form)** — 하나만 남았음. 12.3 / 12.4 / 12.5 + ISSUE 13 (HF catalog) 은 commercialization layer 로 2026-04-20 scope direction 에 따라 **DEFERRED** — `v1.0.0` cut 의 gate 가 아님. Full pipeline: rate-limit check → pg cost check → dispatch → pg record_post (quota counter + billing_events ledger with `actor_user_id` column per ISSUE 23 for ledger-side per-user attribution) → `/quota/status` tenant 내부 조회 → audit persistence via `run_audit_log_writer` (ISSUE 21) → operator query via `GET /admin/audit/log` (ISSUE 22) → rejection 은 구조화된 429. 전체 per-TASK / per-PR chain 은 [`ROADMAP.md`](ROADMAP.md) §EPIC 4. EPIC 4 close → `0.5.x` → `v1.0.0` (major bump — API stabilizes).
...
- **멀티 테넌시 강화 / GPUaaS / ModelaaS / AgentaaS** → EPIC 4 ACTIVE (ISSUE 11 quotas ✅, ISSUE 12 billing telemetry ✅ (invoicing DEFERRED), ISSUE 14 self-service ✅ (PR #246), ISSUE 15 cookie-session login API ✅ (PR #248), ISSUE 16 unified Bearer-or-cookie middleware ✅ (PR #259 / v0.5.9), ISSUE 17 `ValidatedKey.user_id` plumbing ✅ (PR #260 / v0.5.10), ISSUE 19 `AuditEntry` actor fields structural ✅ (PR #262 / v0.5.11), ISSUE 20 TenantContext → AuditEntry plumbing ✅ (PR #263 / v0.5.12), ISSUE 21 pg `audit_log` consumer ✅ (PR #267 / v0.5.13), ISSUE 22 admin `/audit/log` query endpoint ✅ (PR #269 / v0.5.14), ISSUE 23 `billing_events.actor_user_id` ✅ (PR #271 / v0.5.15) — ledger-side per-user attribution; tool + action emitters populate from TenantContext, chat path stays `None` until ISSUE 24 threads user_id through QuotaToken; ISSUE 13 HF catalog discovery/pinning + **ISSUE 18** web UI login form (React/Tailwind in `gadgetron-web`) 남음, 닫으면 `v1.0.0` first production release).
