...
- **멀티 테넌시 강화 / GPUaaS / ModelaaS / AgentaaS** → EPIC 4 ACTIVE (ISSUE 11 quotas ✅, ISSUE 12 billing telemetry ✅ (invoicing DEFERRED), ISSUE 14 self-service ✅ (PR #246), ISSUE 15 cookie-session login API ✅ (PR #248), ISSUE 16 unified Bearer-or-cookie middleware ✅ (PR #259 / v0.5.9), ISSUE 17 `ValidatedKey.user_id` plumbing ✅ (PR #260 / v0.5.10), ISSUE 19 `AuditEntry` actor fields structural ✅ (PR #262 / v0.5.11), ISSUE 20 TenantContext → AuditEntry plumbing ✅ (PR #263 / v0.5.12), ISSUE 21 pg `audit_log` consumer ✅ (PR #267 / v0.5.13), ISSUE 22 admin `/audit/log` query endpoint ✅ (PR #269 / v0.5.14), ISSUE 23 `billing_events.actor_user_id` ✅ (PR #271 / v0.5.15), ISSUE 24 `QuotaToken.user_id` + `AuthenticatedContext.real_user_id` threading ✅ (PR #289 / v0.5.16) — chat + tool + action `billing_events` all populate `actor_user_id` with the same real UUID per request, ISSUE 25 `AuthenticatedContext.user_id` → `api_key_id` rename + audit_log contamination fix ✅ (PR #293 / v0.5.17) — action audit sinks emit `actor.real_user_id.unwrap_or(actor.api_key_id)`; ISSUE 13 HF catalog discovery/pinning + **ISSUE 18** web UI login form (React/Tailwind in `gadgetron-web`) 남음, 닫으면 `v1.0.0` first production release).
