...
> **버전**: `0.4.6` (Phase 2 EPIC 1 + EPIC 2 CLOSED; Phase 2 EPIC 3 **Plugin platform ACTIVE** — **ISSUE 8 functionally complete** (TASK 8.1 `Arc<ArcSwap<DescriptorCatalog>>` 플러밍 substrate PR #211 / 0.4.1, TASK 8.2 `POST /admin/reload-catalog` Management-scoped HTTP endpoint PR #213 / 0.4.2, TASK 8.3 `CatalogSnapshot { catalog, validators }` bundling PR #214 / 0.4.3 — 핸들이 `Arc<ArcSwap<CatalogSnapshot>>` 로 확장되어 catalog+validators 를 lockstep 으로 교체, TASK 8.4 file-based TOML source PR #216 / 0.4.4 — `[web] catalog_path` + `DescriptorCatalog::from_toml_file()` + 파싱 실패 500 (running snapshot 교체 안 됨), TASK 8.5 POSIX `SIGHUP` reloader PR #217 / 0.4.5 — `spawn_sighup_reloader()` Unix-only tokio task + HTTP path 와 동일한 `perform_catalog_reload()` helper 공유, fs-watcher 는 TASK 8.6 로 deferred); **ISSUE 9 (real bundle manifests) STARTED** — TASK 9.1 shipped as PR #219 / 0.4.6 (`BundleMetadata { id, version }` 구조체 + optional `[bundle]` TOML 테이블 + reload response 에 `bundle` 필드 추가 + `bundles/gadgetron-core/bundle.toml` first-party bundle file, drift-test 로 seed_p2b 와 동일성 보장); TASK 9.2 (multi-bundle aggregation) + TASK 9.3 (retire seed_p2b) + ISSUE 10 (bundle marketplace) 가 EPIC 3 close 전 남은 일) · 이전 tag `v0.4.0` (EPIC 2 closure), `v0.3.0` (EPIC 1 closure), `v0.1.0-phase1` (역사) · EPIC 3 close → `v0.5.0` tag, EPIC 4 close → `v1.0.0` (first production release) · 버저닝 정책 `docs/process/06-versioning-policy.md`
> **에디션**: 2021
> **라이선스**: MIT
> **최소 Rust**: 1.80
> **바이너리 이름**: `gadgetron`
