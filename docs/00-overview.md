...
> **버전**: `0.4.5` (Phase 2 EPIC 1 + EPIC 2 CLOSED; Phase 2 EPIC 3 **Plugin platform ACTIVE — ISSUE 8 functionally complete** — TASK 8.1 `Arc<ArcSwap<DescriptorCatalog>>` 플러밍 substrate PR #211 (0.4.1), TASK 8.2 `POST /api/v1/web/workbench/admin/reload-catalog` Management-scoped HTTP endpoint PR #213 (0.4.2), TASK 8.3 `CatalogSnapshot { catalog, validators }` bundling PR #214 (0.4.3) — 핸들이 `Arc<ArcSwap<CatalogSnapshot>>` 로 확장되어 reload 가 catalog + pre-compiled JSON-schema validators 를 lockstep 으로 교체, TASK 8.4 file-based catalog source PR #216 (0.4.4) — `[web] catalog_path` 설정 키 + `DescriptorCatalog::from_toml_file()` + 파싱 실패 500 (running snapshot 교체 안 됨), TASK 8.5 POSIX `SIGHUP` reloader PR #217 (0.4.5) — `spawn_sighup_reloader()` Unix-only tokio task 가 HTTP path 와 동일한 `perform_catalog_reload()` helper 재사용, `kill -HUP <pid>` 로 trigger. Operator reload surface 3 개 (HTTP + SIGHUP + shared helper), fs-watcher 은 demand 에 따라 TASK 8.6 로 deferred. ISSUE 9 (real bundle manifests), ISSUE 10 (bundle marketplace) 가 EPIC 3 close 전 남은 일) · 이전 tag `v0.4.0` (EPIC 2 closure), `v0.3.0` (EPIC 1 closure), `v0.1.0-phase1` (역사) · EPIC 3 close → `v0.5.0` tag, EPIC 4 close → `v1.0.0` (first production release) · 버저닝 정책 `docs/process/06-versioning-policy.md`
> **에디션**: 2021
> **라이선스**: MIT
> **최소 Rust**: 1.80
> **바이너리 이름**: `gadgetron`
