...
> **버전**: `0.5.2` (Phase 2 EPIC 1 + EPIC 2 + **EPIC 3 모두 CLOSED**; **EPIC 4 ACTIVE**) · EPIC 3 는 ISSUE 8 (hot-reload, 5 TASKs, v0.4.1–v0.4.5) + ISSUE 9 (real bundle manifests, 3 TASKs, v0.4.6–v0.4.8) + ISSUE 10 (bundle marketplace, 4 TASKs, v0.4.9–v0.4.12) 세 ISSUE 를 landed 후 PR #228 / v0.5.0 close. **EPIC 4 (Multi-tenant business ops / XaaS) 가 2026-04-20 ACTIVE 로 승격, `v1.0.0` (첫 production release) 를 향함**. ISSUE 11 (quotas + rate limits) 가 첫 ISSUE — TASK 11.1 ✅ (PR #230 / 0.5.1) HTTP 429 에 `Retry-After: 60` + `retry_after_seconds: 60` 필드; **TASK 11.2 ✅ (PR #231 / 0.5.2) 새 per-tenant `TokenBucketRateLimiter` + `RateLimitedQuotaEnforcer` composite 가 rate check 를 cost 체크보다 먼저 실행 (fail fast); opt-in `[quota_rate_limit] requests_per_minute = N, burst = M` 새 설정 — default `requests_per_minute = 0` = no-op 으로 기존 배포는 동작 변화 없음**; TASK 11.3 (limiter 의 실제 refill countdown 을 `GadgetronError` 를 통해 `Retry-After` 에 전달, 60s conservative 상수 대체) 이 남음. Harness 91 PASS, 0 FAIL. EPIC 5 (Cluster platform, PLANNED post-1.0) 이 그 다음 `v2.0.0`. 이전 tag `v0.5.0` (EPIC 3 closure, 2026-04-20), `v0.4.0` (EPIC 2 closure), `v0.3.0` (EPIC 1 closure), `v0.1.0-phase1` (역사) · 버저닝 정책 `docs/process/06-versioning-policy.md`
> **에디션**: 2021
> **라이선스**: MIT
> **최소 Rust**: 1.80
> **바이너리 이름**: `gadgetron`
