...
### Phase 2 — Knowledge Layer + Penny + Bundle Surfaces + Multi-tenant XaaS (v0.2 → **v0.5.18**, EPIC 1 + EPIC 2 + EPIC 3 CLOSED; EPIC 4 **ACTIVE** — ISSUE 11 + ISSUE 12 (telemetry) + ISSUE 14 (self-service) + ISSUE 15 (cookie login) + ISSUE 16 (unified auth middleware) + ISSUE 17 (`ValidatedKey.user_id` plumbing) + ISSUE 19 (`AuditEntry` actor fields structural) + ISSUE 20 (TenantContext → AuditEntry plumbing) + ISSUE 21 (pg audit_log consumer) + ISSUE 22 (admin `/audit/log` query endpoint) + ISSUE 23 (`billing_events.actor_user_id` column) + ISSUE 24 (QuotaToken.user_id + AuthenticatedContext.real_user_id threading — chat + action populate) + ISSUE 25 (`AuthenticatedContext.user_id` → `api_key_id` rename + audit_log contamination fix) + ISSUE 26 (billing-insert SLO counter + `/admin/billing/insert-failures` endpoint) complete; 12.3 / 12.4 / 12.5 + 13 cost-attribution DEFERRED per 2026-04-20 commercialization-layer direction; ISSUE 18 web UI login form remains on multi-user track; ISSUE 27 `real_user_id` → `user_id` rename completion is a DX follow-up, not a v1.0.0 gate)
...
- EPIC 4 (Multi-tenant business ops / XaaS) — **ACTIVE (2026-04-20 승격, `v1.0.0` target)**. ISSUEs 11 / 12 (telemetry) / 14 / 15 / 16 / 17 / 19 / 20 / 21 / 22 / 23 / 24 / 25 / 26 모두 완료 (0.5.0 → 0.5.18). Remaining multi-user scope: **ISSUE 18 (web UI login form)** — 하나만 남았음. 12.3 / 12.4 / 12.5 + ISSUE 13 (HF catalog) 은 commercialization layer 로 2026-04-20 scope direction 에 따라 **DEFERRED** — `v1.0.0` cut 의 gate 가 아님. ISSUE 27 (`AuthenticatedContext.real_user_id` → `user_id` rename) 은 DX follow-up, not a `v1.0.0` gate. Full pipeline: rate-limit check → pg cost check → dispatch → pg record_post (quota counter + billing_events ledger with `actor_user_id` populated end-to-end via ISSUE 23 column + ISSUE 24 QuotaToken/AuthenticatedContext plumbing + ISSUE 25 audit-log contamination fix + ISSUE 26 insert-failure observability counter) → `/quota/status` tenant 내부 조회 → audit persistence via `run_audit_log_writer` (ISSUE 21) → operator query via `GET /admin/audit/log` (ISSUE 22) → rejection 은 구조화된 429. 전체 per-TASK / per-PR chain 은 [`ROADMAP.md`](ROADMAP.md) §EPIC 4. EPIC 4 close → `0.5.x` → `v1.0.0` (major bump — API stabilizes).
