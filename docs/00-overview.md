...
> **버전**: `0.5.5` (Phase 2 EPIC 1 + EPIC 2 + **EPIC 3 모두 CLOSED**; **EPIC 4 ACTIVE, ISSUE 11 complete + ISSUE 12 in-flight (TASK 12.1 shipped)**) · EPIC 3 는 ISSUE 8 (hot-reload, 5 TASKs, v0.4.1–v0.4.5) + ISSUE 9 (real bundle manifests, 3 TASKs, v0.4.6–v0.4.8) + ISSUE 10 (bundle marketplace, 4 TASKs, v0.4.9–v0.4.12) 세 ISSUE 를 landed 후 PR #228 / v0.5.0 close. **EPIC 4 (Multi-tenant business ops / XaaS) 가 2026-04-20 ACTIVE 로 승격, `v1.0.0` (첫 production release) 를 향함**. **ISSUE 11 (quotas + rate limits) complete** (PRs #230/#231/#232/#234 across 0.5.1→0.5.4) — 첫 ISSUE — TASK 11.1 ✅ (PR #230 / 0.5.1) HTTP 429 에 `Retry-After: 60` + `retry_after_seconds: 60` 필드; TASK 11.2 ✅ (PR #231 / 0.5.2) per-tenant `TokenBucketRateLimiter` + `RateLimitedQuotaEnforcer` composite 가 rate check 를 cost 체크보다 먼저 실행 (fail fast); opt-in `[quota_rate_limit] requests_per_minute = N, burst = M` 새 설정 — default `requests_per_minute = 0` = no-op; **TASK 11.3 ✅ (PR #232 / 0.5.3) Postgres-backed daily + monthly spend tracking — 새 `PgQuotaEnforcer` 가 매 요청마다 `quota_configs` 에 UPDATE 한 번으로 `daily_used_cents` + `monthly_used_cents` 증분, CASE-expression rollover 로 UTC midnight 에 daily 를 zero, first-of-month 에 monthly 를 zero (background job 없음 — 첫 post-boundary 요청에서 rollover); 새 migration 이 `usage_day DATE DEFAULT CURRENT_DATE` column 을 `quota_configs` 에 추가; CLI 가 `pg_pool.is_some()` 이면 `PgQuotaEnforcer` 를 선택, 없으면 `InMemoryQuotaEnforcer` 로 fallback; `record_post` 는 fire-and-forget 으로 UPDATE 실패해도 request 는 이미 성공했으므로 tracing::error 만 로그**; TASK 11.4 ✅ (PR #234 / 0.5.4) `GET /api/v1/web/workbench/quota/status` tenant 내부 조회 엔드포인트 — OpenAiCompat scope (사용자가 자기 쿼터를 Management 권한 없이 확인 가능), response `{usage_day, daily: {used_cents, limit_cents, remaining_cents}, monthly: 동일 shape}`, SQL 이 CASE rollover 를 inline 으로 수행해 response 가 boundary 를 이미 반영; `quota_configs` 에 해당 tenant row 가 없으면 schema defaults (1M daily / 10M monthly / usage_day = today) 로 fallback 해서 bootstrap gap 동안 UI 가 "fresh tenant, full quota" 를 렌더; 새 harness gate 7k.5 가 shape + fallback path 를 고정; UI 통합 (dashboard banner, 429 countdown) 은 gadgetron-web follow-up. ISSUE 11 complete. **ISSUE 12 (integer-cent billing) in-flight — TASK 12.1 ✅ (PR #236 / 0.5.5)**: append-only `billing_events` ledger table (integer cents, `event_kind ∈ {chat, tool, action}` CHECK constraint, FK-less `source_event_id` Stripe-style), `PgQuotaEnforcer::record_post` 가 매 chat completion 마다 `billing_events` INSERT 를 `quota_configs` counter UPDATE 옆에 동시 발행 (fire-and-forget — counter-ahead-of-ledger reconciliation 은 TASK 12.4), 새 `GET /api/v1/web/workbench/admin/billing/events` Management-scoped ledger 쿼리 (newest-first, `limit.clamp(1, 500)`, optional `since` lower bound, 핸들러가 tenant boundary pin 해서 cross-tenant read 불가). 남은 ISSUE 12 TASKs: 12.2 (tool/action event kinds), 12.3 (invoice materialization), 12.4 (reconciliation), 12.5 (Stripe ingest). ISSUE 13 (HF catalog) + ISSUE 14 (tenant self-service) 와 함께 **commercialization layer, 2026-04-20 scope direction 에 따라 deferred / opportunistic** — `v1.0.0` cut 의 gate 가 아니며 core product surfaces + ISSUE 11 이 production bar 에 도달하면 출시. Harness 91 PASS, 0 FAIL. EPIC 5 (Cluster platform, PLANNED post-1.0) 이 그 다음 `v2.0.0`. 이전 tag `v0.5.0` (EPIC 3 closure, 2026-04-20), `v0.4.0` (EPIC 2 closure), `v0.3.0` (EPIC 1 closure), `v0.1.0-phase1` (역사) · 버저닝 정책 `docs/process/06-versioning-policy.md`
> **에디션**: 2021
> **라이선스**: MIT
> **최소 Rust**: 1.80
> **바이너리 이름**: `gadgetron`
...
| `gadgetron-cli` | 단일 바이너리 entry point (`gadgetron` 바이너리) + `serve` / `init` / `doctor` / `tenant` / `key` / `mcp serve` 서브커맨드 | [`docs/manual/quickstart.md`](manual/quickstart.md) · [`docs/manual/configuration.md`](manual/configuration.md) |
...
### Phase 2 — Knowledge Layer + Penny + Bundle Surfaces + Multi-tenant XaaS (v0.2 → **v0.5.5**, EPIC 1 + EPIC 2 + EPIC 3 CLOSED; EPIC 4 Multi-tenant business ops / XaaS **ACTIVE** — ISSUE 11 complete + ISSUE 12 in-flight with TASK 12.1 shipped)
...
- **멀티 테넌시 강화 / GPUaaS / ModelaaS / AgentaaS** → EPIC 4 ACTIVE (ISSUE 11 quotas 완료; ISSUE 12 billing + ISSUE 13 HF catalog + ISSUE 14 self-service 는 **commercialization layer, 2026-04-20 scope direction 에 따라 deferred / opportunistic** — `v1.0.0` gate 아님. v1.0.0 = ISSUE 11 + core product surfaces production bar 도달 시 출시; 12/13/14 은 post-1.0 에 선택적 출시).
