...
> **버전**: `0.5.1` (Phase 2 EPIC 1 + EPIC 2 + **EPIC 3 모두 CLOSED**; **EPIC 4 ACTIVE**) · EPIC 3 (Plugin platform) 이 PR #228 에서 close 되어 `v0.5.0` tag 가 찍혔음. EPIC 3 는 ISSUE 8 (hot-reload, 5 TASKs, v0.4.1–v0.4.5) + ISSUE 9 (real bundle manifests, 3 TASKs, v0.4.6–v0.4.8) + ISSUE 10 (bundle marketplace, 4 TASKs, v0.4.9–v0.4.12) 세 ISSUE 를 landed. **EPIC 4 (Multi-tenant business ops / XaaS — integer-cent billing ADR-D-8, HuggingFace catalog, tenant self-service, quotas + SLA enforcement) 가 2026-04-20 ACTIVE 로 승격, `v1.0.0` (첫 production release) 를 향함**. ISSUE 11 (quotas + rate limits) 가 첫 ISSUE — TASK 11.1 ✅ (PR #230 / 0.5.1) 이 HTTP 429 응답에 `Retry-After: 60` 헤더와 body 의 `retry_after_seconds: 60` 필드를 같이 실어 client SDK 가 backoff 를 deterministic 하게 할 수 있게 함; TASK 11.2 (token-bucket enforcer 가 `InMemoryQuotaEnforcer` 를 대체하며 `QuotaToken` 을 통해 실제 refill 시간을 전달) 가 남음. Harness 91 PASS, 0 FAIL. EPIC 5 (Cluster platform, PLANNED post-1.0) 이 그 다음 `v2.0.0`. 이전 tag `v0.5.0` (EPIC 3 closure, 2026-04-20), `v0.4.0` (EPIC 2 closure), `v0.3.0` (EPIC 1 closure), `v0.1.0-phase1` (역사) · 버저닝 정책 `docs/process/06-versioning-policy.md`
> **에디션**: 2021
> **라이선스**: MIT
> **최소 Rust**: 1.80
> **바이너리 이름**: `gadgetron`
