...
> **Status**: Draft v2.1 (2026-04-14) → **IMPLEMENTED on trunk** (v0.2.0 onward). v1 21 blockers + 8 determinism items addressed in v2; v2 mechanical fixes from Round 1.5/2/3 re-review applied in v2.1. All four reviewers: APPROVE WITH MINOR. Round 1.5 security **CLOSED**.
>
> **Shipping landmarks (EPIC 1 — this crate's own surface)**:
> - `/web` chat shell + auth + origin isolation (ISSUE 1 / v0.2.0).
> - `/web/wiki` browser workbench with five-action direct-action gadgets: `knowledge-search` · `wiki-list` · `wiki-read` · `wiki-write` · `wiki-delete` (ISSUE 1–2 / v0.2.0–v0.2.3).
> - Destructive-action approval flow: `pending_approval` → `/approvals/{id}/approve|deny` (ISSUE 3 / v0.2.6 / PR #188).
> - `/web/dashboard` operator observability page with live WebSocket feed from `/events/ws` + `LeftRail` entry (ISSUE 4 / v0.2.7 / PR #194).
>
> **EPIC 4 auth extensions affecting this crate's browser contract**: the doc's "Bearer from localStorage" assumption (§18, §24.6, table at L1454) is **extended — NOT replaced** — by the cookie-session surface. As of v0.5.12, browser clients can reach the same `/v1/*` + `/api/v1/web/workbench/*` surfaces via either the localStorage Bearer path OR the `gadgetron_session` cookie path, with role-derived scope synthesis for cookie auth (admin → `[OpenAiCompat, Management]`; member → `[OpenAiCompat]`). Audit attribution now carries `actor_user_id` + `actor_api_key_id` populated from `TenantContext`. Full per-TASK / per-PR chain: ISSUEs 15 / 16 / 17 / 19 / 20 — see [`ROADMAP.md`](../../ROADMAP.md) §EPIC 4 and [`manual/auth.md`](../../manual/auth.md) §Cookie-session auth. **Web UI login FORM** (React/Tailwind page that POSTs to `/auth/login`) still pending — **ISSUE 18**. pg audit_log consumer → **ISSUE 21**.
>
> The "Ready for TDD after Appendix C nit review" language predates those shipments and is stale — Appendix C nits tracked as follow-ups in subsequent ISSUEs rather than as a pre-TDD gate. — **P2B UX extension: `docs/design/web/expert-knowledge-workbench.md` (Expert Knowledge Workbench UI contract, 2026-04-18)**
> **Author**: PM (Claude)
> **Date**: 2026-04-14 (v2)
