...
> **Status**: Draft v2.1 (2026-04-14) → **IMPLEMENTED on trunk** (v0.2.0 onward). v1 21 blockers + 8 determinism items addressed in v2; v2 mechanical fixes from Round 1.5/2/3 re-review applied in v2.1. All four reviewers: APPROVE WITH MINOR. Round 1.5 security **CLOSED**. **Shipping landmarks**: `/web` chat shell + auth + origin isolation (ISSUE 1 / v0.2.0), `/web/wiki` browser workbench with full five-action direct-action gadgets (`knowledge-search` · `wiki-list` · `wiki-read` · `wiki-write` · `wiki-delete`, ISSUE 1–2 / v0.2.0–v0.2.3), destructive-action approval flow (`pending_approval` → `/approvals/{id}/approve|deny`, ISSUE 3 / v0.2.6 / PR #188), `/web/dashboard` operator observability page with live WebSocket feed from `/events/ws` + `LeftRail` entry (ISSUE 4 / v0.2.7 / PR #194). **EPIC 4 auth extensions affecting this crate's browser contract**: the doc's "Bearer from localStorage" assumption (§18, §24.6, table at L1454) is extended — NOT replaced — by the cookie-session surface. Cookie-session login API shipped via ISSUE 15 TASK 15.1 / PR #248 / v0.5.8 (`POST /api/v1/auth/login` + `/logout` + `GET /whoami`). Unified Bearer-or-cookie `auth_middleware` shipped via ISSUE 16 TASK 16.1 / PR #259 / v0.5.9 — browser clients can now reach the same `/v1/*` + `/api/v1/web/workbench/*` surfaces via either the localStorage Bearer path OR the cookie path, with role-derived scope synthesis for cookie auth (admin → `[OpenAiCompat, Management]`; member → `[OpenAiCompat]`). `ValidatedKey.user_id: Option<Uuid>` plumbing shipped via ISSUE 17 TASK 17.1 / PR #260 / v0.5.10 — affects audit attribution shape; no browser-contract change. `AuditEntry` actor field structural expansion shipped via ISSUE 19 TASK 19.1 / PR #262 / v0.5.11 — struct widens with `actor_user_id` + `actor_api_key_id`; still no browser-contract change. TenantContext → AuditEntry plumbing shipped via ISSUE 20 TASK 20.1 / PR #263 / v0.5.12 — `TenantContext` gains `actor_*` fields populated from `ValidatedKey` by `tenant_context_middleware`; chat handler's 3 `AuditEntry` literals read ctx fields. Cookie-session callers resolve `actor_api_key_id = None` (Uuid::nil() sentinel); Bearer callers get `Some(key_id)`. Still no browser-contract change. **Web UI login FORM** (React/Tailwind page that actually POSTs to `/auth/login`) still pending — split to **ISSUE 18**. pg audit_log consumer → ISSUE 21. The "Ready for TDD after Appendix C nit review" language predates those shipments and is stale — Appendix C nits tracked as follow-ups in subsequent ISSUEs rather than as a pre-TDD gate. — **P2B UX extension: `docs/design/web/expert-knowledge-workbench.md` (Expert Knowledge Workbench UI contract, 2026-04-18)**
> **Author**: PM (Claude)
> **Date**: 2026-04-14 (v2)
