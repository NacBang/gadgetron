...
2. 상세 구현 방안 (위협 시나리오 / inheritance contract / env / admin-tools / approval / audit / STRIDE)
3. 전체 모듈 연결 구도
4. 단위 테스트 계획
5. 통합 테스트 계획
6. Phase 구분
...
### 2.2 설계 원칙 — Penny 는 caller 의 대리자
...
### 2.3 `AuthenticatedContext` 타입-state 설계
...
### 2.4 Penny subprocess env 주입 프로토콜
...
### 2.6 `ADMIN_ONLY_TOOLS` — 컴파일·런타임 이중 차단
...
### 2.7 Approval gate 통합 (ADR-P2A-06 의존)
...
- caller-facing admin 제한은 raw internal reason 대신 remediation 포함 메시지로 렌더
- `AuthenticatedContext::from_penny_env()` 실패는 `AuthInvalid` 계열로 귀결하고 malformed env 값을 echo 하지 않는다
- `tracing` 이벤트:
  - `info`: `penny_spawn_with_inherited_context`, `penny_tool_denied_admin_only`
  - `warn`: `penny_env_missing_required_field`, `penny_attempted_admin_tool`
  - `target = "gadgetron_audit"`: 모든 Penny delegated action, denied action, approval path

### 2.11 의존성

- `gadgetron-core`: 신규 외부 의존성 없음
- `gadgetron-penny`: 기존 `tokio`, `tracing`, `async-trait` 재사용
- tool provider 계층: 추가 의존성 없이 `AuthenticatedContext` 시그니처로 통일
...
web / cli / api caller
        |
        v
gadgetron-gateway -> AuthContext (08)
        |
        v
gadgetron-penny spawn layer
        |
        +--> subprocess env injection
        |
        v
MCP tool registry -> AuthenticatedContext::from_penny_env()
        |
        +--> gadgetron-knowledge ACL (09)
        +--> plugin / gadget providers
        +--> audit_log
```

### 3.2 인터페이스 계약

- 08 문서가 `AuthContext` 원천을 정의하고, 이 문서는 subprocess 경계에서의 inheritance를 고정한다.
- 09 문서는 `AuthenticatedContext`를 받아 ACL을 집행한다.
- approval gate 는 ADR-P2A-06 구현체가 오더라도 inheritance 원칙을 바꾸지 않고 그 위에 얹힌다.

### 3.3 D-12 크레이트 경계 준수

- `AuthenticatedContext`와 관련 enum은 `gadgetron-core`
- subprocess spawn/env wiring 은 `gadgetron-penny`
- 개별 tool authorization은 각 provider 가 하되, 공통 caller identity 타입은 재정의 금지
...
- `from_penny_env_rejects_missing_required_field`
- `from_penny_env_rejects_malformed_uuid`
- `admin_only_tools_are_rejected_for_penny_path`
- `non_admin_cannot_invoke_admin_tool_even_if_tool_registered`
- `service_user_auto_denies_approval_required_flow`
- `kill_on_drop_releases_inherited_context_lock`

### 4.2 테스트 하네스

- env parsing tests 는 isolated process env harness 사용
- approval logic 는 fake registry / fake tool provider 사용
- time-based approval expiry 는 `tokio::time::pause()`로 결정론 보장

### 4.3 커버리지 목표

- inheritance/env parsing branch coverage 90% 이상
- admin-only tool and approval denial paths line coverage 85% 이상
...
- `gadgetron-gateway` + `gadgetron-penny` + fake MCP tool provider
- Penny subprocess spawn -> MCP call -> ACL denial / approval / audit end-to-end
- knowledge tool (`wiki.search`, `wiki.write`)과 server tool (`server.exec` mock) 양쪽 확인

### 5.2 테스트 환경

- subprocess harness with fake Claude shim
- Postgres-backed audit table
- fake approval registry and fake authenticated caller fixtures

### 5.3 회귀 방지

- caller identity 누락 시 tool call이 살아남으면 실패
- admin-only tool이 Penny path에서 성공하면 실패
- cross-session contamination 징후가 있으면 T4 regression이 실패
...
**결론**: Draft v0. inheritance/security 설계 초안.
...
### Round 1 — 2026-04-18 — @gateway-router-lead @inference-engine-lead
**결론**: Conditional Pass
...
**Action Items**:
- A1: workflow 기준 Round 1.5 리뷰어 조합으로 정정
- A2: inheritance contract와 tests를 template 구조로 승격
...
**Action Items**:
- A1: admin-only denial UX와 audit completeness를 본문에 명시
- A2: subprocess env parse failure의 redaction 원칙을 고정
