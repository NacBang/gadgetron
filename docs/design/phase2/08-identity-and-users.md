...
> **담당**: PM (Claude) — Round 1.5 리뷰 예정 (`@xaas-platform-lead`, `@dx-product-lead`)
...
1. 철학 & 컨셉
2. 용어
3. 스키마 — `users` / `teams` / `team_members` / `api_keys` 확장
4. User 유형 및 role
5. Bootstrap admin
6. 인증 플로우 — web UI session vs API key
7. REST API surface
8. CLI 서브커맨드
9. Tenant 단일화 전략 (P2B single-tenant, P2C 확장 슬롯)
10. 마이그레이션 절차 (기존 xaas → multi-user)
11. Phase 분해
12. 오픈 이슈
13. Out of scope
14. 리뷰 로그
...
### 5.1 빈 DB 에서 첫 admin 을 만드는 유일한 경로
...
### 10.1 기존 xaas → multi-user 마이그레이션
...
- [ ] 스키마 마이그레이션 (`20260418_000001_users_teams.sql`)
- [ ] `gadgetron-core::identity::{User, Role, UserId}`, `Team`, `TeamSet`
- [ ] `gadgetron-xaas::UserRepository`, `TeamRepository`, `SessionRepository`
- [ ] `gadgetron-xaas::PasswordHasher` (argon2id)
- [ ] `gadgetron-xaas::AuthContext` + API key → user 해소 로직
- [ ] `gadgetron-gateway::auth_middleware` (web session + API key 통합)
- [ ] `gadgetron-cli::user`, `cli::team`, `cli::auth` 서브커맨드
- [ ] `gadgetron-web`: 로그인 페이지, session 관리, admin 콘솔 (users/teams CRUD)
- [ ] Bootstrap admin 로직 (`gadgetron-cli::main`)
- [ ] Migration 스크립트 (`gadgetron migrate v2b-multiuser`)
- [ ] Manual: `docs/manual/authentication.md`, `docs/manual/admin-operations.md` (신설)
...
## 14. 리뷰 로그 (append-only)

### Round 0 — 2026-04-18 — PM draft
**결론**: Draft v0. 8 open question 포함. Round 1.5 리뷰 진입 준비.
...
**다음 단계**: Round 1.5 (`@xaas-platform-lead`, `@dx-product-lead`) → Q-1~Q-8 답변 → draft v1.

### Round 1.5 — YYYY-MM-DD — @xaas-platform-lead @dx-product-lead
_(pending)_
