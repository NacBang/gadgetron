...
1. 철학 & 컨셉
2. 상세 구현 방안 (용어 / 스키마 / scope / access check / Penny write / seed / search / cache / UI)
3. 전체 모듈 연결 구도
4. 단위 테스트 계획
5. 통합 테스트 계획
6. Phase 구분
...
### 2.3 Scope 타입 및 frontmatter 시맨틱
...
### 2.4 Access check — read / write 함수
...
- caller-facing write/search 거부는 `{error: {message, type, code}}` shape 유지
- 권한 거부는 존재 숨김 원칙을 지키기 위해 기본적으로 `not_found` 또는 `permission_denied` 중 노출이 더 안전한 쪽으로 렌더
- `target = "gadgetron_audit"` 이벤트:
  - `wiki_scope_changed`
  - `wiki_locked_toggled`
  - `wiki_seed_overwrite_blocked`
  - `wiki_admin_private_read`

STRIDE 요약:

- Spoofing: ACL 평가는 항상 `AuthenticatedContext` 기준
- Tampering: `locked` + seed overwrite 보호 + git history
- Repudiation: scope/lock/reset-seed 변경 전부 audit
- Information disclosure: inaccessible 페이지 존재 자체를 숨김
- DoS: pre-filter SQL + team cache 로 검색 비용 상한
- Elevation of privilege: admin/owner/team member 구분을 단일 `can_write` 경로로 집중

### 2.11 의존성

- `gadgetron-core`: `Scope` 관련 타입 추가 외 신규 외부 의존성 없음
- `gadgetron-knowledge`: 기존 `sqlx`, `moka`, `serde_json`, `git2` 재사용
- `gadgetron-xaas`: `TeamCache` invalidate hook 만 추가, 신규 외부 의존성 없음
...
```text
gadgetron-web / gadgetron-cli / penny
            |
            v
gadgetron-gateway -> AuthenticatedContext
            |
            v
gadgetron-knowledge::acl::{can_read, can_write}
            |
            +--> wiki_pages (scope, owner_user_id, locked, source)
            +--> wiki_chunks / hybrid search pre-filter
            +--> gadgetron-xaas::TeamCache
            +--> audit_log + git history
```

### 3.2 인터페이스 계약

- 08 문서가 user/team/session identity를 공급한다.
- 10 문서가 Penny caller inheritance를 고정하고, 이 문서는 그 identity를 소비한다.
- 11 문서의 RAW ingestion 결과물도 동일 ACL 필드를 써야 하므로 별도 bypass 경로를 두지 않는다.

### 3.3 D-12 크레이트 경계 준수

- `Scope` 파싱과 비교 규칙은 `gadgetron-core` 또는 shared knowledge type layer에 둔다.
- SQL pre-filter, wiki write 검증, git/audit fanout은 `gadgetron-knowledge`에 둔다.
- team membership source-of-truth는 `gadgetron-xaas`다.

### 3.4 마이그레이션 — 기존 페이지 → scope 할당
...
- `parse_scope_rejects_unknown_team_prefix`
- `can_read_private_only_owner_or_admin`
- `can_write_team_page_requires_membership_or_admin`
- `locked_page_blocks_team_member_but_not_owner`
- `seed_page_user_edit_sets_source_modified_by`
- `team_cache_invalidate_drops_removed_membership`

### 4.2 테스트 하네스

- ACL 함수는 pure unit test 우선
- SQL pre-filter는 fixed fixture row set + query snapshot
- `moka` TTL 검증은 `tokio::time::pause()` 기반

### 4.3 커버리지 목표

- ACL evaluator branch coverage 90% 이상
- search pre-filter and lock/seed exception paths line coverage 85% 이상
...
- `gadgetron-knowledge` + `gadgetron-xaas` + PostgreSQL
- `wiki.read/write/search/list` MCP tool 전체
- Web UI scope toggle / lock toggle / CLI share/unshare

### 5.2 테스트 환경

- `testcontainers` PostgreSQL + pgvector
- fixture wiki repo with user pages, team pages, org pages, plugin seed pages
- CLI + gateway harness는 동일 seed dataset 사용

### 5.3 회귀 방지

- pre-filter가 제거되면 private page leakage test가 반드시 실패해야 한다
- `locked` guard가 빠지면 team member overwrite test가 실패해야 한다
- `source_modified_by` 보호가 빠지면 seed overwrite regression이 실패해야 한다
...
**결론**: Draft v0. ACL 규칙 초안.

**체크리스트**:
- [x] §3 스키마 (SQL)
- [x] §5 access check (Rust)
- [x] §8 검색 pre-filter (SQL)
- [ ] 템플릿 정합성 — 상위 테스트/모듈 연결 섹션 누락
...
**Action Items**:
- A1: 5대 필수 섹션 형태로 재정렬
- A2: search leakage / locked seed overwrite 회귀 테스트를 명시
...
**Action Items**:
- A1: admin private-read audit와 search result non-disclosure를 문서 본문에 승격
- A2: operator touchpoint를 web/CLI/admin console 기준으로 명시
