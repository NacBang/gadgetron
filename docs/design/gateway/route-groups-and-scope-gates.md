...
| `/api/v1/web/workbench/admin/*` | Bearer | `Scope::Management` | workbench admin plane — catalog hot-reload (`POST /admin/reload-catalog`, TASK 8.2), bundle discovery (`GET /admin/bundles`, TASK 10.1), install + uninstall (`POST + DELETE /admin/bundles/{bundle_id}`, TASK 10.2 with `validate_bundle_id()` path-traversal guard), Ed25519 signature verification on install (TASK 10.4 `signature_hex` request field + `[web.bundle_signing]` trust anchors), billing ledger query (`GET /admin/billing/events`, ISSUE 12 TASK 12.1 / PR #236 — tenant boundary pinned by handler WHERE) | [EPIC 3 complete + EPIC 4 ISSUE 12 in-flight] | **must be matched BEFORE** the broader workbench rule — shipped across PRs #213 / #223 / #224 / #227 (EPIC 3) + #236 (EPIC 4 TASK 12.1) as ISSUE 8–10 + 12 rolled in |
