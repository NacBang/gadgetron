...
> **상태**: Approved
> **작성일**: 2026-04-18
> **최종 업데이트**: 2026-04-20 — **EPIC 3 (Plugin platform) CLOSED `v0.5.0` (PR #228); all 12 TASKs shipped across ISSUE 8 + ISSUE 9 + ISSUE 10.** ISSUE 8: TASK 8.1 / PR #211 `Arc<ArcSwap<DescriptorCatalog>>` 플러밍 substrate, TASK 8.2 / PR #213 `POST /admin/reload-catalog` Management-scoped HTTP endpoint, TASK 8.3 / PR #214 `CatalogSnapshot { catalog, validators }` 번들링 (핸들 `Arc<ArcSwap<CatalogSnapshot>>`), TASK 8.4 / PR #216 file-based TOML source (`[web] catalog_path`), TASK 8.5 / PR #217 POSIX `SIGHUP` reloader (fs-watcher 는 TASK 8.6 로 deferred). ISSUE 9: TASK 9.1 / PR #219 bundle metadata + first-party `bundles/gadgetron-core/bundle.toml` + drift-test, TASK 9.2 / PR #220 multi-bundle aggregation via `[web] bundles_dir` + `from_bundle_dir()` with 중복 id collision error + `bundles: Vec<BundleMetadata>` response, TASK 9.3 / PR #222 bundle-driven harness default (source enum 3-value `{seed_p2b, config_file, bundles_dir}`, Gate 7q.1 retarget + 새 Gate 7q.3, seed_p2b 는 unit-test fixture + drift-guard 로만 존속). ISSUE 10: TASK 10.1 / PR #223 `GET /admin/bundles` bundle-discovery endpoint (read-only, Management-scoped, subdir-sorted ordering, Gates 7q.4 + 7q.5), TASK 10.2 / PR #224 `POST /admin/bundles` install + `DELETE /admin/bundles/{bundle_id}` uninstall (Management-scoped, `validate_bundle_id()` path-traversal guard, composable with reload, 4xx collision = no silent overwrites, Gates 7q.6 + 7q.7 + 7q.8, PASS 86 → 91), TASK 10.3 / PR #226 per-bundle scope isolation (`BundleMetadata.required_scope` inheritance onto descriptors without their own, narrower wins, zero-overhead default, no new gates), TASK 10.4 / PR #227 Ed25519-signed manifests with `[web.bundle_signing]` trust-anchor config + signer-leak-prevention-via-error-envelope-unification (closed EPIC 3).
...
3. **DescriptorCatalog**
   ...
   - 저장: immutable snapshot 을 **`Arc<ArcSwap<CatalogSnapshot>>`** 로 보관 (EPIC 3 / ISSUE 8 — TASK 8.1 / PR #211 에서 `tokio::sync::RwLock<Arc<DescriptorSnapshot>>` → `Arc<ArcSwap<DescriptorCatalog>>` 로 전환, TASK 8.3 / PR #214 에서 `CatalogSnapshot { catalog, validators }` 로 rev — pre-compiled JSON-schema validators 가 catalog 와 함께 스냅샷에 번들되어 atomic swap 한 번으로 둘이 lockstep 으로 교체된다). 읽기 측 (`projection.rs` + `action_service.rs`) 은 매 요청마다 `load()` 로 `Arc<CatalogSnapshot>` 스냅샷을 획득하므로 in-flight 요청은 그 스냅샷 기준으로 마무리 (catalog 와 validator 양쪽 모두 같은 세대를 읽는다 — mismatched pair 관찰 불가), hot-reload 경로는 새 Catalog 를 만들어 `DescriptorCatalog::into_snapshot()` 으로 validator 를 컴파일해 묶은 뒤 `store(new)` 로 포인터를 atomic 하게 교체한다. RwLock 대비 writer 가 in-flight reader 를 block 하지 않는 `arc-swap` 의 hand-off 가 hot-reload 요구사항과 정확히 맞는다. **TASK 8.2–8.5 모두 shipped (ISSUE 8 functionally complete)**: TASK 8.2 / PR #213 — `POST /api/v1/web/workbench/admin/reload-catalog` Management-scoped endpoint 가 이 handle 에 `store()` 를 호출. TASK 8.3 / PR #214 — 위 `CatalogSnapshot` 번들링 으로 TASK 8.2 validator-rebuild 제한 해소. TASK 8.4 / PR #216 — file-based catalog source (`[web] catalog_path` + `DescriptorCatalog::from_toml_file()`) 로 operator 가 on-disk TOML edit 후 reload 가능, 파싱 실패 시 running snapshot 교체 안 됨. TASK 8.5 / PR #217 — POSIX `SIGHUP` trigger (`spawn_sighup_reloader()` Unix-only tokio task) 가 HTTP endpoint 와 동일한 `perform_catalog_reload()` 공유 helper 호출, `kill -HUP <pid>` 로 reload. fs-watcher 는 demand 에 따라 TASK 8.6 follow-up — SIGHUP 이 90% 사용례를 dep/background-thread 없이 커버하므로 우선 미구현. **ISSUE 9 functionally complete**: TASK 9.1 / PR #219 — `BundleMetadata { id, version }` 구조체가 `DescriptorCatalog` 에 optional `[bundle]` TOML 테이블로 연결되어 operator 가 loaded catalog 의 identity 를 reload response 의 `bundle: Option<BundleMetadata>` 필드로 확인 가능 (None 시 `skip_serializing_if` 로 JSON 에서 생략, backwards-compatible); first-party bundle file `bundles/gadgetron-core/bundle.toml` 이 `seed_p2b()` 을 TOML 로 미러링하며 drift-test 가 action id 집합 동일성 보장. TASK 9.2 / PR #220 — multi-bundle aggregation via `[web] bundles_dir` + `DescriptorCatalog::from_bundle_dir()`: subdirectory 별 `<name>/bundle.toml` 을 알파벳 path 순으로 merge, `bundle.toml` 이 없는 subdir 은 silently skip, 중복 action/view id 는 hard `Config` error 로 reject 하면서 running snapshot 은 교체되지 않음 (TASK 8.4 parse-failure 와 동일한 보장), `allow_direct_actions` 는 bundle 간 OR-fold, response 가 `bundles: Vec<BundleMetadata>` 로 확장 (`skip_serializing_if = "Vec::is_empty"`), source-precedence `bundles_dir` > `catalog_path` > `seed_p2b`. TASK 9.3 / PR #222 — bundle-driven harness default: E2E harness fixture 가 `[web] bundles_dir = <repo>/bundles` 로 설정되어 `from_bundle_dir` 경로가 production 기본 source 로 exercised, `source` enum 이 `"bundles_dir"` 값 추가로 3 값 `{"seed_p2b", "config_file", "bundles_dir"}`, Gate 7q.1 이 retarget + 새 Gate 7q.3 이 `.bundles[0].id == "gadgetron-core"` 를 pin, `seed_p2b()` 는 unit-test fixture 와 drift-guard reference 로만 존속 (production 기본 source 는 아님). **ISSUE 10 (bundle marketplace) in-flight** — TASK 10.1 shipped / PR #223: `GET /api/v1/web/workbench/admin/bundles` Management-scoped read-only enumeration endpoint. Scans each `<bundles_dir>/<subdir>/bundle.toml`, returns `ListBundlesResponse { bundles_dir, count, bundles: [BundleDiscoveryEntry { bundle, source_path, action_count, view_count }, ...] }` in subdir-sorted order matching `from_bundle_dir` so tooling can rely on deterministic ordering; live `ArcSwap<CatalogSnapshot>` 핸들은 건드리지 않으므로 in-flight 요청 중에도 polling safe; `bundles_dir` 가 wire 되지 않은 deployment 는 503 `Config`; Gate 7q.4 (shape + enumeration) + Gate 7q.5 (RBAC 403). TASK 10.2 shipped / PR #224: install + uninstall runtime — `POST /admin/bundles` 이 `{bundle_toml: "<text>"}` body 를 받아 `validate_bundle_id()` 로 `^[a-zA-Z0-9_-]{1,64}$` regex 를 disk write 이전에 검증한 뒤 `{bundles_dir}/{id}/bundle.toml` 에 verbatim 기록, response `{installed, bundle_id, manifest_path, reload_hint}`; `DELETE /admin/bundles/{bundle_id}` 가 같은 id guard 로 `std::fs::remove_dir_all`; 둘 다 live catalog 는 swap 하지 않아 operator 가 `reload_hint` 대로 별도 reload activate (snapshot swap 순간이 명시적으로 유지됨 — accidental install 이 reload 이전에 동작을 바꾸지 못함); 기존 id collision 은 4xx `Config` 로 reject (no silent overwrites, `from_bundle_dir` 중복-id hard-error 와 동일 semantics); Gates 7q.6/7q.7/7q.8, PASS 86 → 91. TASK 10.3 shipped / PR #226: per-bundle scope isolation — `BundleMetadata.required_scope: Option<Scope>` 가 `[bundle]` level 에 선언되면 `from_toml_file` post-parse pass 가 view + action 을 walk 해서 자기 scope 가 없는 descriptor 에 bundle scope 를 inherit, narrower explicit scope 는 유지 (narrower wins), bundle 이 scope 를 선언하지 않으면 zero-overhead default (inheritance pass 생략), effective scope 가 descriptor 에 land 되어 downstream audit/log 가 bundle 을 re-read 하지 않고도 introspect 가능; `from_bundle_dir` 도 per-manifest `from_toml_file` 호출 경로를 통해 동일 behavior; scope inheritance 는 action-visibility 시점 filter 이므로 기존 reload + discovery gate 들이 그대로 hold — new harness gate 없음; PASS 그대로 91. **TASK 10.4 (Ed25519-signed manifests — install 시 서명 검증)** 이 EPIC 3 close 전 남은 유일한 일 — TASK 10.2 가 HTTP 로 disk 에 arbitrary TOML 을 쓸 수 있게 하면서 열린 supply-chain attack surface 를 닫는 것이 목적.
4. **ActivityProjector**
