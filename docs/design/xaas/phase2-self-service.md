...
- **ISSUE 25** (type-confusion 하드닝 follow-up, not v1.0.0 gate): ISSUE 24 가 `AuthenticatedContext` 에 `real_user_id` 를 추가했지만 legacy `user_id` 필드를 그대로 둠 (api_key_id placeholder; rustdoc "DO NOT READ" 경고). 14 reader call sites 이 여전히 legacy 를 읽으며 type-confusion landmine 이 남음. ISSUE 25 는 rename `user_id` → `api_key_id` + `real_user_id` → `user_id` + `action_service.rs` audit sink 의 `ActionAuditEvent.actor_user_id` contamination fix + `billing_insert_failures_total{reason}` counter 를 한 PR 로 묶음.
