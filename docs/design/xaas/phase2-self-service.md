...
- **ISSUE 24** (billing-attribution follow-up, not v1.0.0 gate): thread real `user_id` through `QuotaToken` + `AuthenticatedContext` so chat + action `billing_events` rows populate `actor_user_id`. ISSUE 23 landed the column + tool emitter; chat + action paths currently write NULL because neither source carries the real user_id. After ISSUE 24, Gate 7k.6b assertions flip from `tool ≥ 1 non-NULL` to `chat + tool + action ≥ 1 non-NULL`.
