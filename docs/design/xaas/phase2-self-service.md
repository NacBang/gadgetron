...
> **상태**: ISSUE 14 ✅ CLOSED (PR #246 / v0.5.7). ISSUE 15 ✅ CLOSED (PR #248 / v0.5.8). ISSUE 16 ✅ CLOSED (PR #259 / v0.5.9). ISSUE 17 ✅ CLOSED (PR #260 / v0.5.10). ISSUE 19 ✅ CLOSED (PR #262 / v0.5.11). ISSUE 20 ✅ CLOSED (PR #263 / v0.5.12). ISSUE 21 TASK 21.1 (pg audit_log consumer) ✅ CLOSED (PR #267 / v0.5.13). ISSUE 22 TASK 22.1 (admin `/audit/log` query endpoint) ✅ CLOSED (PR #269 / v0.5.14). ISSUE 23 (`billing_events.actor_user_id` column + tool/action emitters populate) ✅ CLOSED (PR #271 / v0.5.15). ISSUE 24 (`QuotaToken.user_id` + `AuthenticatedContext.real_user_id` threading — chat + action `billing_events` now also populate) ✅ CLOSED (PR #289 / v0.5.16). ISSUE 25 (`AuthenticatedContext.user_id` → `api_key_id` rename + audit_log contamination fix — action audit sinks emit `actor.real_user_id.unwrap_or(actor.api_key_id)`) ✅ CLOSED (PR #293 / v0.5.17). ISSUE 26 (billing-insert SLO counter + `/admin/billing/insert-failures` endpoint) ✅ CLOSED (PR #299 / v0.5.18). ISSUE 27 (`/metrics` Prometheus scrape surface exposing `gadgetron_billing_insert_failures_total`) ✅ CLOSED (PR #301 / v0.5.19). 남은 스코프: **ISSUE 18** web UI 로그인 폼 + ISSUE 28 `/web` pre-auth 랜딩 폴리시 (operator bug report) — 둘 다 비-v1.0.0-gate 로 ISSUE 18 만 multi-user gate.
> **작성일**: 2026-04-19
...
**CLOSED**: ISSUE 14 (PR #246 / v0.5.7) + ISSUE 15 (PR #248 / v0.5.8) + ISSUE 16 TASK 16.1 단일 middleware (PR #259 / v0.5.9) + ISSUE 17 TASK 17.1 `ValidatedKey.user_id` plumbing (PR #260 / v0.5.10) + ISSUE 19 TASK 19.1 `AuditEntry` actor fields structural (PR #262 / v0.5.11) + ISSUE 20 TASK 20.1 TenantContext → AuditEntry plumbing (PR #263 / v0.5.12) + ISSUE 21 TASK 21.1 pg `audit_log` consumer (PR #267 / v0.5.13, Gate 7v.7 +2) + ISSUE 22 TASK 22.1 admin `/audit/log` query endpoint (PR #269 / v0.5.14, Gate 7v.8 +2) + ISSUE 23 `billing_events.actor_user_id` + tenant-first composite index (PR #271 / v0.5.15, Gate 7k.6b +3 + Gate 13 regex-fix +1) + ISSUE 24 `QuotaToken.user_id` + `AuthenticatedContext.real_user_id` threading (PR #289 / v0.5.16, Gate 3.5 precondition + Gate 7k.6b-identity +2) + ISSUE 25 `AuthenticatedContext.user_id` → `api_key_id` rename + audit_log contamination fix (PR #293 / v0.5.17, Gate 7v.7 `audit_log.actor_user_id populated` +1) + ISSUE 26 billing-insert SLO counter + `/admin/billing/insert-failures` (PR #299 / v0.5.18, Gate 7k.8 + 7k.9 +2) + ISSUE 27 `/metrics` Prometheus scrape surface (PR #301 / v0.5.19, Gate 7p +2). Harness progression: 126 → 129 (PR #259 ISSUE 16 Gate 7v.6) → 131 (PR #267 ISSUE 21 Gate 7v.7) → 133 (PR #269 ISSUE 22 Gate 7v.8) → 137 (PR #271 ISSUE 23 Gate 7k.6b + Gate 13 regex-fix) → 139 (PR #289 ISSUE 24) → 140 (PR #293 ISSUE 25) → 142 (PR #299 ISSUE 26) → 144 (PR #301 ISSUE 27). ISSUE 17 + 19 + 20 은 behavior-preserving 변경으로 새 gate 없이 기존 가드에 의존.
