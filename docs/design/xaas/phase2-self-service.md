...
> **상태**: ISSUE 14 ✅ CLOSED (PR #246 / v0.5.7). ISSUE 15 ✅ CLOSED (PR #248 / v0.5.8). ISSUE 16 ✅ CLOSED (PR #259 / v0.5.9). ISSUE 17 ✅ CLOSED (PR #260 / v0.5.10). ISSUE 19 ✅ CLOSED (PR #262 / v0.5.11). ISSUE 20 ✅ CLOSED (PR #263 / v0.5.12). ISSUE 21 TASK 21.1 (pg audit_log consumer) ✅ CLOSED (PR #267 / v0.5.13). ISSUE 22 TASK 22.1 (admin `/audit/log` query endpoint) ✅ CLOSED (PR #269 / v0.5.14). ISSUE 23 (`billing_events.actor_user_id` column + tool/action emitters populate) ✅ CLOSED (PR #271 / v0.5.15). ISSUE 24 (`QuotaToken.user_id` + `AuthenticatedContext.real_user_id` threading — chat + action `billing_events` now also populate) ✅ CLOSED (PR #289 / v0.5.16). ISSUE 25 (`AuthenticatedContext.user_id` → `api_key_id` rename + audit_log contamination fix — action audit sinks emit `actor.real_user_id.unwrap_or(actor.api_key_id)`) ✅ CLOSED (PR #293 / v0.5.17). ISSUE 26 (billing-insert SLO counter + `/admin/billing/insert-failures` endpoint) ✅ CLOSED (PR #299 / v0.5.18). 남은 스코프: **ISSUE 18** web UI 로그인 폼 + ISSUE 28 `/web` pre-auth 랜딩 폴리시 (operator bug report); ISSUE 27 (`real_user_id` → `user_id` rename) 은 비-v1.0.0-gate DX follow-up.
> **작성일**: 2026-04-19
