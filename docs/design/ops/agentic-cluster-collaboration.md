...
> **작성일**: 2026-04-15
> **최종 업데이트**: 2026-04-15
> **관련 크레이트**: `gadgetron-core`, `gadgetron-gateway`, `gadgetron-penny`, `gadgetron-knowledge`, `gadgetron-xaas`, `gadgetron-scheduler`, `gadgetron-node`
> **Phase**: [P2] / [P3]
...
>
> **Canonical terminology note**: this draft talks about operational "tool providers", but the current canonical runtime seam is `GadgetProvider` / `GadgetRegistry`. Read legacy `McpToolProvider` references below as `GadgetProvider`.
...
| **B. Personal assistant 제품 정의 강화** | Penny 중심 소비자형 비서로 설명 | 클러스터 운영, 워크로드 실행, 관리자 승인 체계를 하위 기능으로 축소해 버린다 |
...
3. **Tool-first extensibility**: 새로운 운영 능력은 ad-hoc 코드가 아니라 `GadgetProvider` 기반 category/gadget 으로 추가한다.
...
POST /v1/chat/completions
  - 기존 계약 유지
  - model = "penny" 일 때 CollaborationContext 를 내부 생성
...
#### 2.1.3 도구 category 확장 (`GadgetProvider`)

기존 `GadgetProvider` trait 을 기반으로 새로운 plane 을 category 확장으로 수용한다.
...
- `gadgetron-penny`: intent classification, tool orchestration, experience draft creation
...
- `gadgetron-penny`: fake tool providers + fake approval registry
...
| 항목 | Phase |
|------|-------|
| Assistant Plane as `model = "penny"` entry | [P2A] |
