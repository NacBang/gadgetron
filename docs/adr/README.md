...
| [P2A-06](ADR-P2A-06-approval-flow-deferred-to-p2b.md) | 2026-04-14 | ACCEPTED | 대화형 승인 흐름 P2B 연기 | `04 v1` 리뷰 4회차 24 blockers 발생 → P2A 는 scaffold 만 구현, UI 승인 카드 + cross-process approval 브리지는 P2B 이관 (ADR-P2A-05 §(d)(e) amend). 현재는 [ROADMAP.md](../ROADMAP.md) ISSUE 3 (production safety, v0.2.6 NEXT) 가 `ApprovalStore` 슬라이스만 가져가며, 나머지 §Phase 2B 항목들은 이후 ROADMAP ISSUE 들로 분할 예정 — ADR 상단 Reframing note 참조. |
