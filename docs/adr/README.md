...
| [P2A-06](ADR-P2A-06-approval-flow-deferred-to-p2b.md) | 2026-04-14 | ACCEPTED | 대화형 승인 흐름 P2B 연기 | `04 v1` 리뷰 4회차 24 blockers 발생 → P2A 는 scaffold 만 구현, UI 승인 카드 + cross-process approval 브리지는 P2B 이관 (ADR-P2A-05 §(d)(e) amend). [ROADMAP.md](../ROADMAP.md) **ISSUE 3 (production safety, v0.2.6, PR #188) 가 `ApprovalStore` + approve/deny + `ActionAuditSink` + /audit/events 를 shipped**, **ISSUE 4 (operator observability, v0.2.7, PR #194) 가 /usage/summary + /events/ws + 실 `cost_cents`** 를 이어 shipped. 나머지 §Phase 2B 항목 (SEC-MCP-B1 cross-process bridge, SSE approval event, args_digest, `<ApprovalCard>` UI 등) 은 이후 ROADMAP ISSUE 들로 분할 예정 — ADR 상단 Reframing + Status update note 참조. |
