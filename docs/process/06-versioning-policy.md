...
- **EPIC 4 progress** (patch-level bumps per ISSUE within 0.5.x):
  ...
  - **ISSUE 24** thread real `user_id` through `QuotaToken` + `AuthenticatedContext` ⏳ planned — closes the chat + action NULL `actor_user_id` from ISSUE 23. Not a `v1.0.0` gate; no version bump scheduled. Full sketch in [`ROADMAP.md §EPIC 4 ISSUE 24`](../ROADMAP.md).
