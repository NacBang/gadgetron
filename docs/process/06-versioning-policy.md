...
- **Workspace version**: `0.2.11` (EPIC 1 close-criteria + EPIC 2 두 ISSUE shipped + ISSUE 7 in-flight: TASK 7.1 + 7.2 shipped)
- **활성 EPIC**: EPIC 1 (Workbench MVP) close-criteria 단계 + EPIC 2 (Agent autonomy) 진행 중. EPIC 1 shipped — ISSUE 1 (0.2.0), ISSUE 2 (0.2.1→0.2.4), ISSUE 2b (0.2.5), ISSUE 3 (0.2.6, PR #188 — production safety), ISSUE 4 (0.2.7, PR #194 — operator observability). EPIC 2 shipped — ISSUE 5 (0.2.8, PR #199 — Penny tool-call audit surface), ISSUE 6 (0.2.9, PR #201 — Penny-attributed activity feed). EPIC 2 in-flight — **ISSUE 7 (first-class MCP server)**: TASK 7.1 shipped (0.2.10, PR #204 — `GET /v1/tools` discovery via `GadgetCatalog` trait, harness gate 7i.2); **TASK 7.2 shipped (0.2.11, PR #205 — `POST /v1/tools/{name}/invoke` dispatches through the existing `Arc<dyn GadgetDispatcher>` seam so the L3 allowed-names gate runs on this path too)**; TASK 7.3 (cross-session audit: external-agent tool invocation 을 `tool_audit_events` 에 `external_caller_id` 로 상관 연결) 남아 있음.
- **다음 patch bump**: ISSUE 7 의 TASK 7.3 머지 시 `0.2.11` → `0.2.12` (TASK 단위로 patch bump 이 붙는 패턴 — PR #204 가 첫 사례, PR #205 가 두 번째). EPIC 1 close 는 별도의 manual-validation 단계 통과 후 `0.3.0` minor bump.
