...
- 서비스 실행 경로(데모/운영/로컬 제공 방식)를 추가·변경하는 문서는 **완전 기동 스크립트와 운영 루프**까지 함께 설계해야 한다. 최소 `build/start/stop/status/logs` 또는 동등한 제공 수단의 소유자가 문서에 명시되어야 한다.
...
- 문서가 약속한 실행 스크립트/운영 절차로 실제 서비스가 끝까지 올라오는지 확인
