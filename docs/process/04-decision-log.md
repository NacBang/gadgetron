...
`ADMIN_ONLY_TOOLS` const 에 admin 전용 tool (`user.create`, `team.create`, `bundle.enable`, `config.set`, `key.issue`, `audit.export`, `user.impersonate` 등) 선언. Bundle 이 이들을 `register_tool` 하려 하면 **panic**. Penny 가 자기 권한을 올리는 경로 영구 차단.
...
## D-20260418-25: Drift-fix PR 4 — coordinator gate cleanup + U-D re-scoping

**날짜**: 2026-04-18
**유형**: Follow-up fix (drift-fix PR 5 머지 직후, 5-min cron iteration 3)
**상태**: 🟢 승인 (codex cycle-15 sequencing note — "PR 5 → PR 4 → PR 2")
**관련 문서**: D-20260418-23 (drift-fix PR 1 — U-D initial flag), `docs/design/gateway/workbench-projection-and-actions.md` §2.2.6
**Follows**: D-20260418-24 (drift-fix PR 5)

### 배경

Cycle-14 drift audit 에서 flag 된 두 항목:

1. **N3**: `InProcessCandidateCoordinator::with_confirmation_gate` 가 no-op stub. 실제 confirmation gate 정책은 store 측에서 구현, coordinator 메서드는 호출되지 않음.
2. **U-D**: `DescriptorCatalog.allow_direct_actions` 가 Scope 에 bound 안됨.

Pre-implementation 재검토 결과:

- **N3 는 실제 drift** — coordinator method 가 no-op + 어디서도 호출 안됨. 제거.
- **U-D 는 misinterpretation** — doc 74 §2.3 (line 481-483) 가 명시적으로 두 gate 를 구분한다:
  - `required_scope` = per-descriptor actor-scope filter
  - `allow_direct_actions` = instance-level global policy
  - 두 gate 는 orthogonal (line 148: "`disabled_reason` 은 actor privilege 문제가 아니라 instance config/runtime availability 때문이다")
  - 현재 구현은 이미 두 gate 를 모두 정확히 적용. spec-correct.

### 결정: N3 cleanup + U-D close-out

1. **N3 fix** — `InProcessCandidateCoordinator::with_confirmation_gate` dead code 삭제
2. **U-D close-out** — `DescriptorCatalog` docstring + doc 74 §2.2.6 에 orthogonal dual-gate 구분 명시
3. 감사 리포트 (`docs/reviews/document-consistency-sweep-2026-04-18.md`) 에 U-D 재분류 기록

### Non-scope

- store-level `with_confirmation_gate` 은 그대로 (작동 중)
- `allow_direct_actions` 의 Scope 연동 (spec 과 일치하므로 변경 불필요)

### 영향 받는 크레이트

- `gadgetron-knowledge`: coordinator no-op 메서드 삭제 (~15 LOC)
- `gadgetron-gateway`: `DescriptorCatalog` docstring 보강 (~20 LOC)
- 문서: doc 74 §2.2.6 orthogonal-gate 구분 (~30 LOC)

**Total 예상**: ~75 LOC (대부분 문서)

### 시행 순서

1. 본 커밋: D-entry land
2. Feature branch `fix/drift-pr4-gate-cleanup` (생성됨)
3. 직접 구현
4. cargo fmt/clippy/test full workspace
5. PR + CI + admin merge
