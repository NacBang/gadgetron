...
- **Penny 없이 위키 직접 편집** — [web.md §/web/wiki](web.md#web-wiki--브라우저-워크벤치-wiki-crud) 의 `/web/wiki` 워크벤치. Penny/LLM 라운드트립 없이 네 direct-action (`knowledge-search` · `wiki-list` · `wiki-read` · `wiki-write`) 으로 바로 CRUD — 빠르고 결정적이지만 자연어·추론 없음.
