...
2. **Gadgetron 서버 설정 파일이 준비되어 있어야 합니다.** Web UI 자체는 `/web`를 서빙하고, `kairos` 모델을 쓰려면 `gadgetron.toml`에 `[knowledge]` 섹션이 있어야 합니다. 자세한 설정은 [kairos.md](kairos.md)를 참조하십시오.
...
출력된 `gad_live_...` 키를 복사하십시오. PostgreSQL-backed 운영 환경이면 대신 tenant를 만든 뒤 `key create --tenant-id <uuid>`를 사용하십시오.
...
- [ ] `./target/release/gadgetron serve --config gadgetron.toml` → `http://localhost:8080/web` 접속 → 채팅 UI 가 로드됨
...
- 응답이 비어 있으면 `gadgetron.toml` 에 일반 프로바이더 블록이 없고, `kairos`를 원한다면 `[knowledge]` 섹션도 빠져 있을 가능성이 큽니다. `[knowledge]` 또는 `[providers.*]`를 추가하십시오.
...
- `docs/manual/kairos.md` — Kairos 협업 에이전트 런타임 설정·사용
