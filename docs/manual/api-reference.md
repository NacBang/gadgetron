...
| `quota_exceeded` | 429 | `quota_error` | Tenant's daily spending limit reached. **Every HTTP 429 response carries both the `Retry-After: 60` HTTP header AND a `retry_after_seconds: 60` field inside the error JSON body** (ISSUE 11 TASK 11.1 / v0.5.1 / PR #230) — SDK clients that honor either surface back off deterministically instead of retrying in a tight loop. `retry_after_seconds` is `Null`-absent from non-429 responses (403, 401, 404 keep the base body shape since `Retry-After` has specific HTTP semantics). `QUOTA_RETRY_AFTER_SECONDS = 60` is a conservative constant today; TASK 11.2 (token-bucket enforcer replacing `InMemoryQuotaEnforcer` with per-tenant buckets) will thread the real refill time through `QuotaToken` so the header reports the exact countdown. |
...
| Field | Type | Notes |
|-------|------|-------|
| `gateway_version` | non-empty string | Cargo workspace version, e.g. `"0.5.2"`. |
...
```json
{
  ...
  "bundles": [
    {"id": "gadgetron-core", "version": "0.5.2"},
...
```json
{
  ...
  "bundles": [
    {
      "bundle": {"id": "gadgetron-core", "version": "0.5.2"},
