...
This manual is the **single source of truth for what trunk actually ships today** (workspace version `0.4.12` — EPIC 1 Workbench MVP closed `v0.3.0` PR #208, EPIC 2 Agent autonomy closed `v0.4.0` PR #209, EPIC 3 Plugin platform ACTIVE with all three ISSUEs functionally complete: ISSUE 8 shipped TASK 8.1–8.5, ISSUE 9 shipped TASK 9.1–9.3, ISSUE 10 shipped TASK 10.1–10.4. ISSUE 10 TASK 10.4 (PR #227) adds Ed25519-signed bundle manifests — `[web.bundle_signing]` config block declares trusted publisher pubkeys + optional `require_signature`; `POST /admin/bundles` install accepts a `signature_hex` field and runs `verify_bundle_signature` BEFORE TOML parse so signature errors are indistinguishable from signed-malformed parse errors (no signer leak via error text); policy matrix covers unsigned-allowed / unsigned-required / valid-sig / tampered-sig / unknown-key / no-trust-anchors branches; default config preserves TASK 10.2 unsigned behavior. EPIC 3 close → `v0.5.0` is the natural next step). If a behaviour is described here, it exists in the binary you can `cargo build` from `main`.
