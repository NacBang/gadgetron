...
This manual is the **single source of truth for what trunk actually ships today** (workspace version `0.4.11` — EPIC 1 Workbench MVP closed `v0.3.0` PR #208, EPIC 2 Agent autonomy closed `v0.4.0` PR #209, EPIC 3 Plugin platform ACTIVE with ISSUE 8 and ISSUE 9 both functionally complete; ISSUE 10 in-flight with TASK 10.1 + 10.2 + 10.3 shipped: TASK 10.1 (PR #223) added `GET /admin/bundles` read-only enumeration, TASK 10.2 (PR #224) added `POST /admin/bundles` + `DELETE /admin/bundles/{bundle_id}` for runtime install/uninstall with path-traversal-safe id regex + no-silent-overwrite collision policy, TASK 10.3 (PR #226) added bundle-level `required_scope` inheritance — `BundleMetadata.required_scope: Option<Scope>` declared once at the `[bundle]` level floors every view/action the bundle ships, so actors without the scope see NONE of the bundle's descriptors (narrower explicit scopes on individual descriptors still win). TASK 10.4 (Ed25519 signed manifests) pending before EPIC 3 close). If a behaviour is described here, it exists in the binary you can `cargo build` from `main`.
