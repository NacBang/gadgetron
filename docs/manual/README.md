...
This manual is the **single source of truth for what trunk actually ships today** (workspace version `0.5.18` — three complete EPICs + ISSUE 11 + **ISSUE 12 closed at telemetry scope** + **ISSUE 14 (tenant self-service, PR #246 / 0.5.7)** + **ISSUE 15 TASK 15.1 (cookie-session login API, PR #248 / 0.5.8)** + **ISSUE 16 TASK 16.1 (unified Bearer-or-cookie auth middleware, PR #259 / 0.5.9)** + **ISSUE 17 TASK 17.1 (`ValidatedKey.user_id` plumbing, PR #260 / 0.5.10)** + **ISSUE 19 TASK 19.1 (`AuditEntry` actor fields structural, PR #262 / 0.5.11)** + **ISSUE 20 TASK 20.1 (`TenantContext` → `AuditEntry` plumbing, PR #263 / 0.5.12)** + **ISSUE 21 TASK 21.1 (pg `audit_log` consumer, PR #267 / 0.5.13)** + **ISSUE 22 TASK 22.1 (admin `GET /audit/log` query endpoint, PR #269 / 0.5.14)** + **ISSUE 23 (billing_events.actor_user_id column + tool/action emitters populate it, PR #271 / 0.5.15)** + **ISSUE 24 (`QuotaToken.user_id` + `AuthenticatedContext.real_user_id` threading so chat + action billing rows also populate `actor_user_id`, PR #289 / 0.5.16)** + **ISSUE 25 (`AuthenticatedContext.user_id` → `api_key_id` rename + audit_log contamination fix — action audit sinks emit `actor.real_user_id.unwrap_or(actor.api_key_id)`, PR #293 / 0.5.17)** + **ISSUE 26 (billing-insert SLO counter + `GET /admin/billing/insert-failures` endpoint, PR #299 / 0.5.18)** — see [auth.md §Cookie-session auth](auth.md).
...
- **ISSUE 27** finish the `AuthenticatedContext.real_user_id` → `user_id` rename ⏳ planned — lets call sites read `actor.user_id.unwrap_or(actor.api_key_id)` as the obvious fallback. Pure DX; no behavior change. Not a `v1.0.0` gate.
