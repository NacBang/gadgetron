...
- **ISSUE 26** billing-insert SLO counter + alert ⏳ planned — emit `billing_insert_failures_total{reason}` from `insert_billing_event` failure paths (today `Err` fires only to `tracing::warn!(target: "billing")`). Alert rule: `increase(billing_insert_failures_total[5m]) > 0`. Not a `v1.0.0` gate.
