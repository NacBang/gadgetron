...
This manual is the **single source of truth for what trunk actually ships today** (workspace version `0.5.8` — three complete EPICs + ISSUE 11 complete + **ISSUE 12 closed at telemetry scope** (TASKs 12.1 + 12.2) + **ISSUE 14 (tenant self-service) closed (PR #246 / 0.5.7)** + **ISSUE 15 TASK 15.1 (cookie-session login API) shipped (PR #248 / 0.5.8)**: EPIC 1 Workbench MVP closed `v0.3.0` PR #208, EPIC 2 Agent autonomy closed `v0.4.0` PR #209, **EPIC 3 Plugin platform CLOSED `v0.5.0` PR #228**. **EPIC 4 (Multi-tenant business ops / XaaS) now ACTIVE** (promoted 2026-04-20). ISSUE 11 (quotas + rate limits) complete across 0.5.1–0.5.4 (PRs #230/#231/#232/#234): structured 429 + Retry-After, `[quota_rate_limit]` token-bucket, `PgQuotaEnforcer` Postgres-backed spend tracking + CASE rollover, `GET /api/v1/web/workbench/quota/status` OpenAiCompat tenant introspection. ISSUE 12 closed at telemetry scope (PR #241 / 0.5.6): TASK 12.1 `billing_events` ledger + chat INSERT + `GET /api/v1/web/workbench/admin/billing/events` (PR #236 / 0.5.5); TASK 12.2 tool + action emission with `source_event_id = audit_event_id` joins (PR #241 / 0.5.6). ISSUE 14 closed (PR #246 / 0.5.7): TASKs 14.1–14.5 + 14.7 — users / teams / team_members / user_sessions schema, argon2id first-admin bootstrap, admin user CRUD + single-admin guard, user self-service key CRUD + scope narrowing, team CRUD + cross-tenant user rejection, `gadgetron user` + `gadgetron team` CLI. ISSUE 15 TASK 15.1 shipped (PR #248 / 0.5.8): cookie-session login API — `POST /api/v1/auth/login` + `POST /api/v1/auth/logout` + `GET /api/v1/auth/whoami` (argon2id PHC password verify, SHA-256 hashed cookies in `user_sessions`, service-role + inactive-user blocked; see [auth.md §Cookie-session auth](auth.md)). TASKs 12.3 (invoice materialization), 12.4 (reconciliation), 12.5 (Stripe ingest) + ISSUE 13 (HF catalog monetization) are **commercialization layer** per 2026-04-20 scope direction — they do NOT gate `v1.0.0`. ISSUE 14 was on that list at 2026-04-20 but has since shipped; it's multi-user-foundation infrastructure, not commercialization. **Remaining multi-user work (ISSUE 16)**: web UI login form + unified Bearer-or-cookie middleware + Google OAuth social login. Full EPIC 4 quota pipeline: rate-limit check → pg cost check → dispatch → pg record_post (quota counter + billing_events ledger) → tenant introspection via /quota/status → rejections surface as structured 429. EPIC 4 "close for 1.0" = ISSUE 11 + ISSUE 14 + ISSUE 15 + ISSUE 16 + core product surfaces meeting production bar; 12.3 / 12.4 / 12.5 / 13 ship post-1.0 as patch / minor bumps once market pull justifies. If a behaviour is described here, it exists in the binary you can `cargo build` from `main`.
