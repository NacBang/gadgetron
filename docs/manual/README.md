...
This manual is the **single source of truth for what trunk actually ships today** (workspace version `0.5.8` — three complete EPICs + ISSUE 11 + **ISSUE 12 closed at telemetry scope** + **ISSUE 14 (tenant self-service, PR #246 / 0.5.7)** + **ISSUE 15 TASK 15.1 (cookie-session login API, PR #248 / 0.5.8)** — see [auth.md §Cookie-session auth](auth.md).
