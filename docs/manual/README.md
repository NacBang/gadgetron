...
This manual is the **single source of truth for what trunk actually ships today** (workspace version `0.4.8` — EPIC 1 Workbench MVP closed `v0.3.0` PR #208, EPIC 2 Agent autonomy closed `v0.4.0` PR #209, EPIC 3 Plugin platform ACTIVE with ISSUE 8 and ISSUE 9 both functionally complete: ISSUE 8 shipped TASK 8.1–8.5 (PRs #211 / #213 / #214 / #216 / #217) giving operators a three-trigger catalog-reload surface (HTTP endpoint, SIGHUP signal, shared `perform_catalog_reload()` helper; fs-watcher deferred to TASK 8.6); ISSUE 9 shipped TASK 9.1–9.3 (PRs #219 / #220 / #222) adding `BundleMetadata`, multi-bundle aggregation via `[web] bundles_dir` + `DescriptorCatalog::from_bundle_dir()`, and the bundle-driven default — the E2E harness now boots against the in-tree `bundles/` directory so every harness run exercises the production code path operators will use. `source` enum widened to `{"seed_p2b", "config_file", "bundles_dir"}`. ISSUE 10 (bundle marketplace) is the only work left before EPIC 3 close). If a behaviour is described here, it exists in the binary you can `cargo build` from `main`.
