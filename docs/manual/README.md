...
This manual is the **single source of truth for what trunk actually ships today** (workspace version `0.5.3` — three complete EPICs: EPIC 1 Workbench MVP closed `v0.3.0` PR #208, EPIC 2 Agent autonomy closed `v0.4.0` PR #209, **EPIC 3 Plugin platform CLOSED `v0.5.0` PR #228**. **EPIC 4 (Multi-tenant business ops / XaaS) now ACTIVE** (promoted 2026-04-20) — ISSUE 11 (quotas + rate limits) in-flight with TASK 11.1 + TASK 11.2 + TASK 11.3 shipped: TASK 11.1 (PR #230 / 0.5.1) adds structured `Retry-After: 60` + `retry_after_seconds: 60` on every 429; TASK 11.2 (PR #231 / 0.5.2) adds per-tenant `TokenBucketRateLimiter` wrapped by `RateLimitedQuotaEnforcer`, opt-in via `[quota_rate_limit]`; TASK 11.3 (PR #232 / 0.5.3) adds Postgres-backed daily + monthly spend tracking via new `PgQuotaEnforcer` that runs one UPDATE per request with CASE-expression rollover (daily zeros at UTC midnight, monthly at first-of-month; no background job — rollover happens on the first post-boundary request). Pipeline: rate-limit check → pg cost check → dispatch → pg record_post. TASK 11.4 (`/web` UI surface for the 429 path) + ISSUE 12 (billing) + 13 (HF catalog) + 14 (self-service) gate EPIC 4 close → `v1.0.0`. If a behaviour is described here, it exists in the binary you can `cargo build` from `main`.
