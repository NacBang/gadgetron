...
This manual is the **single source of truth for what trunk actually ships today** (workspace version `0.4.9` — EPIC 1 Workbench MVP closed `v0.3.0` PR #208, EPIC 2 Agent autonomy closed `v0.4.0` PR #209, EPIC 3 Plugin platform ACTIVE with ISSUE 8 and ISSUE 9 both functionally complete: ISSUE 8 shipped TASK 8.1–8.5 (PRs #211 / #213 / #214 / #216 / #217) giving operators a three-trigger catalog-reload surface; ISSUE 9 shipped TASK 9.1–9.3 (PRs #219 / #220 / #222) adding `BundleMetadata`, multi-bundle aggregation via `[web] bundles_dir` + `from_bundle_dir()`, and the bundle-driven harness default with `source` enum `{"seed_p2b", "config_file", "bundles_dir"}`; **ISSUE 10 (bundle marketplace) STARTED** — TASK 10.1 shipped (PR #223) adds `GET /api/v1/web/workbench/admin/bundles` Management-scoped bundle-discovery endpoint that reads `bundles_dir` subdirectories without modifying the live catalog and returns `{bundles_dir, count, bundles: [{bundle?, source_path, action_count, view_count}, ...]}` in deterministic subdir-sorted order matching `from_bundle_dir`. TASK 10.2 (install/uninstall) + 10.3 (per-bundle scope isolation) + 10.4 (signed manifests) pending before EPIC 3 close). If a behaviour is described here, it exists in the binary you can `cargo build` from `main`.
