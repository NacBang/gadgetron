...
This manual is the **single source of truth for what trunk actually ships today** (workspace version `0.5.1` — three complete EPICs: EPIC 1 Workbench MVP closed `v0.3.0` PR #208, EPIC 2 Agent autonomy closed `v0.4.0` PR #209, **EPIC 3 Plugin platform CLOSED `v0.5.0` PR #228**. EPIC 3 delivered three ISSUEs (ISSUE 8 DescriptorCatalog hot-reload / ISSUE 9 real bundle manifests / ISSUE 10 bundle marketplace) with full hot-reload + signed-install surface. **EPIC 4 (Multi-tenant business ops / XaaS) now ACTIVE** (promoted 2026-04-20) — ISSUE 11 (quotas + rate limits) in-flight with TASK 11.1 shipped (PR #230 / 0.5.1): every `ApiError` with HTTP 429 now carries both the `Retry-After: 60` header AND a `retry_after_seconds: 60` field in the JSON body; SDK clients that honor either surface can back off deterministically instead of retrying in a tight loop. TASK 11.2 (token-bucket enforcer) pending before ISSUE 11 complete; ISSUE 12 (billing) + 13 (HF catalog) + 14 (self-service) gate EPIC 4 close → `v1.0.0`. If a behaviour is described here, it exists in the binary you can `cargo build` from `main`.
