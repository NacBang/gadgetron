...
This manual is the **single source of truth for what trunk actually ships today** (workspace version `0.4.7` — EPIC 1 Workbench MVP closed `v0.3.0` PR #208, EPIC 2 Agent autonomy closed `v0.4.0` PR #209, EPIC 3 Plugin platform ACTIVE with ISSUE 8 functionally complete: TASK 8.1 substrate via PR #211, TASK 8.2 reload endpoint via PR #213, TASK 8.3 `CatalogSnapshot { catalog, validators }` bundling via PR #214, TASK 8.4 file-based catalog source via PR #216, TASK 8.5 POSIX `SIGHUP` reloader via PR #217 — operators now have three reload triggers (HTTP endpoint, SIGHUP signal, both sharing the `perform_catalog_reload()` code path; fs-watcher deferred to TASK 8.6 if demand emerges); ISSUE 9 (real bundle manifests) progressing via PR #219 + #220 — TASK 9.1 adds `BundleMetadata { id, version }` + optional `[bundle]` TOML table + first-party bundle file at `bundles/gadgetron-core/bundle.toml` mirroring `seed_p2b()`; TASK 9.2 adds multi-bundle aggregation via `[web] bundles_dir` + `DescriptorCatalog::from_bundle_dir()` with hard collision errors on duplicate action/view ids + `bundles: Vec<BundleMetadata>` on the reload response; precedence is `bundles_dir` > `catalog_path` > `seed_p2b` fallback. TASK 9.3 (retire seed_p2b) + ISSUE 10 (bundle marketplace) pending before EPIC 3 close). If a behaviour is described here, it exists in the binary you can `cargo build` from `main`.
