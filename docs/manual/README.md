...
This manual is the **single source of truth for what trunk actually ships today** (workspace version `0.4.10` — EPIC 1 Workbench MVP closed `v0.3.0` PR #208, EPIC 2 Agent autonomy closed `v0.4.0` PR #209, EPIC 3 Plugin platform ACTIVE with ISSUE 8 and ISSUE 9 both functionally complete; ISSUE 10 in-flight with TASK 10.1 + 10.2 shipped: TASK 10.1 (PR #223) added `GET /admin/bundles` read-only enumeration, TASK 10.2 (PR #224) added `POST /admin/bundles` + `DELETE /admin/bundles/{bundle_id}` for runtime install/uninstall — install writes the TOML to `{bundles_dir}/{id}/bundle.toml`, does NOT swap the live catalog (operator triggers reload separately), id regex `[a-zA-Z0-9_-]{1,64}` rejects path-traversal, collision with an existing id is a 4xx Config error so there are no silent overwrites. TASK 10.3 (per-bundle scope isolation) + 10.4 (Ed25519 signed manifests) pending before EPIC 3 close). If a behaviour is described here, it exists in the binary you can `cargo build` from `main`.
