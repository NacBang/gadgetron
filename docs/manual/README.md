...
This manual is the **single source of truth for what trunk actually ships today** (workspace version `0.5.0` — three complete EPICs: EPIC 1 Workbench MVP closed `v0.3.0` PR #208, EPIC 2 Agent autonomy closed `v0.4.0` PR #209, **EPIC 3 Plugin platform CLOSED `v0.5.0` PR #228**. EPIC 3 delivered three ISSUEs: ISSUE 8 DescriptorCatalog hot-reload (5 TASKs, v0.4.1–v0.4.5) giving operators an HTTP + SIGHUP reload surface backed by `Arc<ArcSwap<CatalogSnapshot>>`; ISSUE 9 real bundle manifests (3 TASKs, v0.4.6–v0.4.8) with `BundleMetadata` + multi-bundle aggregation via `[web] bundles_dir` + bundle-driven harness default; ISSUE 10 bundle marketplace (4 TASKs, v0.4.9–v0.4.12) with discovery + runtime install/uninstall + per-bundle scope isolation + Ed25519 signed manifests. EPIC 4 (Multi-tenant business ops / XaaS — billing, HF catalog, tenant self-service, quotas) is the next active target toward `v1.0.0` first production release). If a behaviour is described here, it exists in the binary you can `cargo build` from `main`.
