...
This manual is the **single source of truth for what trunk actually ships today** (workspace version `0.5.2` — three complete EPICs: EPIC 1 Workbench MVP closed `v0.3.0` PR #208, EPIC 2 Agent autonomy closed `v0.4.0` PR #209, **EPIC 3 Plugin platform CLOSED `v0.5.0` PR #228**. EPIC 3 delivered three ISSUEs (ISSUE 8 hot-reload / ISSUE 9 real bundle manifests / ISSUE 10 bundle marketplace). **EPIC 4 (Multi-tenant business ops / XaaS) now ACTIVE** (promoted 2026-04-20) — ISSUE 11 (quotas + rate limits) in-flight with TASK 11.1 + TASK 11.2 shipped: TASK 11.1 (PR #230 / 0.5.1) adds structured `Retry-After: 60` header + `retry_after_seconds: 60` body field on every 429 so SDKs can back off deterministically; TASK 11.2 (PR #231 / 0.5.2) adds per-tenant `TokenBucketRateLimiter` wrapped by `RateLimitedQuotaEnforcer` composite, opt-in via `[quota_rate_limit] requests_per_minute = N, burst = M` config — default off so existing deployments see zero behavior change. TASK 11.3 (Postgres-backed daily/monthly spend tracking) + TASK 11.4 (`/web` UI surface for the 429 path — quota status banner + retry countdown) + ISSUE 12 (billing) + 13 (HF catalog) + 14 (self-service) gate EPIC 4 close → `v1.0.0`. If a behaviour is described here, it exists in the binary you can `cargo build` from `main`.
