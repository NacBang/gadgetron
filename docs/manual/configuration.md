...
# Default false. Set true ONLY when the gateway sits behind a TLS-terminating
# reverse proxy (Nginx / Caddy per installation.md §Production deployment).
# Enabling this while serving plain HTTP breaks every subresource fetch with
# ERR_SSL_PROTOCOL_ERROR — the browser tries HTTPS against an HTTP server.
...
- `upgrade_insecure_requests` (default `false`): appends the `upgrade-insecure-requests` directive to the `/web` Content-Security-Policy header. Defense-in-depth on top of browser mixed-content blocking: tells the browser to auto-upgrade any HTTP subresource fetch to HTTPS. **Only safe when TLS terminates in front of the gateway** (Nginx / Caddy). **Plain-HTTP deployments MUST leave this off** — Chrome's enforcement of this directive fires SSL against the plain-HTTP port, causing every CSS / JS / font / icon fetch to fail with `ERR_SSL_PROTOCOL_ERROR`. The page renders fully unstyled: empty-button icons, default serif text, concatenated status-bar text. Observed 2026-04-20 on a LAN deployment running the binary directly on `http://10.100.1.5:18080`.
