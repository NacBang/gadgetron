...
> **상태**: Phase 2A 설계 프리뷰. 현재 바이너리에는 아직 구현되지 않음.
> **대상 독자**: 설계 검토자 / 향후 Kairos 운영자.
...
같은 Gadgetron 바이너리가 두 레이어를 모두 제공하는 방향으로 설계되어 있습니다.

---

## 현재 상태

2026-04-15 기준 현재 저장소의 Phase 1 바이너리에는 아래 항목이 아직 없습니다.

- `gadgetron kairos ...` 서브커맨드
- `gadgetron mcp serve` 서브커맨드
- `kairos` 모델 등록
- `gadgetron-kairos` / `gadgetron-knowledge` 크레이트

이 페이지의 나머지 내용은 **계획된 Phase 2 운영 경험과 설계 계약**을 설명합니다. 아래 명령과 설정 예시는 현재 바이너리에서 그대로 실행되는 사용 설명서가 아니라, 구현 목표를 설명하는 프리뷰로 읽어야 합니다.
...
현재 Phase 1 바이너리 기준 임시 로컬 키는 아래처럼 생성합니다.

```sh
./target/release/gadgetron key create
```

PostgreSQL 기반 영속 키가 필요하면 현재 CLI의 Phase 1 경로를 사용하십시오.
