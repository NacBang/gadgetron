...
// Default gadgetini IPv6 (ULA, /64). The gadgetini ships with this address
// burned into its USB-side network config — every unit answers on the
// same address, so registration only needs to know which interface on
// the PARENT host points at the gadgetini.
pub const DEFAULT_USB_IPV6: &str = "fd12:3456:789a:1::2";

// Default name of the parent-host interface that talks to the gadgetini.
// In practice the link is USB → on-board ETH bridge → USB on the
// gadgetini, so the kernel exposes it as a USB-CDC ethernet interface
// whose name varies by chassis (`usb0`, `enpXsYf1np1`, etc.). `usb0` is
// the most common factory default; operators override per host via the
// `parent_iface` field on `GadgetiniRecord`.
pub const DEFAULT_USB_PARENT_IFACE: &str = "usb0";

// The `coolant_temp` key was originally an aggregate temp written by the
// gadgetini firmware, but on shipping units it never gets populated —
// the four inlet/outlet probes carry the actual values. Polling it just
// produced a steady stream of "—" in the UI, so it's dropped from the
// MGET list. The aggregate is still computable client-side (avg of the
// four probes) when needed.
pub const GADGETINI_REDIS_KEYS: [&str; 11] = [
    "air_humit",
    "air_temp",
    "chassis_stabil",
    "coolant_delta_t1",
...
/// How the gadgetron server reaches a gadgetini child board.
///
/// - `Usb` (default, original deployment): the gadgetini sits on the
///   parent host's USB-CDC ethernet bridge, answering at the burned-in
///   IPv6 ULA (`fd12:3456:789a:1::2/64` by default). gadgetron has no
///   direct path — it SSHes to the parent over LAN/ethernet, then runs
///   `nc -6 <ipv6_link_local> <port>` inside the parent as the
///   ProxyCommand to forward the gadgetini SSH session. The "USB" naming
///   is shorthand for the parent→gadgetini hop; the gadgetron→parent
///   leg is plain ethernet/SSH. Requires the parent's USB-CDC iface name
///   (`parent_iface`, defaults to `usb0`) and the gadgetini IPv6
///   (`ipv6_link_local`, despite the field name this is normally a ULA,
///   not fe80::; the field accepts either).
/// - `Direct`: the gadgetini has its own routable IP (chassis variants
///   without the USB tunnel, or a lab bench setup). gadgetron SSHes
///   straight to `host_name`, no ProxyCommand. `parent_iface` and
///   `ipv6_link_local` are ignored.
#[derive(Debug, Clone, Copy, Serialize, Deserialize, PartialEq, Eq, Default)]
#[serde(rename_all = "snake_case")]
pub enum GadgetiniMode {
    #[default]
    Usb,
    Direct,
}
...
pub struct GadgetiniRecord {
    ...
    pub ssh_port: u16,
    /// Parent USB interface name. Required when `mode = Usb`; ignored
    /// (and may be empty) when `mode = Direct`. Kept as `String` rather
    /// than `Option<String>` so existing inventory.json rows that
    /// pre-date the mode field deserialize unchanged.
    #[serde(default)]
    pub parent_iface: String,
    /// IPv6 link-local address of the gadgetini on `parent_iface`. Same
    /// USB-only contract as `parent_iface`.
    #[serde(default)]
...
pub fn parse_redis_mget_values(values: &[&str]) -> ParsedGadgetiniStats {
    ...
    // `coolant_temp` (the singular aggregate at the old index 6) is no
    // longer in `GADGETINI_REDIS_KEYS` — shipped firmware writes garbage
    // (negative values) into it. Indices below shifted up by one.
    parsed.stats.coolant_temp_inlet1_c =
        parse_f32(values, 6, "coolant_temp_inlet1", &mut parsed.warnings);
    parsed.stats.coolant_temp_inlet2_c =
        parse_f32(values, 7, "coolant_temp_inlet2", &mut parsed.warnings);
    parsed.stats.coolant_temp_outlet1_c =
        parse_f32(values, 8, "coolant_temp_outlet1", &mut parsed.warnings);
    parsed.stats.coolant_temp_outlet2_c =
        parse_f32(values, 9, "coolant_temp_outlet2", &mut parsed.warnings);
    parsed.stats.host_status_code = parse_i64(values, 10, "host_stat", &mut parsed.warnings);
...
pub fn build_child_ssh_argv(
    ...
    // Direct mode: SSH straight from the gadgetron server to the
    // gadgetini IP. No parent involvement, no ProxyCommand. host_name
    // MUST be set (parse_gadgetini_record enforces this).
    if matches!(record.mode, GadgetiniMode::Direct) {
...
pub fn build_child_password_sshpass_argv(
    ...
    // Direct mode skips ProxyCommand entirely and sshpass authenticates
    // with the gadgetini's own SSH daemon at host_name:ssh_port.
    if matches!(record.mode, GadgetiniMode::Direct) {
        let child_host = record
            .host_name
            .as_deref()
...
/// Hard cap on the gadgetini password-bootstrap step. ConnectTimeout=8
/// in the SSH argv only covers TCP connect — once auth begins, ssh has
/// no built-in deadline. A wrong factory password leaves sshpass stuck
/// at the auth prompt indefinitely (we observed 4-minute stalls in the
/// UI before adding this), so wrap the whole sshpass invocation with a
/// tokio timeout that kills the child if it doesn't complete in time.
const CHILD_PASSWORD_BOOTSTRAP_TIMEOUT: Duration = Duration::from_secs(30);
...
async fn exec_child_with_password(
    ...
    let output = match tokio::time::timeout(
        CHILD_PASSWORD_BOOTSTRAP_TIMEOUT,
        child.wait_with_output(),
    )
    .await
    {
        Ok(Ok(out)) => out,
        Ok(Err(e)) => return Err(SshError::Io(format!("wait gadgetini sshpass: {e}"))),
        Err(_) => {
            return Err(SshError::Failed {
                code: 124,
                stderr: format!(
                    "gadgetini password bootstrap timed out after {}s — \
                     usually means the password was rejected and sshpass is \
                     waiting on a follow-up prompt. Verify the gadgetini's \
                     factory password (or set a custom one in the UI) and retry.",
                    CHILD_PASSWORD_BOOTSTRAP_TIMEOUT.as_secs()
                ),
            });
        }
    };
...
",
    ...
    fn redis_mget_command_uses_fixed_key_order() {
        assert_eq!(
            redis_mget_command(),
            "redis-cli --raw MGET air_humit air_temp chassis_stabil coolant_delta_t1 coolant_leak coolant_level coolant_temp_inlet1 coolant_temp_inlet2 coolant_temp_outlet1 coolant_temp_outlet2 host_stat"
        );
    }
