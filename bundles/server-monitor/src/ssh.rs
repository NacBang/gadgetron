...
pub async fn generate_keypair(priv_path: &Path, comment: &str) -> Result<(), SshError> {
    ...
    let pub_path = pubkey_path_for(priv_path);
...
/// Append `.pub` to `priv_path` rather than `with_extension("pub")`,
/// which would replace any existing extension. ssh-keygen's `-f <path>`
/// always emits `<path>.pub` (append), so for a key at `<id>.gadgetini`
/// the companion is `<id>.gadgetini.pub`, not `<id>.pub`.
pub fn pubkey_path_for(priv_path: &Path) -> PathBuf {
