...
mod tests {
    ...
    fn stats_to_samples_fans_out_gadgetini_cooling_metrics() {
        ...
        // `cooling.coolant_temp` is intentionally absent — see
        // `GADGETINI_REDIS_KEYS` for the rationale.
        assert!(!names.contains(&"cooling.coolant_temp"));

        let inlet1 = samples
            .iter()
            .find(|s| s.metric == "cooling.coolant_inlet1_temp")
            .expect("inlet1 sample");
        assert!((inlet1.value - 39.2).abs() < 0.001);
        assert_eq!(inlet1.unit.as_deref(), Some("celsius"));
        assert_eq!(inlet1.labels, json!({ "source": "gadgetini" }));
    }
