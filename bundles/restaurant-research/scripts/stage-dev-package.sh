...
bash "$ROOT/bundles/restaurant-research/scripts/build-package.sh" "$BUILD_DIR"
...
publish_tmp=$(mktemp -d "$INSTALL_ROOT/.restaurant-research.XXXXXX")
backup=""
cleanup() {
  rm -rf "$publish_tmp"
...
if [[ -e "$TARGET" ]]; then backup="$INSTALL_ROOT/.restaurant-research.previous.$$"; mv "$TARGET" "$backup"; fi
...
echo "Restaurant Research development package installed: $TARGET"
