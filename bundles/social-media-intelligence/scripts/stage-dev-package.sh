...
umask 077
if [[ ! -f "$KEY_PATH" ]]; then openssl genpkey -algorithm Ed25519 -out "$KEY_PATH"; fi
[[ ! -L "$KEY_PATH" && -f "$KEY_PATH" ]] || { echo "development signing key must be a regular file" >&2; exit 1; }
chmod 0600 "$KEY_PATH"

bash "$ROOT/bundles/social-media-intelligence/scripts/build-package.sh" "$BUILD_DIR"
...
publish_tmp=$(mktemp -d "$INSTALL_ROOT/.social-media-intelligence.XXXXXX")
backup=""
cleanup() {
  rm -rf "$publish_tmp"
...
if [[ -e "$TARGET" ]]; then backup="$INSTALL_ROOT/.social-media-intelligence.previous.$$"; mv "$TARGET" "$backup"; fi
...
echo "Social Media Intelligence development package installed: $TARGET"
