//! `gadgetron-bundle-document-formats` — document ingestion format extractors.
//!
...
mod tests {
    ...
    fn pdf_plug_is_declared_when_feature_enabled() {
        // Companion of the markdown test. Regression lock — the
        // manifest drift-check depends on the `pdf` PlugId
        // appearing iff the `pdf` feature was compiled in.
        let b = DocumentFormatsBundle::new();
        let ids: Vec<&str> = b.manifest().plugs.iter().map(|p| p.as_str()).collect();
        #[cfg(feature = "pdf")]
        assert!(
