...
//! text RAW bytes.
//!
//! Markdown is the canonical wiki format, so the extractor only validates
//! UTF-8 and emits `StructureHint::Heading` for each ATX-style `#`-prefix
//! line. The chunker (`gadgetron-knowledge::chunking`) consumes those hints
//! for heading-aware splitting.
//!
//! # What this extractor does NOT do (deferred)
//!
//! - Inline code fences (```), tables, setext headings (`===` / `---`) — the
//!   chunker still works without them; future work adds richer hint emission.
...
impl Extractor for MarkdownExtractor {
    ...
    async fn extract(
        ...
        // UTF-8 is mandatory — the pipeline rejects non-UTF-8 rather
        // than silently replacing. A hint could opt into encoding
        // guessing in a later iteration.
