...
bash "$ROOT/bundles/news-intelligence/scripts/build-package.sh" "$BUILD_DIR"
...
publish_tmp=$(mktemp -d "$INSTALL_ROOT/.news-intelligence.XXXXXX")
backup=""
cleanup() {
  rm -rf "$publish_tmp"
...
if [[ -e "$TARGET" ]]; then backup="$INSTALL_ROOT/.news-intelligence.previous.$$"; mv "$TARGET" "$backup"; fi
...
echo "News Intelligence development package installed: $TARGET"
