//! Independent News Intelligence Bundle runtime.
...
pub struct NewsIntelligenceHandler;

#[async_trait]
impl BundleGadgetHandler for NewsIntelligenceHandler {
    async fn health(&self, broker: &SharedBundleBroker) -> HealthReport {
        for table in [
            "news_article_snapshots",
            "news_events",
            "news_claims",
            "news_briefings",
        ] {
            for permission in ["news-read", "news-write"] {
                let resource =
        ...
        match broker
            ...
            .await
            .probe(
                LocalId::new("news-collections").expect("static permission is valid"),
                collection,
            )
            .await
        {
            Ok(result) if result.readiness == BrokerResourceReadiness::Ready => {
                HealthReport::with_message(
                    ...
                    "News topics and derived knowledge storage are ready",
                )
            }
            Ok(result) => HealthReport::with_message(
                ...
                result
                    .message
                    .unwrap_or_else(|| "News Topic collection is unavailable".into()),
            ),
            Err(error) => HealthReport::with_message(
                HealthStatus::Degraded,
                format!("News Topic probe failed: {}", error.public_message()),
            ),
        }
    }
    ...
    async fn invoke(
        ...
        news::invoke(invocation, broker).await
    }
}
