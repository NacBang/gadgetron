...
use gadgetron_bundle_news_intelligence::NewsIntelligenceHandler;
...
async fn main() {
    let identity = BundleRuntimeIdentity::new(
        BundleId::new("news-intelligence").expect("static Bundle id is valid"),
    ...
    let mut runtime = GadgetBundleRuntime::new(
        ...
        "News Intelligence",
        NewsIntelligenceHandler,
    )
    .unwrap_or_else(|error| {
        eprintln!("cannot initialize News Intelligence runtime: {error}");
        std::process::exit(2);
    });
    ...
    if let Err(error) = runtime.serve(tokio::io::stdin(), tokio::io::stdout()).await {
        eprintln!("News Intelligence runtime stopped: {error}");
        std::process::exit(1);
    }
}
