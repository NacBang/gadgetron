...
fn package_is_independent_cited_and_zero_egress() {
    ...
    assert_eq!(package.bundle.id.as_str(), "community-intelligence");
    ...
    assert!(package
        ...
        .gadgets
        .iter()
        .all(|gadget| gadget.name.as_str().starts_with("community.")));
    ...
    let providers = &package.capabilities.collection_profiles[0].query_providers;
    assert_eq!(providers.len(), 2);
    assert!(!providers[0].requires_configuration);
    assert!(providers[1].requires_configuration);
    ...
    .unwrap();
    assert!(ontology.type_by_id("SolutionPattern").is_some());
    ...
    let distillation: serde_json::Value =
        serde_json::from_slice(&fs::read(root.join("recipes/solution-distillation.json")).unwrap())
            .unwrap();
    assert!(distillation["core_review_output"]
        .as_array()
        .unwrap()
        .iter()
        .any(|value| value == "typed_cross_bundle_relations"));
    assert!(distillation["rules"]
        .as_array()
        .unwrap()
        .iter()
        .filter_map(serde_json::Value::as_str)
        .any(|rule| rule.contains("properties.applies_to")));
}
