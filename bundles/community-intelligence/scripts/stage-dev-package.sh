...
bash "$ROOT/bundles/community-intelligence/scripts/build-package.sh" "$BUILD_DIR"
...
publish_tmp=$(mktemp -d "$INSTALL_ROOT/.community-intelligence.XXXXXX")
backup=""
cleanup() {
  rm -rf "$publish_tmp"
...
if [[ -e "$TARGET" ]]; then backup="$INSTALL_ROOT/.community-intelligence.previous.$$"; mv "$TARGET" "$backup"; fi
...
echo "Community Intelligence development package installed: $TARGET"
