...
use gadgetron_bundle_community_intelligence::CommunityIntelligenceHandler;
...
async fn main() {
    let identity = BundleRuntimeIdentity::new(
        BundleId::new("community-intelligence").expect("static Bundle id is valid"),
    ...
    let mut runtime = GadgetBundleRuntime::new(
        ...
        CommunityIntelligenceHandler,
    )
    .unwrap_or_else(|error| {
        eprintln!("cannot initialize Community Intelligence runtime: {error}");
        std::process::exit(2);
    });
    ...
    if let Err(error) = runtime.serve(tokio::io::stdin(), tokio::io::stdout()).await {
        eprintln!("Community Intelligence runtime stopped: {error}");
        std::process::exit(1);
    }
}
